/* $Id: eazel-proxy-util.c,v 1.1.1.1 2001-01-16 15:26:28 ghudson Exp $
 * 
 * eazel-proxy-util:
 * 
	  ...
	  const char *login_path)
{
	...
	user = eazel_proxy_util_do_login (cntl, login,  
				...
				target_path, login_path, &fail_code
	       );
	...
	if ( NULL != user ) {
		...
		printf ("Login FAILED '%s' (%d).\n", ammonite_fail_code_to_string (fail_code), fail_code);
	}
}
...
do_login_prompt (
	...
	if (!gnome_inited) {
		fprintf (stderr, "Error initing GNOME\n");
		exit (-1);
	}

	user = ammonite_do_prompt_login (NULL, target_path, login_path, &fail_code);
