...
static void  WcxLateBinder_PossiblyRemoveCallback  _(( WcLateBind ));
    ...
    XtPointer	clientData, callData;
{
    ...
    WcxLateBinder_PossiblyRemoveCallback( lb );
}

/*  -- Remove Late Binding Callback
************************************************************************
    This seems like a hack, I am sure I will refine this.  Ideas anyone?

    WcOnceOnlyCB is the only caller of this external procedure.  After
    WcLateBinderCB invokes a method or callback, the WcLateBinderCB
    with the clientData may possibly be removed.
*/
    ...
    WcLateBind	lb;
{
	 ...
	 * If there is anything between the parens in the
	 * resource file, pass that (lb->args).  Otherwise,
	 * pass the clientData provided when the callback
	 * was registered (lb->regClosure).
	 *===============================================
	 */
	if ( WcNull(lb->args) && lb->regClosure != NULL )
	    lb->Callback( lb->widget, lb->regClosure, lb->callData );
	else
...
"Wcl Warning: WcLateBinder cannot invoke `%s'\n\
    ...
    WcLateBind	lb;
{
    ...
    if ( lb->object == (XtPointer)0  &&  wcl->verboseWarnings )
    {
	/*==========================================
...
"Wcl Warning: WcLateBinder trying to invoke `%s'\n\
