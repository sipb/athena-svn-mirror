...
*
*  Applications should also always call WcAppName( argv, argc ) and
*  WcAppClass( argc, argv ) to get the application name and class from the
*  command line, which can then be passed to XtInitialize().  WcAppName() and
*  WcAppClass() both look for -name <appName> in the argv, or else they use
*  argv[0].
*
...
#define WcNwclAppClassName		"wclAppClassName"
...
#define WcCWclAppClassName		"WclAppClassName"
...
/* -- Un-set the object pointer related to all widgets.
 *    All references to the "this" pointer are removed.
 */
extern void WcDetachThis _((
		XtPointer	/* "this" pointer, pointer to application obj */
));
...
/*============================================================================
 * Initialization and initial widget tree creation routines:
 *
 * Many applications simply use the function WcWidgetCreation() to initialize
 * Wcl, install the Wcl provided Xrm converters, and to create the initial
 * non-empty widget tree.  WcWidgetCreation() complains and returns 1 if no
 * popups or children of the application shell could be created.
 *
 * WcInitialize() must be called before any widgets are created by Wcl.  If
 * your application starts out without an initial widget tree, then you
 * should invoke WcInitialize() and then dynamically create the widget trees
 * later as you need them.
 *
 * WcAddConverters() installs the Wcl provided Xrm converters:
 * StringToWidget, StringToWidgetList, StringToCallback, , and Wcl private
 * converters StringToQuarkRec and StringToDynamicLibs.  Most applications
 * do not need to call WcAddConverters(), as it is called by
 * WcInitialize().  However, if you do something which might clobber the
 * installed converters, then you need to call WcAddConverters() to ensure
 * the Wcl converters are used.  This probably only happens after you
 * initialize a widget class which installs a string-to-widget converter,
 * such as the Athena Form widget or any Motif widget.  FYI, libXmp and
 * libXp know this, and they ensure that the Wcl converters are installed
 * after all Motif or Athena converters are installed.  See Xmp/Xmp.c and
 * Xp/Xp.c for details, specifically XmpRegisterMotif() and
 * XpRegisterAthena().
 */
...
extern void WcAddConverters _((		/* called by WcInitialize */
		XtAppContext
));
 ...
 */
extern char* WcWidgetToFullName _(( Widget ));			/* USE XtFree */

/*  -- Print the widget tree starting from this widget
*/
extern void WcPrintTree  _(( Widget ));

/* -- Returns the class name of the widget - DO NOT FREE RETURN VALUE!
*/
extern char* WcWidgetClassName _(( Widget ));			/* DONT FREE */

/* -- Returns the name of the XEvent - DO NOT FREE RETURN VALUE!
*/
extern char* WcXEventName _(( XEvent* ));			/* DONT FREE */

/*============================================================================
 * Parsing and various string utilities.
 */

/* -- Obtain application name and class from the argv passed to main().
 *    The caller must free return values with XtFree().
 */
extern char* WcAppName _((
...
/* -- Break and Splice Lines using columns per line.
 *
 *    WcBreakIntoLines() takes the argument string and inserts newlines to
 *    make lines less than or equal to columnsPerLine characters on each line.
 *    It tries to be intelligent, breaking lines at whitespace, or at
 *    columnsPerLine if no whitespace in the line.  Multiple blanks following
 *    `.' are collapsed into a single blank.
 *
 *    WcSpliceLines() removes newlines which look like they were inserted by
 *    WcBreakIntoLines().  Newlines which preceed lines beginning with
 *    whitespace are kept, as are newlines which seem intentional: i.e.,
 *    following punctuation where the next word could have fit on the line.
 *
 *    Both functions allocate the return value using XtMalloc(), so free after
 *    use with XtFree().
 */
extern char* WcBreakIntoLines _(( String, int columnsPerLine ));
extern char* WcSpliceLines    _(( String, int columnsPerLine ));
