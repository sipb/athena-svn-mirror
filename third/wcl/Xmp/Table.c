...
#ifdef sun			/* really `if OpenWindows'               */
...
/*ARGSUSED*/
void XmpCvtStrToXrmNameListLists( args, num_args, from, to )
    XrmValue* args;		/* Arguments to converter */
    Cardinal* num_args;		/* Number of arguments    */
    XrmValue* from;		/* From type              */
    XrmValue* to;		/* To type                */
{
    ...
    if (*num_args != 0) {
	XtErrorMsg("XmpCvtStrToXrmNameListLists", "wrongParameters",
		   "XtToolkitError",
...
static void XmpTableClassInitialize()
{
    ...
    XtAddConverter(	XtRString, XtRXrmNameListLists,
			XmpCvtStrToXrmNameListLists, (XtConvertArgList)0, 0 );
}
...
#endif
{
	...
	*/
	XmpTableForgetProposedLayout( new );
	...
	else if ( new->table.real_layout == (XmpTableLoc)0 )
	{
	    /* SetValues during creation, or before any layout has been
	     * done.  We need to do a complete recomputation and placement
	     */
	    XmpTableNewLayout( new );
	}
	else
    ...
    XtWidgetGeometry *reply;	/* Actual reply to request   */
{
    ...
    */
     ...
     * requesting pre-approved sizes.  Note: the request_mode must be only
     * for any combination of the pre-approved sizes and nothing else --
     * XtCWQueryOnly MUST NOT BE set!
     *
     * For awhile, this `if' required the request_mode to match the
     * approved_mode.  Some widgets (XmFrame) are broken in this regard
     * (XmFrame queries only width so Table uses request width and current
     * height, then XmFrame requests resize of both width & height even
     * though only width changes).  This would cause an endless loop,
     * because the mode never matched.
     */
    if ( tw->table.approved_child        == child
      ...
      && 0 == ( request->request_mode & !(CWWidth|CWHeight|CWBorderWidth) ) )
    {
	/* Request is equivalent to approved.
	*/
...
**==============================**
   ...
   data.
*/
static void XmpTableRemoveFromDefaultLayoutCB _((Widget,XtPointer,XtPointer));
    ...
    XmpTableLoc    loc;
{
    ...
    XtAddCallback( loc->w, XtNdestroyCallback,
		   XmpTableRemoveFromDefaultLayoutCB, (XtPointer)tw );
}
...
*/
/*ARGSUSED*/
static void XmpTableRemoveFromDefaultLayoutCB( w, client, call )
    Widget    w;
    XtPointer client, call;
{
    XmpTableWidget tw  = (XmpTableWidget)client;
    XmpTableLoc    loc = XmpTableLocFind( tw->table.default_layout, w );
...
*/
void XmpTableConsiderSameSize( tw )
    XmpTableWidget tw;
{
    XmpTableLoc loc, real_layout;
    int         same_what;
    ...
    XmpTableWidget tw;
{
    ...
    */
	...
	tw->table.query_mode   = query.request_mode & (CWWidth | CWHeight);
    ...
    XmpTableWidget tw;
{
	...
	if ( x      != loc->w->core.x
	  || y      != loc->w->core.y
	  || width  != loc->w->core.width
	  || height != loc->w->core.height
	  || bw     != loc->w->core.border_width )
	{
	    XmeConfigureObject( loc->w, x, y, width, height, bw );
	}
...
#else
	    _XmResizeObject( (RectObj)(loc->w), width, height, bw );
#endif
	}
	...
	if ( x != loc->w->core.x 
	  || y != loc->w->core.y )
	{
#if (XmVERSION == 1 && XmREVISION >= 2 && XmUPDATE_LEVEL >= 2) || XmVERSION > 1
    ...
    int			*xP, *yP;		/* out */
{
    ...
    */
	...
	new_y = cell_y + (cell_h - total_h)/2;	/* center in cell         */
    ...
    String optString;
{
    ...
    /* null, Null, NULL, none, NONE, ...  Anything starting with `N'
    */
    if ( optString && ( *optString == 'n' || *optString == 'N') )
	return 0;
