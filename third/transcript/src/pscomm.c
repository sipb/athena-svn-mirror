...
_NOTICE RCSID[] = "$Header: /afs/dev.mit.edu/source/repository/third/transcript/src/pscomm.c,v 1.1 1996-10-14 05:00:27 ghudson Exp $";
#endif
/* pscomm.c
 *
 * Copyright (C) 1985,1986,1987,1990,1991,1992 Adobe Systems Incorporated.
 * All rights 
 * reserved. GOVERNMENT END USERS: See Notice file in TranScript library
 * directory -- probably /usr/lib/ps/Notice
 *
 * 4.2BSD lpr/lpd communications filter for PostScript printers (formerly
 * "psif" in TranScript release 1.0)
 *
 * pscomm is the general communications filter for sending files to a
 *  PostScript printer via RS232 lines.  It does page accounting, error
 *  handling/reporting, job logging, banner page printing, etc. It observes
 *  (parts of) the PostScript file structuring conventions. In particular,
 *  it distinguishes between PostScript files (beginning with the "%!"
 *  magic number) -- which are shipped to the printer -- and text files (no
 *  magic number) which are formatted and listed on the printer.  Files
 *  which begin with "%!PS-Adobe-" may be page-reversed if the target
 *  printer has that option specified.
 *
 * depending on the values of BANNERFIRST and BANNERLAST, pscomm looks for a
 * file named ".banner", (created by the "of" filter) in the current
 * working directory and ships it to the printer also.
 *
 ...
 *    REVERSE - page reversal filter program (no reversal if null or missing)
 *    PSLIBDIR - transcript library directory
 *    JOBOUTPUT	- file for actual printer stream output (if defined)
 *
 * General flow of control for communications: 1) "Sync" with printer: Send
 * small accounting job, and make sure reply matches what we sent.  If not
 * a match, sleep a little and try again. 2) Send user job: Start listener
 * process, and write user job to printer. Banner may get sent first.
 * CTRL-T gets sent occasionally. Listener get ALL output from printer. If
 * listener sees "idle" status message it aborts, which flushes the job
 * (printer rebooted). End-of-job character is written to printer after
 * job has been sent. 3) Wait for user job to complete: Listener process
 * exits when end-of-job character is received from printer.  "idle"
 * status will make job abort as in (2) above. 4) If doing accounting or
 * "bannerlast", use accounting job to "sync" printer again.  A banner may
 * be sent before the accounting job. Note that only output from the user
 * job is read by the listener process. Output from the banner, for
 * instance, is ignored (there shouldn't be any). Also, note that jobs
 * containing or printing end-of-job characters will may print correctly
 * -- the "sync" process guarantees the next job will be executed
 * correctly, however.
 *
...
Revision 3.3  1992/07/14  22:47:16  snichols
Updated copyright.

Revision 3.2  1992/05/26  21:50:32  snichols
comments after endif

Revision 3.1  1992/01/13  22:32:09  snichols
work-around for bug in serial driver in Laserwriter IIf and IIg.

Revision 3.0  1991/06/17  16:50:45  snichols
Release 3.0

Revision 2.9  1991/06/11  23:34:42  snichols
cleaned up some include file stuff - don't need sys/resource.h, should
include time.h, rather than sys/time.h

Revision 2.8  1990/12/12  10:20:54  snichols
missed a spot in the cleanup.

 * Revision 2.7  90/12/12  10:19:03  snichols
 * new configuration stuff
 *
 * Revision 2.6  90/12/11  09:52:25  snichols
 * cleaned up after emacs went wild reformatting comments.
 *
 * Revision 2.5  90/11/21  10:15:22  snichols
 * handle status messages that don't end with \r\n.
 *
 * Revision 2.4  90/11/16  14:12:09  snichols
 * use str(r)chr instead of (r)index (XPG3).  Added pipelining.  Fixed
 * various error messages with missing parameters.
 *
 * Revision 2.3  90/10/11  15:03:15  snichols
 * Now only deals with communication.  Page reversal, text2PS
 * conversion, and other features moved into psdman.
 *
 * Revision 2.1  90/07/10  14:49:37  snichols
 * Re-sync versions and releases
 *
 * Revision 2.2  87/11/17  16:51:17  byron Release 2.1
 *
 ...
 * 1. Added check for printer timeout, which aborts job and tries
 * again. 2. Added warning if CTRL-D is found while job is still being sent.
 *
 *
 ...
 * Also, added more code to give up when job is aborted and
 * things look confused.  Added errno declaration for portability.
 *
 * Revision 2.1.1.11  87/06/25  15:58:45  byron
 * Bug in page reverser process critical region.  Would have caused
 * pscomm to hang if reverser got done before parent continued
 * (and redid stdin).
 *
...
#ifdef BRIDGE			/* Unsupported code for Bridge
...
#ifdef HAVE_SYS_WAIT_H    /* defined only if sys/wait.h is POSIX.1 compliant */
#define WAIT_STATUS int
#else
#define WAIT_STATUS union wait
#ifndef WIFSIGNALED
#define WIFSIGNALED(exit) (exit.w_termsig != 0)
#endif
#ifndef WTERMSIG
#define WTERMSIG(exit) (exit.w_termsig)
#endif
#ifndef WEXITSTATUS
#define WEXITSTATUS(exit) (exit.w_termsig)
#endif
#endif
...
private time_t starttime;	/* Timer start. For status warnings */
private int saveerror;		/* Place to save errno when exiting */

private int brokenserial = FALSE;
...
private WAIT_STATUS status;	/* Return value from wait() */
...
donefile:			/* Done sending the user's file */
	...
	if (!pipeline) {
	    ...
	    while (!childdone)
		pause();	/* Wait for listener to finish */
	    VOIDC alarm(0);
	}
	...
	/* This process listens while the user job is sent. It
	   communicates with the parent by signalling a SIGEMT, then
	   exiting.  The exit code is the only form of communication
	   implemented. The user job is aborted: A) If we get a status of
	   "idle" back from the printer (see NextCh() routine), or B) If
	   we get EOF back while reading the printer (somebody unplugged
	   the comm line?), or C) If we receive a SIGEMT signal (parent
	   wants us to die). This process ignores most signals except
	   SIGEMT.
	
	A signal is used before exiting to make it easy for the parent to
	   use wait() to get the child status, and for portability to
	   System V. */
	register int r;
	register ChStat i;
...
WAIT_STATUS exitstatus;     /* Status returned by the child */
{
    debugp((stderr, "%s: Listener return status: 0x%x\n", prog, exitstatus));
    if (WIFSIGNALED(exitstatus)) {	/* Some signal got the child */
	...
	switch (WEXITSTATUS(exitstatus)) { /* Depends on child's exit status */
...
#else
    if (psin != NULL)
	VOIDC fclose(psin);	/* Close old one */
    ...
    long *pagecount;		/* The current page count in the printer */
{
	...
	VOIDC sprintf(mybuf, getpages, mpid, synccount, "\004", "\004");
	...
	*mp = '\0';
	while (TRUE) {		/* Listen for return string */
	    ...
	    if (r == '*') {
		/* from second little job tacked onto accounting job */
		seenstar++;
		continue;
	    }
	    if ((r & 0377) == 004) {
		/* may need to cope with printer sending back 2 ctrl-d's */
		if (firstone) {
		    /* first ctrl-d, we know that there is at least 1 more
		       */ 
		    firstone = FALSE;
		    continue;
		}
		if (seenstar && !brokenserial) {
		    /* we saw the star before seeing 2nd ctrl-d, so this
		       isn't a broken one, and we're done */
		    break;
		}
		if (!seenstar) {
		    /* we're seeing the 2nd ctrl-d before the star, broken
		       serial driver, two more coming */
		    brokenserial = TRUE;
		    continue;
		}
		/* we only get here if we've seen the third ctrl-d, so read
		   the 4th to get rid of it, and get out of here */
		r = getc(psin);
		break;
	    }
	...
	*mp = '\0';
	...
	if (sleeptime > MAXSLEEP) {
	    if (jobaborted)
		dieanyway();	/* If already aborted, just croak */
	    sleeptime = MAXSLEEP;
	}
tryagain:
    ...
    VOIDC alarm(0);		/* Make sure we don't longjmp() after
				   exiting */
}
