...
_NOTICE RCSID[]="$Header: /afs/dev.mit.edu/source/repository/third/transcript/src/pscatmap.c,v 1.1.1.1 1996-10-07 20:25:50 ghudson Exp $";
#endif
/* pscatmap.c
 *
 ...
 * Revision 3.2  1993/05/25  21:24:12  snichols
 * cleanup for Solaris.
 *
 ...
 * Release 3.0
 *
 * Revision 2.8  1991/06/12  00:14:36  snichols
 * added forward declarations.
 *
 * Revision 2.7  1991/03/27  00:46:01  snichols
 * only include string.h.
 *
 * Revision 2.6  1991/03/25  21:28:38  snichols
 * brought it up to date with rest of software in using strchr.
 *
 * Revision 2.5  1991/03/01  14:04:42  snichols
 * Use default resource path in ListPSResourceFiles.
 *
 * Revision 2.4  91/02/25  17:04:33  snichols
 * Use PSRes package for mapping font name to AFM file name.
 * 
 * Revision 2.3  90/12/12  10:09:45  snichols
...
private GetFont () {
    ...
    *eqp++ = '\0';
    ...
    if (*eqp == '/') {
	VOIDC strncpy(afmpath,eqp, 200);
    }
    else {
	count = ListPSResourceFiles(NULL, ResourceDir, PSResFontAFM, eqp,
				    &names, &files);
	if (count == 0) {
	    fprintf(stderr,"No AFM file found for %s\n", eqp);
	    exit(1);
	}
	VOIDC strncpy(afmpath, files[0], 200);
    }
    ReadAFM(afmpath);
    nfonts++;
    return;
...
#endif    
{
    if (((struct ctab *)a)->catcode < ((struct ctab *)b)->catcode)
	return -1;
    if (((struct ctab *)a)->catcode > ((struct ctab *)b)->catcode)
	return 1; 
    return 0;
...
private WriteTable() {
    ...
    qsort((void *) ctab, nchars, sizeof (struct ctab), compCTent);
    /* write it out */
...
{
    register int i;
    i = ((struct ctab *)a)->trcode - ((struct ctab *)b)->trcode;
    if (i == 0) return ((((struct ctab *)a)->catcode>>7) -
			(((struct ctab *)b)->catcode>>7));
    return(i);
}
...
private DoWidths() {
    ...
    qsort((void *) ctab, nchars, sizeof(struct ctab), compTRcode);
    for (f = 0; f < 4; f++) {
	dofont(f);
    }
}
...
int  fontno; /* troff font number 0-3 */
{
	...
	if ((ct >= &ctab[nchars]) || (curcode != ct->trcode) ||
	    ...
	    if (strchr(Ascenders, curcode)) asde |= ASCENDER;
	    if (strchr(Descenders, curcode)) asde |= DESCENDER;
	}
#ifdef SYSV
