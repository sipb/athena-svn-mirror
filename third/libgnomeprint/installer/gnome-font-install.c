...
static void gfi_verify_font_entry (GFFontEntry *e, gboolean forced);
static void gfi_verify_afm_file (GFFontEntry *e, GFFileEntry *f, gboolean forced);
static void gfi_verify_pfb_file (GFFontEntry *e, GFFileEntry *f, gboolean forced);
static void gfi_verify_ttf_file (GFFontEntry *e, GFFileEntry *f, gboolean forced);
...
static gboolean gfi_refresh = FALSE;
...
static const struct poptOption options[] = {
	...
	{ "refresh", 0, POPT_ARG_NONE, &gfi_refresh, 0,
	  N_("Reverifies all font files, even for fontmaps of same version"), NULL },
	...
	{ "dynamic", 'd', POPT_ARG_NONE, &gfi_dynamicmap, 0,
	  N_("Create " FONTMAPDIR_DYNAMIC "/gnome-print.fontmap"), NULL },
	{ "static", 's', POPT_ARG_NONE, &gfi_staticmap, 0,
	  N_("Create " FONTMAPDIR_STATIC"/gnome-print.fontmap"), NULL },
	...
	{ "dir", 0, POPT_ARG_NONE, &gfi_query_dirs, 0,
	  N_("Return the $FONTMAPDIR_STATIC and $FONTMAPDIR_DYNAMIC used for this gnome-print installation)"), NULL },
...
/* It is safe to add as many more as you want */
...
static const guchar *smart_aliases[] = {
	DATADIR "/gnome/libgnomeprint-2.0/fonts/adobe-urw.font",
	DATADIR "/gnome/fonts/adobe-urw.font",
	DATADIR "/fonts/adobe-urw.font",
	"/usr/share/gnome/libgnomeprint-2.0/fonts/adobe-urw.font",
...
main (int argc, const char ** argv)
{
	...
	/* Directories */
	if (gfi_query_dirs) {
		fprintf (stderr, "$FONTMAPDIR_STATIC=%s\n", FONTMAPDIR_STATIC);
		fprintf (stderr, "$FONTMAPDIR_DYNAMIC=%s\n", FONTMAPDIR_DYNAMIC);
		return 0;
	}
	...
	if (gfi_staticmap) {
		...
		if (!gfi_target) gfi_target = g_strdup (FONTMAPDIR_STATIC "/gnome-print.fontmap");
	} else if (gfi_dynamicmap) {
		mastermap = db->dynamicmaps;
		newmap = gf_fontmap_new (GF_FONTMAP_DYNAMIC, mastermap->path);
		if (!gfi_target) gfi_target = g_strdup (FONTMAPDIR_DYNAMIC "/gnome-print.fontmap");
	} else {
		mastermap = db->usermaps;
		newmap = gf_fontmap_new (GF_FONTMAP_USER, mastermap->path);
		if (!gfi_target) gfi_target = g_build_filename (g_get_home_dir (), ".gnome/fonts/gnome-print.fontmap", NULL);
	}
...
gfi_verify_fontmap (GFFontMap *map)
{
	while (map) {
		...
		for (e = map->fonts; e != NULL; e = e->next) {
			gfi_verify_font_entry (e, gfi_refresh || (map->gpversion < ourversion));
...
static void
gfi_verify_font_entry (GFFontEntry *e, gboolean forced)
{
	switch (e->type) {
	case GF_FONT_ENTRY_TYPE1:
		if (e->isalias) {
			...
			/* Verify pfb as usual */
			gfi_verify_pfb_file (e, &e->files[0], forced);
	...
	case GF_FONT_ENTRY_TRUETYPE:
		gfi_verify_ttf_file (e, &e->files[0], forced);
		break;
	default:
			...
			if (seemsok) {
				...
				if (dir) {
					while ((dent = readdir (dir))) {
						...
						fn = g_build_filename (path, dent->d_name, NULL);
						if (!stat (fn, &s)) {
...
gfi_try_font_file (const guchar *fn)
{
	...
	if (!g_path_is_absolute (fn)) {
		...
		name = g_build_filename (cdir, fn, NULL);
	} else {
		name = g_strdup (fn);
	}
...
gfi_process_alias (GFIAliasData *a)
{
	...
	/* Register it */
	if (gfi_debug) fprintf (stderr, "Registered Type1 alias: %s/%s/%s\n", new->familyname, new->speciesname, new->name);
...
gfi_ensure_directory (const gchar *path)
{
	...
	if (!g_path_is_absolute (path)) {
		gchar *cdir;
		cdir = g_get_current_dir ();
		t = g_build_filename (cdir, path, NULL);
	} else {
		t = g_strdup (path);
	}
	l = NULL;
	...
	while (l->next) {
		gchar *n;
		n = g_build_filename (p, (gchar *) l->data, NULL);
