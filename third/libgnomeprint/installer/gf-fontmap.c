...
static void gf_fm_load_font_2_0_type1 (GFFontMap *map, xmlNodePtr node, gboolean isalias);
...
gf_font_db_new (void)
{
	...
	path = g_build_filename (homedir, ".gnome/fonts/gnome-print.fontmap", NULL);
	map = gf_fontmap_new (GF_FONTMAP_USER, path);
	db->usermaps = map;
	g_free (path);

	path = g_strdup (FONTMAPDIR_DYNAMIC "/gnome-print.fontmap");
	map = gf_fontmap_new (GF_FONTMAP_DYNAMIC, path);
	db->dynamicmaps = map;
	g_free (path);

	path = g_strdup (FONTMAPDIR_STATIC "/gnome-print.fontmap");
...
gf_font_db_load (void)
{
	...
	/* User maps */
	homedir = g_get_home_dir ();
	mapdir = g_build_filename (homedir, ".gnome/fonts", NULL);
...
gf_fontmap_load_dir (GFFontDB *db, const guchar *dirname, GFFontMapType type)
{
	...
	if (dir) {
		...
		while (files) {
			struct stat s;
			gchar *filename;
			filename = g_build_filename (dirname, (gchar *) files->data, NULL);
		...
		if (root && !strcmp (root->name, "fontmap")) {
			...
			if (version) {
				if (!strcmp (version, "2.0")) {
					...
					if (gpversion) {
						map->gpversion = (gint) floor (1000 * atof (gpversion) + 0.5);
						xmlFree (gpversion);
					}
		...
		if (!strcmp (child->name, "font")) {
			...
			if (format) {
				if (!strcmp (format, "type1") || !strcmp (format, "type1alias")) {
					...
					gf_fm_load_font_2_0_type1 (map, child, !strcmp (format, "type1alias"));
...
static void
gf_fm_load_font_2_0_type1 (GFFontMap *map, xmlNodePtr node, gboolean isalias)
{
	GFFontEntry *e;
