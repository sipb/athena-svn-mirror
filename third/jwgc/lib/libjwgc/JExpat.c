...
/* $Id: JExpat.c,v 1.1.1.1 2006-03-10 15:32:26 ghudson Exp $ */

#include "libjwgc.h"
#include "libxode.h"

void 
expat_startElement(void *userdata, const char *name, const char **atts)
{
	/* get the xode pointed to by the userdata */
	xode *x = userdata;
	xode current = *x;
	...
	if (current == NULL) {
		...
		xode_put_expat_attribs(current, atts);
		*x = current;
	}
	else {
		*x = xode_insert_tag(current, name);
		xode_put_expat_attribs(*x, atts);
	}
}

void 
expat_endElement(void *userdata, const char *name)
{
	xode *x = userdata;
	xode current = *x;
...
void 
expat_charData(void *userdata, const char *s, int len)
{
	xode *x = userdata;
	xode current = *x;
...
xode 
xode_str(char *str, int len)
{
	XML_Parser p;
	xode *x, node;		/* pointer to an xode */
	...
	*x = NULL;		/* pointer to NULL */
	...
	if (!XML_Parse(p, str, len, 1)) {
		/*
		 * jdebug(ZONE,"xode_str_error: %s",(char
	...
	free(x);
	XML_ParserFree(p);
	return node;		/* return the xode x points to */
}
	...
	*x = NULL;		/* pointer to NULL */
	...
	do {
		...
		if (!XML_Parse(p, buf, len, done)) {
			/*
			 * jdebug(ZONE,"xode_file_parseerror: %s",(char
			 * *)XML_ErrorString(XML_GetErrorCode(p)));
			 */
			xode_free(*x);
			*x = NULL;
...
xode2file(char *file, xode node)
{
	...
	ftmp = spools(xode_get_pool(node), file, ".t.m.p", xode_get_pool(node));
...
void 
xode_put_expat_attribs(xode owner, const char **atts)
{
	int i = 0;
	if (atts == NULL)
		return;
