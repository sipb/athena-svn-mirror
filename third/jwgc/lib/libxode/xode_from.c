...
xode xode_from_str(char *str, int len)
{
    ...
    *x = NULL; /* pointer to NULL */
    ...
    {
        dprintf(dXML, "xmlnode_str_error: %s",(char *)XML_ErrorString(XML_GetErrorCode(p)));
        xode_free(*x);
        *x = NULL;
    }
    node = *x;
...
xode xode_from_file(char *file)
{
    ...
    if(fd < 0) {
        dprintf(dXML, "unable to open file [%d]: %s\n", errno, _file);
        return NULL;
    }
    ...
    *x = NULL; /* pointer to NULL */
    ...
    do{
        ...
        {
            dprintf(dXML, "xmlnode_file_parseerror: %s",(char *)XML_ErrorString(XML_GetErrorCode(p)));
            xode_free(*x);
            *x = NULL;
            done = 1;
        }
