...
#define E_ICON_BAR_SMALL_ICON_WIDTH	16
#define E_ICON_BAR_SMALL_ICON_HEIGHT	16
...
static void e_icon_bar_dispose (GObject *object);
...
#define PARENT_TYPE E_CANVAS_TYPE
static ECanvasClass *parent_class;

E_MAKE_TYPE (e_icon_bar,
	     "EIconBar",
	     EIconBar,
	     e_icon_bar_class_init,
	     e_icon_bar_init,
	     PARENT_TYPE)
...
e_icon_bar_class_init (EIconBarClass *class)
{
	...
	e_icon_bar_signals[ITEM_SELECTED] =
		g_signal_new ("item_selected",
			      ...
			      G_STRUCT_OFFSET (EIconBarClass, selected_item),
			      NULL, NULL,
			      e_marshal_NONE__BOXED_INT,
			      G_TYPE_NONE, 2, GDK_TYPE_EVENT,
			      G_TYPE_INT);
	e_icon_bar_signals[ITEM_DRAGGED] =
		g_signal_new ("item_dragged",
			      G_OBJECT_CLASS_TYPE (object_class),
			      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
			      G_STRUCT_OFFSET (EIconBarClass, dragged_item),
			      NULL, NULL,
			      e_marshal_NONE__BOXED_INT,
			      G_TYPE_NONE, 2, GDK_TYPE_EVENT,
			      G_TYPE_INT);
	...
	/* Method override */
	object_class->dispose		 = e_icon_bar_dispose;
...
e_icon_bar_new (void)
{
	...
	icon_bar = GTK_WIDGET (g_object_new (E_ICON_BAR_TYPE, NULL));
...
static void
e_icon_bar_dispose (GObject *object)
{
	EIconBar *icon_bar;
	int item_num;
	EIconBarItem *item;
...
{
	EIconBar *icon_bar;
	gint canvas_width, canvas_height, height, val;
	...
	val = GTK_LAYOUT (widget)->vadjustment->value;

	gnome_canvas_set_scroll_region (GNOME_CANVAS (widget),
					0, 0, canvas_width,
					MAX (height, canvas_height - 1));

	GTK_LAYOUT (widget)->vadjustment->value = val;
	...
	if (icon_bar->vadjustment_value_changed_id == 0)
		icon_bar->vadjustment_value_changed_id = g_signal_connect (GTK_LAYOUT (widget)->vadjustment,
									   "value_changed",
									   G_CALLBACK (e_icon_bar_vadjustment_value_changed),
									   icon_bar);
...
e_icon_bar_style_set (GtkWidget *widget, GtkStyle *previous_style) {
	...
	for (item_num = 0; item_num < icon_bar->items->len; item_num++) {
		...
		flattened = flatten_alpha (item->pixbuf,
					   rgb_from_gdk_color (&style->bg [GTK_STATE_ACTIVE]));
		     ...
		     gint	     position)
{
	...
	g_signal_connect (item.text, "event",
			  G_CALLBACK (e_icon_bar_on_item_event),
			  icon_bar);
	...
	g_signal_connect (item.image, "event",
			  G_CALLBACK (e_icon_bar_on_item_event),
			  icon_bar);
				 ...
				 GdkPixbuf	  *image)
{
	EIconBarItem *item;
	GdkPixbuf *flattened;
	GtkStyle *style = GTK_WIDGET (icon_bar)->style;
			  ...
			  EIconBar *icon_bar)
{
	...
	case GDK_BUTTON_PRESS:
		...
		if (icon_bar->editing_item_num == -1
			...
			g_signal_stop_emission_by_name (item,
							"event");
			return TRUE;
		}
		break;
			 ...
			 GdkEvent *event)
{
	...
	if (button == 1 && item_num != -1) {
		...
		/* We set owner_events to FALSE so we get all pointer events.
		 */
		gdk_pointer_grab (GTK_LAYOUT (icon_bar)->bin_window, FALSE,
				  GDK_BUTTON_RELEASE_MASK |
				  GDK_BUTTON1_MOTION_MASK,
				  NULL, NULL, event->button.time);
	} else if (button == 3) {
		g_signal_emit (icon_bar,
			       e_icon_bar_signals[ITEM_SELECTED], 0,
			       event, item_num);
	}
}
			...
			GdkEvent *event)
{
	...
	if (event && event->motion.state & GDK_BUTTON1_MASK
		...
		if (icon_bar->enable_drags
		    && (abs (event->motion.x - icon_bar->pressed_x) > E_ICON_BAR_DRAG_START_OFFSET
			|| abs (event->motion.y - icon_bar->pressed_y) > E_ICON_BAR_DRAG_START_OFFSET)) {
			icon_bar->dragged_item_num = icon_bar->pressed_item_num;

			/* We ungrab the pointer and emit the item-dragged
			   signal. Note that a drag must be started here or
			   the icon bar may act strange as it no longer has
			   a pointer grab. */
			gdk_pointer_ungrab (event->motion.time);

			g_signal_emit (icon_bar,
				       e_icon_bar_signals[ITEM_DRAGGED], 0,
			       ...
			       gint item_num)
{
	...
	item = &g_array_index (icon_bar->items,
			       EIconBarItem, item_num);
	e_canvas_item_grab_focus (item->text, TRUE);
}
...
void
e_icon_bar_set_enable_drags	(EIconBar	 *icon_bar,
				 gboolean	  enable_drags)
{
	g_return_if_fail (E_IS_ICON_BAR (icon_bar));

	icon_bar->enable_drags = enable_drags;
}
