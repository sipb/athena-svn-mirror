...
enum {
	...
	PROP_FRAME_PLACEMENT,
	PROP_SHADOW_TYPE,
	PROP_SCROLLBAR_SPACING
};
...
static void e_scroll_frame_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec);
static void e_scroll_frame_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);
static void e_scroll_frame_dispose (GObject *object);
static void e_scroll_frame_finalize (GObject *object);
 ...
 **/
E_MAKE_TYPE (e_scroll_frame,
	     "EScrollFrame",
	     EScrollFrame,
	     e_scroll_frame_class_init,
	     e_scroll_frame_init,
	     PARENT_TYPE)
...
e_scroll_frame_class_init (EScrollFrameClass *class)
{
	...
	object_class->set_property = e_scroll_frame_set_property;
	object_class->get_property = e_scroll_frame_get_property;
	object_class->dispose = e_scroll_frame_dispose;
	...
	g_object_class_install_property (object_class, PROP_HSCROLLBAR_POLICY,
					 g_param_spec_enum ("hscrollbar_policy",
							    _( "hscrollbar policy" ),
							    _( "hscrollbar policy" ),
							    GTK_TYPE_POLICY_TYPE, GTK_POLICY_AUTOMATIC,
							    G_PARAM_READWRITE));

	g_object_class_install_property (object_class, PROP_VSCROLLBAR_POLICY,
	...
	g_object_class_install_property (object_class, PROP_FRAME_PLACEMENT,
					 g_param_spec_enum ("frame_placement",
							    _( "Frame placement" ),
							    _( "Frame placement" ),
							    GTK_TYPE_CORNER_TYPE, GTK_CORNER_TOP_LEFT,
							    G_PARAM_READWRITE));

	g_object_class_install_property (object_class, PROP_SHADOW_TYPE,
					 g_param_spec_enum ("shadow_type",
							    _( "Shadow type" ),
							    _( "Shadow type" ),
							    GTK_TYPE_SHADOW_TYPE, GTK_CORNER_TOP_LEFT,
							    G_PARAM_READWRITE));

	g_object_class_install_property (object_class, PROP_SHADOW_TYPE,
					 g_param_spec_uint ("scrollbar_spacing",
							    _( "Scrollbar spacing" ),
							    _( "Scrollbar spacing" ),
							    0, G_MAXUINT, 0,
							    G_PARAM_READWRITE));
...
static void
e_scroll_frame_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
	EScrollFrame *sf;
	ScrollFramePrivate *priv;
	...
	case PROP_HADJUSTMENT:
		e_scroll_frame_set_hadjustment (sf, g_value_get_object (value));
		break;

	case PROP_VADJUSTMENT:
		e_scroll_frame_set_vadjustment (sf, g_value_get_object (value));
		break;

	case PROP_HSCROLLBAR_POLICY:
		e_scroll_frame_set_policy (sf, g_value_get_enum (value), priv->vsb_policy);
		break;

	case PROP_VSCROLLBAR_POLICY:
		e_scroll_frame_set_policy (sf, priv->hsb_policy, g_value_get_enum (value));
		break;

	case PROP_FRAME_PLACEMENT:
		e_scroll_frame_set_placement (sf, g_value_get_enum (value));
		break;

	case PROP_SHADOW_TYPE:
		e_scroll_frame_set_shadow_type (sf, g_value_get_enum (value));
		break;

	case PROP_SCROLLBAR_SPACING:
		e_scroll_frame_set_scrollbar_spacing (sf, g_value_get_uint (value));
		break;
...
static void
e_scroll_frame_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
	EScrollFrame *sf;
	ScrollFramePrivate *priv;
	...
	switch (prop_id) {
	case PROP_HADJUSTMENT:
		g_value_set_object (value, e_scroll_frame_get_hadjustment (sf));
		break;

	case PROP_VADJUSTMENT:
		g_value_set_object (value, e_scroll_frame_get_vadjustment (sf));
		break;

	case PROP_HSCROLLBAR_POLICY:
		g_value_set_enum (value, priv->hsb_policy);
		break;

	case PROP_VSCROLLBAR_POLICY:
		g_value_set_enum (value, priv->vsb_policy);
		break;

	case PROP_FRAME_PLACEMENT:
		g_value_set_enum (value, priv->frame_placement);
		break;
	...
	case PROP_SCROLLBAR_SPACING:
		g_value_set_uint (value, priv->sb_spacing);
		break;
...
static void
e_scroll_frame_dispose (GObject *object)
{
	EScrollFrame *sf;
	ScrollFramePrivate *priv;
...
static void
e_scroll_frame_finalize (GObject *object)
{
	EScrollFrame *sf;
	ScrollFramePrivate *priv;
...
e_scroll_frame_size_request (GtkWidget *widget, GtkRequisition *requisition)
{
	...
	if (bin->child && GTK_WIDGET_VISIBLE (bin->child)) {
		...
		if (priv->hsb_policy == GTK_POLICY_NEVER)
			...
			aux_info = g_object_get_qdata (G_OBJECT (bin->child),
						       quark_aux_info);
...
e_scroll_frame_set_hadjustment (EScrollFrame *sf, GtkAdjustment *adj)
{
	...
	if (adj)
		...
		adj = GTK_ADJUSTMENT (g_object_new (GTK_TYPE_ADJUSTMENT, NULL));

	if (!priv->hsb) {
		gtk_widget_push_composite_child ();
		priv->hsb = e_hscrollbar_new (adj);
		...
		g_signal_handlers_disconnect_matched (old_adj,
