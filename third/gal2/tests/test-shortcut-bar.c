...
main (int argc, char *argv[])
{
...
#if 0
	e_shortcut_bar_set_enable_drags (E_SHORTCUT_BAR (shortcut_bar),
					 FALSE);
#endif
	...
	for (i = 0; i < NUM_SHORTCUT_TYPES; i++) {
		...
		if (pathname) {
			icon_pixbufs[i] = gdk_pixbuf_new_from_file (pathname, NULL);
			g_print ("type %d -- %s %s\n",
				 i, pathname, icon_pixbufs[i] == NULL ? "(not found)" : "");
		} else {
			g_warning ("Failed to locate '%s'", icon_filenames [i]);
			icon_pixbufs[i] = NULL;
		}
	}
			 ...
			 gchar		*name)
{
	...
	e_shortcut_model_add_item (shortcut_bar->model,
				   group_num, item_num, url, name, NULL);
}
