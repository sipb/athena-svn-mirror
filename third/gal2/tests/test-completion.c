...
#include <gnome.h>
#include "e-text/e-completion.h"
#include "e-text/e-completion-callbacks.h"
#include "e-text/e-entry.h"
...
dict_check (gpointer ptr)
{
	...
	while (limit > 0
		...
		e_completion_match_set_text (match, word_array[i], word_array[i]);
...
static void
request_dict_search (ECompletionCallbacks *cc, const gchar *txt, gint pos, gint limit, gpointer user_data)
{
	ECompletion *complete = E_COMPLETION (cc);
	gint len = strlen (txt);
...
static void
end_dict_search (ECompletionCallbacks *cc, gpointer user_data)
{
	if (dict_tag != 0) {
		gtk_timeout_remove (dict_tag);
		dict_tag = 0;
