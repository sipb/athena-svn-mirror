...
#include <sys/types.h>
    ...
    AC_DEFINE(HAVE_LONGLONG, 1, [defined if we have long long])
...
#include <sys/types.h>
    ...
    AC_DEFINE(HAVE_LONGDOUBLE, 1, [defined if we have long double])
...
[dnl
AC_DEFINE(PTH_NO_PATCH_WARNING, 1, 
          [defined if we don't show 2.4 kernel patch warning])
msg="disabled"
],[
msg="enabled"
])dnl
...
msg="enabled"
AC_DEFINE(PTH_DEBUG, 1, [defined iw we want debugging in pth])
],[
if test ".$ac_cv_prog_gcc" = ".yes"; then
case "$CFLAGS" in
    *-pipe* ) ;;
...
AC_DEFINE_UNQUOTED($1, $val, [defined if we need a separate stack])
])
...
AC_MSG_RESULT([$nsig])
AC_DEFINE_UNQUOTED($1, $nsig, [number of signals])
])
