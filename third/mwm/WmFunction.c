...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: WmFunction.c,v $ $Revision: 1.1.1.1 $ $Date: 1997-03-25 09:12:18 $"
#endif
#endif
/*
...
#endif /* _NO_PROTO */
{
        ...
        if (pcdFocus)
	{
		 ...
		 * and accelerators.
		 *
		 * The following section was modified for CR 5652
		 * to correct input focus problem with windows that
		 * don't want the focus.  A more complete solution should
		...
		if (wmGD.enforceKeyFocus ||
		  ...
		  /* According to ICCCM the client area should not get
		   * focus, so don't change it. This needs to be fixed
		   * to be the client frame to get input focus, but
		   * mwm can't handle this right now.
		   */
		  if (!(flags & CLIENT_AREA_FOCUS) && 
		      !pcdFocus->inputFocusModel) {
		    /* Do Nothing for now
		     * This means there is no way to get 
		     * at the system menu using the keyboard
		     * although it is possible with the mouse.
		     */
		  }
		  else
	            XSetInputFocus (DISPLAY, focusWindow, RevertToParent,
