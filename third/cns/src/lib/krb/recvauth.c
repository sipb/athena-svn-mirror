...
char *version;			 /* version string (filled in) */
{
    ...
    char *cp;
    int rem;
    KRB_INT32 tkt_len, priv_len;
    unsigned KRB_INT32 cksum;
	...
	tkt_len = (KRB_INT32) atoi((char *) tmp_buf);
	...
	/* sanity check */
	ticket->length = ntohl((unsigned KRB_INT32)tkt_len);
	...
	priv_len = krb_mk_priv((unsigned char *)&cksum,
			       tmp_buf,
			       (unsigned KRB_INT32) sizeof(cksum),
			       schedule,
			       &kdata->session,
			       laddr,
			       faddr);
...
mutual_fail:
	    priv_len = -1;
	    tkt_len = htonl((unsigned KRB_INT32) priv_len);
