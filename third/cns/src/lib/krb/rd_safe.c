...
krb_rd_safe(in,in_length,key,sender,receiver,m_data)
    ...
    unsigned KRB_INT32 in_length; /* length of "in" msg */
...
{
    unsigned KRB_INT32 calc_cksum[4];
    unsigned KRB_INT32 big_cksum[4];
    int swap_bytes;

    u_char     *p,*q;
    unsigned KRB_INT32  src_addr; /* Can't send structs since no
				   * guarantees on size */
    unsigned KRB_INT32 t_local;	/* Local time in our machine */
    KRB_INT32 delta_t;		/* Difference between timestamps */
