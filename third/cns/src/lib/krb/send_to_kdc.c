...
/* End of kludge (FIXME) for FTP Software WinSock stack.  */
	...
	/* if h_addr is just an alias, don't fill it in */
	/* but if h_addr_list is empty, try to at least use h_addr */
	if (!host->h_addr_list || (&host->h_addr_list[0] != &host->h_addr)) {
	...
	/* save all of the addresses -- even though we don't send to them,
	   we may get a reply from any of them */
	if (host->h_addr_list) {
	  int n_addrs, i;
	  char **addr_list;
	  /* count the entries */
	  for (n_addrs = 0; host->h_addr_list[n_addrs]; n_addrs++);
	  /* include the NULL entry */
	  n_addrs++;
	  /* copy each address into the new list */
	  addr_list = (char **) malloc(n_addrs * sizeof(char*));
	  for (i = 0; i < n_addrs-1; i++) {
	    cp = malloc((unsigned)host->h_length);
	    if(!cp) {
	      retval = SKDC_CANT;
	      /* but free the ones we've already done */
	      for (i--; i >= 0; i--) free(addr_list[i]);
	      goto rtn;
	    }
	    addr_list[i] = cp;
	    _fmemcpy(addr_list[i], host->h_addr_list[i], host->h_length);
	  }
	  addr_list[i] = 0;
	  /* stuff the copy into the slot */
    ...
    struct hostent *addrs;
{
    ...
    for (hp = addrs; hp->h_name != (char *)NULL; hp++) {
        char **thisaddr;
	int thisindex = 0;
	for (thisaddr = hp->h_addr_list; thisaddr[thisindex]; thisindex++) {
	  if (!memcmp(thisaddr[thisindex], (char *)&from.sin_addr.s_addr,
		      hp->h_length)) {
            DEB (("Received it\n"));
            return 1;
	  }
