...
krb_mk_auth(options, ticket, service, inst, realm, checksum, version, buf)
     ...
     char *realm;		/* realm */
     unsigned KRB_INT32 checksum; /* checksum to include in request */
     ...
     KTEXT buf;			/* Output buffer to fill  */
{
    ...
    tkt_len = htonl((unsigned KRB_INT32) ticket->length);
...
krb_check_auth (buf, checksum, msg_data, session, schedule, laddr, faddr)
     ...
     unsigned KRB_INT32 checksum; /* checksum we included in request */
