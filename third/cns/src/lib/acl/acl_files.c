...
int acl_exact_match(acl, principal)
char *acl;
char *principal;
{
    int idx;
...
/* Returns nonzero if it can be determined that acl contains principal */
/* Recognizes wildcards in acl using "*" in place of one, two, or all
   three components.  No partial wildcarding on any component.  */
int acl_check(acl, principal)
char *acl;
char *principal;
{
    char buf[MAX_PRINCIPAL_SIZE];
    ...
    char name[MAX_PRINCIPAL_SIZE];
    char inst[MAX_PRINCIPAL_SIZE]; /* include "." */
    char realm[MAX_PRINCIPAL_SIZE]; /* include "@" */
    ...
    s = strchr(canon, REALM_SEP);
    strcpy (realm, s);
    *s = 0;
    s = strchr (canon, INST_SEP);
    strcpy (inst, s);
    *s = 0;
    strcpy (name, canon);
