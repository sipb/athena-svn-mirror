...
*/
int vals_to_stream(dt_in, dt_out)
Kadm_vals *dt_in;
u_char **dt_out;
{
  ...
  for (vsloop=31; vsloop>=0; vsloop--)
    if (IS_FIELD(vsloop,dt_in->fields)) {
	  ...
	  break;
      case KADM_EXPDATE:
	  stsize+=vts_long((unsigned KRB_INT32) dt_in->exp_date, dt_out, stsize);
	  break;
      case KADM_ATTR:
...
int build_field_header(cont, st)
...
int vts_long(dat, st, loc)
unsigned KRB_INT32 dat;		/* the attributes field */
u_char **st;			/* a base pointer to the stream */
int loc;			/* offset into the stream for current data */
{
  KRB_INT32 temp;			/* to hold the net order short */
  extern char *realloc();

  temp = htonl(dat);		/* convert to network order */
  *st = (u_char *) realloc ((char *)*st, (unsigned)(loc + sizeof(KRB_INT32)));
  memcpy((char *)(*st + loc), (char *) &temp, sizeof(KRB_INT32));
  return sizeof(KRB_INT32);
}

    
int vts_char(dat, st, loc)
...
*/
int stream_to_vals(dt_in, dt_out, maxlen)
u_char *dt_in;
Kadm_vals *dt_out;
int maxlen;				/* max length to use */
{
  ...
  for (vsloop=31; vsloop>=0; vsloop--)
    if (IS_FIELD(vsloop,dt_out->fields))
      ...
      case KADM_EXPDATE:
	  {
	    unsigned KRB_INT32 exp_date;

	    if ((status = stv_long(dt_in, &exp_date, stsize,
				   maxlen)) < 0)
	        return(-1);
...
int check_field_header(st, cont, maxlen)
u_char *st;			/* stream */
u_char *cont;			/* container for fields data */
int maxlen;
{
...
int stv_string(st, dat, loc, stlen, maxlen)
...
int stv_short(st, dat, loc, maxlen)
u_char *st;			/* a base pointer to the stream */
u_short *dat;			/* the attributes field */
int loc;			/* offset into the stream for current data */
int maxlen;
...
int stv_long(st, dat, loc, maxlen)
u_char *st;			/* a base pointer to the stream */
unsigned KRB_INT32 *dat;	/* the attributes field */
int loc;			/* offset into the stream for current data */
int maxlen;			/* maximum length of st */
{
  ...
  if (loc + sizeof(KRB_INT32) > maxlen)
      return(-1);
  memcpy((char *) &temp, (char *) st + loc, sizeof(KRB_INT32));
  *dat = ntohl(temp);		/* convert to network order */
  return sizeof(KRB_INT32);
}
    
int stv_char(st, dat, loc, maxlen)
u_char *st;			/* a base pointer to the stream */
u_char *dat;			/* the attributes field */
int loc;			/* offset into the stream for current data */
int maxlen;
