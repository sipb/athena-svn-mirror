    ...
    char  **argv;
{
	...
	display_tktfile(tkt_file, tgt_test, long_form, show_kvnos);
    exit(0);
}
...
display_tktfile(file, tgt_test, long_form, show_kvnos)
     char *file;
     int tgt_test, long_form, show_kvnos;
{
    char    pname[ANAME_SZ];
    char    pinst[INST_SZ];
	...
	if (!tgt_test)
		fprintf(stderr, "%s: %s\n", whoami, krb_get_err_text (k_errno));
	exit(1);
    }
    /* Close ticket file */
    (void) tf_close();
...
{
    fprintf(stderr,
        "Usage: %s [ -s | -t ] [ -file filename ] [ -srvtab ] [ -version ]\n", whoami);
    exit(1);
}
