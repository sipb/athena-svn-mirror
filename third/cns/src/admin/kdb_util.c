...
clear_secrets ()
{
  memset((char *)master_key, 0, sizeof (des_cblock));
  memset((char *)master_key_schedule, 0, sizeof (Key_schedule));
  memset((char *)new_master_key, 0, sizeof (des_cblock));
  memset((char *)new_master_key_schedule, 0, sizeof (Key_schedule));
}
     ...
     Principal *p;
{
  ...
  if ((strncmp (p->name, KERB_M_NAME, ANAME_SZ) == 0) &&
    ...
    memcpy((char *) key, (char *)new_master_key, sizeof (des_cblock));
    (p->key_version)++;
  } else {
    memcpy((char *)key, (char *)&(p->key_low), sizeof(KRB_INT32));
    memcpy((char *) (((KRB_INT32 *) key) + 1), (char *)&(p->key_high), sizeof(KRB_INT32));
  ...
  memcpy((char *)&(p->key_low), (char *)key, sizeof(KRB_INT32));
  memcpy((char *)&(p->key_high), (char *)(((KRB_INT32 *) key) + 1), sizeof(KRB_INT32));
  memset((char *)key, 0, sizeof (key));  /* a little paranoia ... */
     ...
     FILE *out;
{
  ...
  if (kdb_get_master_key (TRUE, master_key, master_key_schedule, 0) != 0) {
    fprintf (stderr, "Couldn't get master key.\n");
    clear_secrets ();
    exit (-1);
  }
  ...
  if (kdb_get_master_key (TRUE, new_master_key, new_master_key_schedule, 1) != 0) {
    fprintf (stderr, "Couldn't get new master key.\n");
    clear_secrets ();
    exit (-1);
  }
     ...
     FILE *out;
{
  ...
  if (kdb_get_master_key (TRUE, master_key, master_key_schedule, 0) != 0L) {
    fprintf (stderr, "Couldn't get master key.\n");
    clear_secrets();
    exit (-1);
  }
   ...
   * be the same! 
   */
  memcpy((char *)&key_from_db, (char *)&principal_data[0].key_low, sizeof(KRB_INT32));
  memcpy((char *)(((KRB_INT32 *) &key_from_db) + 1),
	 (char *)&principal_data[0].key_high, sizeof(KRB_INT32));
#ifndef NOENCRYPTION
