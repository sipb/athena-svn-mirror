  ...
  char *argv[];
{
	...
	for (i = 1; i < argc; i++) {
		...
		if (++i >= argc)
		    ...
		    dbname = (char *) malloc(strlen(argv[i])+1);
		    strcpy(dbname, argv[i]);
		    if(kerb_db_set_name(dbname) != 0) {
			fprintf(stderr, "Could not set alternate database name (%s)\n", argv[i]);
			usage();
		    }
		    argv[i][0] = '-';
		}
    ...
    if (kdb_get_master_key_from (prompt, master_key, master_key_schedule, 0, kfile) != 0) {
	...
	for (i = 0; i < n; i++) {
	    ...
	    memcpy(session_key, &princs[i].key_low, sizeof(KRB_INT32));
	    memcpy(session_key + sizeof(KRB_INT32), &princs[i].key_high, 
		   sizeof(KRB_INT32));
...
{
    fprintf(stderr, 
	    "Usage: %s [-n] [-r realm] [-k master key file] [-d database] instance [instance ...]\n", progname);
    exit(1);
}
