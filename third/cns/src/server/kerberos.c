...
#include <signal.h>
#ifndef __linux__
/* new gcc releases don't have it... */
#include <sgtty.h>
#endif
#include <sys/ioctl.h>
#include <sys/time.h>
...
#ifdef hpux
/* just to get setsid, since hpux has no TIOCNOTTY */
#include <unistd.h>
#endif

#ifndef NO_SYSIO
#ifdef __svr4__
/* get FIONREAD from sys/filio.h, so what if it is a compatibility feature */
#include <sys/filio.h>
#endif
#endif
...
struct sockaddr_in tmp_sin = {AF_INET};
int     f;
...
static int Pflag;		/* require preauthentication */
...
static void usage()
{
    fprintf(stderr, "Usage: %s [-s] [-m] [-n] [-p pause_seconds]%s%s\n", progname, 
    	    " [-a max_age] [-l log_file] [-r realm] [-P] [-i] [-u port] [-k master key file]"
	    ," [database_pathname]"
	    );
    exit(1);
}
    ...
    char  **argv;
{
    ...
    while ((c = getopt(argc, argv, "snmPip:a:l:r:u:k:")) != EOF) {
	switch(c) {
	case 's':
	    /*
	...
	case 'm':
	    ...
	    break;
 	case 'P':
 	    Pflag++;		/* require preauthentication */
 	    break;
	case 'i':
	    /* ignore source IP address of packets */
	...
	printf("\tMaster key will be retrieved from %s\n", kfile);
    if (Pflag)
	printf("\tPreauthentication of Initial Ticket Requests required\n");
    if (krb_ignore_ip_address)
	printf("\tIgnoring packet IP addresses\n");
    if (udp_port != -1)
	printf("\tListening on alternate port %d\n", udp_port);
    ...
    if (udp_port != -1)
	tmp_sin.sin_port = htons(udp_port);
    else if (sp = getservbyname("kerberos", "udp"))
        tmp_sin.sin_port = sp->s_port;
    else
        tmp_sin.sin_port = htons(KERBEROS_PORT); /* kerberos/udp */
    ...
    if (bind(f, (struct sockaddr *) &tmp_sin, S_AD_SZ) < 0) {
    ...
    /* setup master key */
    if (kdb_get_master_key_from (mflag, master_key, master_key_schedule, 0, kfile) != 0) {
    ...
    KTEXT   pkt;
{
	...
	{
	    unsigned KRB_INT32  time_ws;	/* Workstation time */
	    unsigned KRB_INT32  req_life;	/* Requested liftime */
	    char   *service;	/* Service name */
	    char   *instance;	/* Service instance */
	    int     kerno;	/* Kerberos error number */
	    unsigned char *preauth_p;  /* Pre-authentication data */
	    int     preauth_len; /* length of data */
	    ...
	    ptr = (unsigned char *) pkt_time_ws(pkt) + 4;

	    req_life = (unsigned KRB_INT32) (*ptr++);

	    service = (char *)ptr;
	    instance = (char *)ptr + strlen(service) + 1;
	    if (Pflag) {
		preauth_p = (unsigned char *) instance + strlen(instance) + 1;
		preauth_len = (pkt->dat+pkt->length) - preauth_p;
	    }
	    ...
	    memcpy(key, &s_name_data.key_low, sizeof(KRB_INT32));
	    memcpy(((KRB_INT32 *) key) + 1, &s_name_data.key_high, sizeof(KRB_INT32));
	    ...
	    memcpy(key, &a_name_data.key_low, sizeof(KRB_INT32));
	    memcpy(((KRB_INT32 *) key) + 1, &a_name_data.key_high, sizeof(KRB_INT32));

	    /* unseal the a_name key from the master key */
	    kdb_encrypt_key(key, key, master_key, 
			    master_key_schedule, DECRYPT);

	    /* handle preauthentication here */
	    if (Pflag) {
		int status;
		status = krb_rd_preauth(pkt,preauth_p,preauth_len,
					&a_name_data, key);
		if (status) {
		    lt = klog(L_INI_REQ,
			      "Initial ticket preauth failed (%d)[%s]",
			      preauth_len, preauth_p,
			      0);
		    kerb_err_reply(client, pkt, status, lt);
		    return;
		}
	    }

	    /* so now, "key" is really the device key */
	    /* SNK support here */
	    if (strlen(req_inst_ptr) >= 5 
		&& !strcmp(req_inst_ptr+strlen(req_inst_ptr)-5, "+SNK4")) {
	      /* generate 2 challenges
		 cook them into responses
		 make the key
		 record the challenges and version in the buffer
		 so then we can have create_ciph start after that. */
	      /* the two challenges are only 6 digits... so two halves
		 of the output of random_key are plenty */
	      unsigned long chall[2];
	      static char version[] = "cnssnk01";
	      char challenge[2][8];
	      C_Block resp[2], key1;
	      char respout[2][8];
	      Key_schedule devks, ks;
	      int i, j;
	      static KTEXT_ST ciphtmp_st;
	      KTEXT   ciphtmp = &ciphtmp_st;

	      des_key_sched(key, devks);

	      do {
		des_new_random_key(chall);
		chall[0] %= 1000000;
		chall[1] %= 1000000;
		...
		des_ecb_encrypt((des_cblock *) challenge[i],
				(des_cblock *) resp[i], devks, 1);
		for (j=0; j<4; j++) {
		  char n[2];
		  int k;
		  n[0] = resp[i][j] & 0xf;
		  ...
		  *(4*i+j+(char*)&key1) = (n[1]<<4) | n[0];
		}
	      }
	      des_fixup_key_parity(key1);
	      des_key_sched(key1, ks);
	      des_ecb_encrypt(&key1, &key, ks, 1);
	      des_fixup_key_parity(key);
	      /* at this point, key has been changed from the device key 
		 to the challenge key. */

	      ciphtmp->length = 0;
	      create_ciph(ciphtmp, session_key, s_name_data.name,
			  s_name_data.instance, local_realm, lifetime,
			  s_name_data.key_version, tk, kerb_time.tv_sec, key);
	      ciph->length = ciphtmp->length + 24;
	      memcpy(ciph->dat, version, 8);
	      memcpy(ciph->dat+8, challenge[0], 8);
	      memcpy(ciph->dat+16, challenge[1], 8);
	      memcpy(ciph->dat+24, ciphtmp->dat, ciphtmp->length);
	    } else {
    ...
    case AUTH_MSG_APPL_REQUEST:
	{
		...
		kerb_err_reply(client, pkt, KFAILURE, lt);
		return;
	    }
    ...
    char   *r;			/* Realm for desired key */
{
    ...
    klog(L_KRB_PINFO, "Getting key for %s", r);
    ...
    memcpy(key, &p->key_low, sizeof(KRB_INT32));
    memcpy(((KRB_INT32 *) key) + 1, &p->key_high, sizeof(KRB_INT32));
    kdb_encrypt_key(key, key, master_key,
		    master_key_schedule, DECRYPT);
    krb_set_key(key, 0);
    strcpy(lastrealm, r);
