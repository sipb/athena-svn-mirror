...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/gtx/textobject.c,v 1.1.1.1 2002-01-31 21:49:03 zacheiss Exp $");
...
{ /*gator_text_create*/
    ...
    if (objects_debug) {
      fprintf(stderr, "[%s:%s] Private data passed to text object at 0x%x:\n", mn, rn, text_onp);
    ...
    if (objects_debug)
      fprintf(stderr, "[%s:%s] Number of lines in window: %d: ", mn, rn, text_data->numLines);
...
{ /*gator_text_display*/
    ...
    if (objects_debug)
      fprintf(stderr, "[%s:%s] Displaying text object at 0x%x, object-specific data at 0x%x\n", mn, rn, onp, text_data);
    ...
    if (objects_debug)
      fprintf(stderr, "[%s:%s] Drawing %d populated lines, starting with entry %d (index %d) at 0x%x\n", mn, rn, currLinesUsed, currEnt, currIdx, curr_ent);
...
{ /*gator_text_Write*/
    ...
    /*
      ...
      */
    if (objects_debug) {
      fprintf(stderr, "[%s:%s] Writing %d chars to text object at 0x%x (highlight=%d, skip=%d: '", rn, numChars, onp, highlight, skip);
	...
	(cbHdr->currEnt >= text_data->firstEntShown)) {
      if (objects_debug)
	fprintf(stderr, "[%s:%s] Current entry is on screen.  Tracking this write\n", mn, rn);
      oldCurrEnt = cbHdr->currEnt;
      redisplay = 1;
    }
    else {
      if (objects_debug)
	fprintf(stderr, "[%s:%s] Current entry NOT on screen, not tracking write\n", mn, rn);
      redisplay = 0;
    }
	...
	*/
      ...
      if (objects_debug)
	fprintf(stderr, "[%s:%s] Preparing to redisplay.  Difference in shown lines=%d, difference in written lines=%d\n", mn, rn, shownDiff, writeDiff);
	...
	if (objects_debug)
	  fprintf(stderr, "[%s:%s] Empty lines appeared on screen, bumping bottom line shown by %d; new writeDiff is %d\n", mn, rn, bumpAmount, writeDiff);
      }
	...
	*/
      if (writeDiff > 0) {
	if (objects_debug)
	  fprintf(stderr, "[%s:%s] Still more lines need to be tracked.  Moving first & last shown down by %d\n", mn, rn, writeDiff);
    ...
    if (code) {
      fprintf(stderr, "[%s:%s] Can't write to text object's circular buffer, errror code is %d\n", mn, rn, code);
      return(code);
    }
...
{ /*gator_text_BlankLine*/
     ...
     */
    if (objects_debug)
      fprintf(stderr, "[%s:%s] Writing %d blank lines to text object at 0x%x\n", mn, rn, numBlanks, onp);

    text_data = (struct gator_textobj *)(onp->o_data);
    code = gator_textcb_BlankLine(text_data->cbHdr, numBlanks);
    if (code) {
      fprintf(stderr, "[%s:%s] Can't write %d blank lines to text object at 0x%x\n", mn, rn, numBlanks, onp);
      return(code);
    }
