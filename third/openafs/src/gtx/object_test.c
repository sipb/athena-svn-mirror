...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/gtx/object_test.c,v 1.1.1.2 2004-02-13 17:52:03 zacheiss Exp $");
...
{ /*test_objects*/
     ...
     */
    if (object_debug)
      fprintf(stderr, "[%s:%s] Setting up initialization params for window package %d\n", pn, rn, pkg);
    ...
    if (code) {
	fprintf(stderr, "[%s:%s] Can't initialize gator objects package for window system %d; error is: %d\n", pn, rn, pkg, code);
	return(code);
    }
    ...
    for (i=0; i<10; i++) {
      ...
      if (code)
	fprintf(stderr, "[%s:%s] Can't set gator light at 0x%x to %d (%s)\n", pn, rn, light_onp1, setting, (setting? "ON" : "OFF"));
      else
    ...
    if (code)
      fprintf(stderr, "[%s:%s] Can't write '%s' (%d chars) to text object at 0x%x; error code is %d\n", pn, rn, s, strlen(s), text_onp1, code);
    sleep(2);
    ...
    sprintf(s, "%s", "This is the very, very, very, very, very, very, very long fourth line");
    code = gator_text_Write(text_onp1, s, strlen(s), 0, 1);
    if (code)
      fprintf(stderr, "[%s:%s] Can't write '%s' (%d chars) to text object at 0x%x; error code is %d\n", pn, rn, s, strlen(s), text_onp1, code);
    sleep(2);
    ...
    for (i = 0; i<10; i++) {
      ...
      if (code)
      fprintf(stderr, "[%s:%s] Can't scroll up 1 line in text object at 0x%x\n", pn, rn, text_onp1);
      sleep(2);
    }

    for (i = 0; i < 10; i++) {
      code = gator_text_Scroll(text_onp1, 2, GATOR_TEXT_SCROLL_DOWN);
      fprintf(stderr, "[%s:%s] Can't scroll down 2 lines in text object at 0x%x\n", pn, rn, text_onp1);
      sleep(2);
    }
