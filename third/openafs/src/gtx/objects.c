...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/gtx/objects.c,v 1.1.1.1 2002-01-31 21:49:03 zacheiss Exp $");
...
{ /*gator_objects_init*/
    ...
    if (code) {
	fprintf(stderr, "[%s:%s] Can't initialize object dictionary: error code is %d\n", mn, rn, code);
	return(code);
    }
    ...
    /*
      ...
      */
    if (objects_debug)
      fprintf(stderr, "[%s:%s] Initializing gator object creation function array.\n", mn, rn);
...
{ /*gator_objects_create*/
    ...
    /*
      ...
      */
    if (objects_debug)
      fprintf(stderr, "[%s:%s] Calling the creation routine for gator object type %d\n", mn, rn, params->cr_type);
