...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/pinstall/install.c,v 1.1.1.1.2.1 2003-01-03 18:53:06 ghudson Exp $");
...
#ifdef AFS_DECOSF_ENV
    if (swapheader) return 0;
#else
    if (swapheader) swap_filehdr(&fheader, gethostsex());
#endif /* DEC OSF */
    /* Already stripped? */
    ...
    char **argv;
{
	...
	if (!setMode) mode = istat.st_mode;	/* Was 0755:> this is the default for our rcs to work */
#else /* AFS_AIX_ENV */
