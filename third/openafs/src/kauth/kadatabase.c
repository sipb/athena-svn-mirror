...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/kauth/kadatabase.c,v 1.1.1.1 2002-01-31 21:31:53 zacheiss Exp $");
...
afs_int32 NameHash (aname, ainstance)
  register char *aname;
...
afs_int32 kawrite (tt, pos, buff, len)
...
afs_int32 karead (tt, pos, buff, len)
  struct ubik_trans *tt;
  afs_int32		     pos;
  char		    *buff;
  afs_int32		     len;
  ...
  int initFlags;			/* same as init_kaprocs (see which) */
{
    ...
    maxCachedKeys = 10;
    keyCache = (struct cachedKey *)malloc (maxCachedKeys * sizeof(struct cachedKey));
    keyCacheVersion = 0;
    if (initFlags & 4) {
	maxKeyLifetime = 90;
    } else {
...
{   register afs_int32 code;
    ...
    if (!ubik_CacheUpdate (at)) return 0;
    
    ObtainWriteLock (&cheader_lock);
    if ((code = karead (at, 0, (char *) &iversion, sizeof(iversion))) ||
	(code = karead (at, sizeof(cheader)-sizeof(afs_int32),
			(char *) &tversion, sizeof(afs_int32)))) {
	if (code == UEOF) printf ("No data base\n");
    ...
    /* if here, we have no version number or the wrong version number in the
       file */
    if ((code == UEOF) || ((iversion == 0) && (tversion == 0))) code = KAEMPTY;
    else code = KAIO;

    if ((db_init == 0) || (code == KAIO)) return code;
    ...
    if (code) return KAIO;		/* return the error code */
...
afs_int32 AllocBlock (at, tentry)
...
{   register afs_int32 code;
	...
	if (code) return 0;		/* can't read block */
...
afs_int32 FindBlock (at, aname, ainstance, toP, tentry)
  struct ubik_trans *at;
  char *aname;
  char *ainstance;
  afs_int32 *toP;
  struct kaentry    *tentry;
{
    ...
    *toP = 0;
    ...
    for (to = ntohl(cheader.nameHash[i]); to != NULLO; to = ntohl(tentry->next)) {
	code = karead(at, to, (char *) tentry, sizeof(kaentry));
	if (code) return code;
	/* see if the name matches */
	if (!strcmp(aname, tentry->userID.name) &&
	    (ainstance == (char *) 0 || !strcmp(ainstance, tentry->userID.instance))) {
	    *toP = to;			/* found it */
	    return 0;
	}
    }
...
afs_int32 ThreadBlock (at, index, tentry)
  struct ubik_trans *at;
  afs_int32		     index;
  struct kaentry    *tentry;
{   int  code;
...
afs_int32 UnthreadBlock (at, aentry)
  struct ubik_trans *at;
  struct kaentry    *aentry;
{
	...
	if (!strcmp(aentry->userID.name, tentry.userID.name) &&
		...
		code = kawrite (at, lo, (char *)&tentry.next, sizeof(afs_int32));
		if (code) return KAIO;
	    }
...
afs_int32 NextBlock (at, index, tentry, remaining)
  struct ubik_trans *at;
  afs_int32		     index;
  struct kaentry    *tentry;
  afs_int32		    *remaining;
...
afs_int32 ka_NewKey (tt, tentryaddr, tentry, key)
  struct ubik_trans *tt;
  ...
  struct ktc_encryptionKey *key;
{
  ...
  int   foundcurrentkey = 0, addednewkey = 0, modified;
     ...
     * this link.
     */
    for (prevptr = 0, okeysaddr = ntohl(cheader.kvnoPtr);
	 okeysaddr;
	 prevptr = nextprevptr, okeysaddr = nextaddr) {
...
#ifdef AUTH_DBM_LOG
	     if (foundcurrentkey) {
	        ViceLog(0,("Warning: Entry %s.%s contains more than one valid key: fixing\n",
			tentry->userID.name, tentry->userID.instance));
	     }
#endif
	     foundcurrentkey = 1;
	  ...
	  if (!prevptr) {
	     ...
	     code = kawrite(tt, prevptr, (char *)&okeys.next, sizeof(afs_int32));
	  }
	  if (code) return code;
    ...
    if (!addednewkey) {
       ...
       if (!addednewkey) return KACREATEFAIL;
    ...
    if (oldtotalkeyentries != ntohl(tentry->misc.asServer.nOldKeys)) {
       ViceLog(0,("Warning: Entry %s.%s reports %d oldkeys, found %d: fixing\n",
	       tentry->userID.name, tentry->userID.instance,
    ...
    es_Report ("New kvno is %d, now are %d oldkeys\n", 
	       newkeyver, newtotalkeyentries);
    return 0;
}

afs_int32 ka_DelKey (tt, tentryaddr, tentry)
  struct ubik_trans *tt;
  afs_int32		     tentryaddr;
  struct kaentry    *tentry;
{
...
{   int i;
	...
	if (keyCache[i].used) {
		...
		int  j = info->kcUsed;
		int  k;
		char principal[sizeof(keyCache[0].name) + sizeof(keyCache[0].inst)];

		info->kcInfo[j].used = keyCache[i].superseded;
		info->kcInfo[j].kvno = keyCache[i].kvno;
		info->kcInfo[j].primary = (keyCache[i].superseded == NEVERDATE);
...
{   int   i;
    ...
    /* i == maxCachedKeys */
    keyCache = (struct cachedKey *)realloc
	(keyCache, (maxCachedKeys*=2) * sizeof(struct cachedKey));
    if (keyCache == 0) {
	...
	while (j<maxCachedKeys) keyCache[j++].used = 0;
    }
    goto encache;
}
...
afs_int32 ka_LookupKvno (tt, name, inst, kvno, key)
  struct ubik_trans *tt;
  char		    *name;
  char		    *inst;
  afs_int32		     kvno;
...
afs_int32 ka_LookupKey (tt, name, inst, kvno, key)
  struct ubik_trans *tt;
  char		    *name;
  char		    *inst;
...
{   int  nfound;
    ...
    if (keyCacheVersion == ntohl(cheader.specialKeysVersion)) return 0;
	...
	if (code) return KAIO;
	/* get name & instance */
	code = karead (tt, ntohl(okeys.entry), (char *)&tentry, sizeof(tentry));
	if (code) return KAIO;

	/* get all the old keys in this block */
	for (i=0; i<NOLDKEYS; i++)
	    if (okeys.keys[i].superseded) {
		code = ka_LookupKvno (tt, tentry.userID.name, tentry.userID.instance,
				      ntohl(okeys.keys[i].version), &k);
		if (code) return code;
	    }
    }
    if (++nfound > maxCachedKeys) return KADATABASEINCONSISTENT;
    return 0;
}

afs_int32 update_admin_count (tt, delta)
  struct ubik_trans *tt;
  int		     delta;
{   afs_int32 to;
    afs_int32 code;
    ...
    code = kawrite (tt, to, (char *)&cheader.admin_accounts, sizeof(afs_int32));
    if (code) return KAIO;
    return 0;
}

static int index_OK (index)
  afs_int32 index;
{
    if ((index < sizeof(cheader)) || (index >= ntohl(cheader.eofPtr)) ||
	((index - sizeof(cheader)) % sizeof(kaentry) != 0)) return 0;
    return 1;
}

#define LEGALCHARS ".ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_"

int name_instance_legal (name, instance)
  char *name;
  char *instance;
{   int code;
...
#if 1
    code = (strlen(name) < MAXKTCNAMELEN) && (strlen(instance) < MAXKTCNAMELEN);
#else
    map	= LEGALCHARS;			/* permitted chars, instance allows <period> */
    code = (strlen (name) > 0) && string_legal (instance, map) && string_legal (name, map+1);
#endif
    if (!code) dynamic_statistics.string_checks++;
    return code;
}

static int string_legal (str, map)
  char *str;
  char *map;
{   int slen;

    slen = strlen (str);
    if (slen >= MAXKTCNAMELEN) return 0; /* with trailing null must fit in data base */
