...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/kauth/user_nt.c,v 1.1.1.1 2002-01-31 21:32:39 zacheiss Exp $");
...
static long ka_AddHostProc(void *rockp, struct sockaddr_in *addrp, char *namep)
{
...
static char *ka_MapKerberosError(int code)
{
	switch (code) {
		case INTK_BADPW:
	...
	char **reasonP)
{
	...
	if (sp) krb_set_port(ntohs(sp->s_port));

	*reasonP = NULL;
	code = krb_get_in_tkt_ext(name, instance, upperRealm, "afs", "",
		lifetime, &key1, &key2, &ticket, &ticketLen, &sessionKey,
...
static void swap_u_int32
    (afs_uint32 *u)
{
   *u = *u >> 24 | (*u & 0x00ff0000) >> 8 | (*u & 0x0000ff00) << 8 | *u << 24;
}

static void swap_u_int16
    (afs_uint16 *u) \
{
    *u = *u >> 8 | *u << 8;
}
...
static int kaserver_map_error_code
    (int code,
     char *etext,
     char **reasonP)
{
...
static int krb_get_in_tkt_ext(user, instance, realm, service, sinstance, life,
	key1, key2, ticketpp, ticketLenp, outKeyp, kvnop, expp, reasonp)
    char *user;
    char *instance;
    char *realm;
    char *service;
    ...
    char **reasonp;
{
    ...
    *(pkt->dat+(pkt->length)++) = kerberos_life;
    ...
    case AUTH_MSG_ERR_REPLY:
        ...
        if (swap_bytes) swap_u_int32(&rep_err_code);
	/* kaservers return bogus error codes in different ways, so map it
	   from the error text if this is the case */
	return kaserver_map_error_code(rep_err_code, pkt_err_text(rpkt), reasonp);
    ...
    if (swap_bytes) swap_u_int32(&exp_date);
    ...
    if (swap_bytes) swap_u_int32(&kdc_time);
	...
	*ticketpp = malloc(tkt->length);
    ...
    else if (tkt->length > (unsigned long) *ticketLenp) return -1;
    *ticketLenp = tkt->length;
    memcpy(*ticketpp, tkt->dat, tkt->length);
    *kvnop = kvno;
    if (expp) *expp = life_to_time(kdc_time, (char) lifetime);
...
int krb_add_host
    (struct sockaddr_in *server_list_p)
{
    ...
    KTEXT rpkt;
{
    ...
    /* No more than max */
    if (numIters > CLIENT_KRB_RETRY) numIters = CLIENT_KRB_RETRY;
    /* At least one */
    ...
    struct sockaddr_in *_to;
{
    ...
    if ((code = recvfrom(f, (char *)(rpkt->dat), sizeof(rpkt->dat), 0,
...
{   char  password[8+1];		/* crypt's limit is 8 chars anyway */
    ...
    strncpy((char *) key, (char *)crypt(password, "p1") + 2, sizeof(struct ktc_encryptionKey));
