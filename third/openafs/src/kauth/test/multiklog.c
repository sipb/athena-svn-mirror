...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/kauth/test/multiklog.c,v 1.1.1.1 2002-01-31 21:50:10 zacheiss Exp $");
...
#define aREPCOUNT 9
    ...
    cmd_AddParm(ts, "-repeat", CMD_SINGLE, CMD_OPTIONAL, "number of times to repeat authentication");
  ...
  struct cmd_syndesc *as;
{
	...
	if (sp == life) {
bad_lifetime:
	    if (!Silent) fprintf (stderr, "%s: translating '%s' to lifetime\n",
			       rn, life);
	    return -1;
	}
	if (*sp == ':') {
     ...
     */
    for (i=0; i<reps; i++) {
      code = ka_UserAuthenticateGeneral (KA_USERAUTH_VERSION, pw->pw_name,
