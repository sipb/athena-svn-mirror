...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/kauth/test/test_getticket.c,v 1.1.1.1 2002-01-31 21:31:54 zacheiss Exp $");
  ...
  IN int line;
{
    ...
    if (line) printf ("Crashing at line %d\n", line);
    exit (line);
}
...
static void SetFields (conn, name, flags, expiration, lifetime)
  ...
  IN long lifetime;
{
    ...
    if (strcmp (name, KA_TGS_NAME) == 0) instance = realm;
    else instance = "";
    if (flags) {
	which = "flags";
	if (flags & KAFNOTGS)
	...
	flags |= KAFNORMAL;
	if (strcmp (name, admin_user) == 0) flags |= KAFADMIN;
    }
    else if (expiration) {
	which = "expiration";
...
static void CheckLife (tokenEnd, start, expectedLife, match)
  IN Date tokenEnd;
  IN Date start;
{
    Date expectedEnd;
    ...
    if (match == EXACT) {
	if (abs(expectedEnd - tokenEnd) <= 2) return;
    }
    if (match == KERBEROS) {
...
static void GetTokenLife (name, passwd, expectedLife, match)
  IN char *name;
  IN char *passwd;
  ...
  IN char *arock;
{
	...
	code = ktc_GetToken (&tgs_server, &tgs_token, sizeof(tgs_token),
			     &tgs_client);
	if (code) {
	...
	if (code != KABADUSER) {
	    com_err (whoami, code, "expected BADUSER error, getting AFS token w/ old tgs token but with NOTGS set");
	    CRASH();
	} else printf ("Received expected error code\n");
	SetFlags (ubikConn, name, KAFNORMAL);
	...
	if (patient) {			/* this requires waiting too long */
		...
		if (((now - tgs_token.startTime) % 60) == 0) {
		    fprintf (stdout,
			     "%d seconds to go...", (now-tgs_token.startTime));
		    fflush (stdout);
		}
	    ...
	    code = ka_GetServerToken (AUTH_SUPERUSER, "", cell,
				      MAXKTCTICKETLIFETIME, &afsToken, 1);
	    if (code) {
		com_err (whoami, code, "getting AFS token w/ old tgs token");
		CRASH();
	    }
	    CheckLife (afsToken.endTime, afsToken.startTime,
		       3600-(5*60), EXACT);
	}
