...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/rx/multi.example/sample_server.c,v 1.1.1.1 2002-01-31 21:49:23 zacheiss Exp $");
...
main() {
    ...
    service = rx_NewService(0, SAMPLE_SERVICE_ID, "sample", securityObjects, N_SECURITY_OBJECTS, TEST__ExecuteRequest);
...
int TEST_Add(call, verbose, a, b, result)
    struct rx_call *call;
    int verbose;
    int a,b;
    int *result;
{
    if (verbose) printf("TEST_Add(%d,%d,%d)\n", verbose, a, b);
    *result = a + b;
    return 0;
}

int TEST_Sub(call, verbose, a, b, result)
    struct rx_call *call;
    int verbose;
    int a,b;
    int *result;
{
    if (verbose) printf("TEST_Sub(%d,%d,%d)\n", verbose, a, b);
    *result = a - b;
    return 0;
}
