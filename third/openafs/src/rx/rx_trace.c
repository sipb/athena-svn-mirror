...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/rx/rx_trace.c,v 1.1.1.1 2002-01-31 21:32:04 zacheiss Exp $");
...
char **argv;
{
  ...
  while (read(rxi_logfd, &ip, sizeof(struct rx_trace))) {
    printf("%9u ", ip.now);
    switch (ip.event) {
    case RX_CALL_END:     putchar('E'); break;
    case RX_CALL_START:   putchar('S'); break;
    case RX_CALL_ARRIVAL: putchar('A'); break;
    case RX_TRACE_DROP:   putchar('D'); break;
