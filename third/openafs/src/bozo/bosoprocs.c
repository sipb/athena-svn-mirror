...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/bozo/bosoprocs.c,v 1.1.1.2 2002-12-13 20:40:19 zacheiss Exp $");
...
struct ktime *aktime; {
    ...
    if (DoLogging) bozo_Log("%s is executing SetRestartTime\n", caller);
...
register char *aname; {
    ...
    if (DoLogging) bozo_Log("%s is executing UnInstall '%s'\n", caller, filepath);
...
static void SaveOldFiles(char *aname)
{
    register afs_int32 code;
...
afs_int32 mode; {
    ...
    if (DoLogging) bozo_Log("%s is executing Install '%s'\n", caller, filepath);
...
char *aname; {
	...
	bozo_Log("ERROR: SetCellName: cell name '%s' exceeds %ld bytes (cell name not changed)\n",
...
register struct rx_call *acall; {
    ...
    if (DoLogging) bozo_Log("%s is executing Wait for All\n", caller);
...
fail:
    osi_auditU(acall, BOS_DeleteBnodeEvent, code, AUD_STR, ainstance, AUD_END);
    return code;
}

static swproc(abnode, arock)
register struct bnode *abnode;
char *arock; {
    if (abnode->goal == BSTAT_NORMAL) return 0; /* this one's not shutting down */
...
static stproc(abnode, arock)
struct bnode *abnode;
char *arock; {
    if (abnode->fileGoal == BSTAT_SHUTDOWN) return 0;	/* don't do these guys */
...
char **astatDescr; {
    ...
    *astatDescr = (char *) malloc(BOZO_BSSIZE);
    ...
    bnode_Release(tb);
    if (code) (*astatDescr)[0] = 0;	/* null string means no further info */
    return 0;
...
void bozo_ShutdownAndExit(int asignal)
{
    ...
    bozo_Log("Shutdown of BOS server and processes in response to signal %d\n",
	     asignal);
