...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/bozo/smail-notifier.c,v 1.1.1.1 2002-01-31 21:48:55 zacheiss Exp $");
    ...
    char **argv;
{
    ...
    afs_int32 pid=-1, lastExit=-1, lastSignal=-1, rsTime=-1, rsCount=-1;
    afs_int32 procStartTime=-1, procStarts=-1;
    afs_int32 lastAnyExit=-1, lastErrorExit=-1, errorCode=-1, errorSignal=-1, goal=-1;
    char *timeStamp;
	...
	if (!strcmp(typep, "BEGIN") && !strcmp(cmd, "bnode_proc"))  {
		...
		if (!strcmp(typep, "comLine:"))
		    ...
		    printf("Unexpected token %s in the bnode_proc (should be END)\n", typep);
		    exit(1);
		}
	    }
...
*/
		    ...
		    printf("Unexpected token %s in the bnode (should be END)\n", typep);
		    exit(1);
		}
	    }
	} else {
    ...
    bufp += strlen(bufp);    
    (void) sprintf(bufp, "AUTOMATIC NOTIFICATION EVENT FOR AFS SERVER INSTANCE %s\n\n", name);
    bufp += strlen(bufp);    
    (void) sprintf(bufp, "Server Process id was: %d\n", pid);
    bufp += strlen(bufp);    
    (void) sprintf(bufp, "Server command line: %s\n", comLine);
    bufp += strlen(bufp);    
    if (strcmp(coreName, "(null)")) core = 1;
    bp = comLine;
    ...
    (void) sprintf(bufp, "There is %score dump left %sfor this server\n", 
		   (core?(c?"a recent ":"an 'old' "):"no "), (core?bp:""));
    bufp += strlen(bufp);    
#ifdef	notdef
