...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/butc/butc_xbsa.c,v 1.1.1.1 2002-01-31 21:48:57 zacheiss Exp $");
...
BSA_Int16 (*XBSAQueryObject)(BSA_UInt32, QueryDescriptor *, ObjectDescriptor *);
...
xbsa_error(int rc, struct butx_transactionInfo *info)
{
	...
	break;
      case BSA_RC_NO_MATCH :
        ELog(0,"     XBSA: There were no matches found for the specified object\n");
	break;
	...
	break;
      case BSA_RC_DESC_TOO_LONG :
        ELog(0,"     XBSA: The description string was longer than expected\n");
	break;
      case BSA_RC_OBJINFO_TOO_LONG :
        ELog(0,"     XBSA: The object info string was longer than expected\n");
	break;
 ...
 * XBSAQueryApiVersion
 */
afs_int32 xbsa_MountLibrary(struct butx_transactionInfo *info, afs_int32 serverType)
{
    void *dynlib;
    int rc;
	...
	dynlib = dlopen("/usr/lib/libXApi.a(bsashr10.o)", RTLD_NOW | RTLD_LOCAL | RTLD_MEMBER);
	if (dynlib == NULL) {
	    dynlib = dlopen("/usr/lib/libXApi.a(xbsa.o)", RTLD_NOW | RTLD_LOCAL | RTLD_MEMBER);
	}
...
#if defined(AFS_AIX_ENV) || defined(AFS_SUN5_ENV)
    XBSAInit        = (BSA_Int16(*)(BSA_UInt32 *, SecurityToken *, ObjectOwner *, char **))dlsym((void *)dynlib, "BSAInit");
    XBSABeginTxn    = (BSA_Int16(*)(BSA_UInt32))dlsym((void *)dynlib, "BSABeginTxn");
    XBSAEndTxn      = (BSA_Int16(*)(BSA_UInt32, Vote))dlsym((void *)dynlib, "BSAEndTxn");
    XBSATerminate   = (BSA_Int16(*)(BSA_UInt32))dlsym((void *)dynlib, "BSATerminate");
    XBSAQueryObject = (BSA_Int16(*)(BSA_UInt32, QueryDescriptor *, ObjectDescriptor *))dlsym((void *)dynlib, "BSAQueryObject");
    XBSAGetObject   = (BSA_Int16(*)(BSA_UInt32, ObjectDescriptor *, DataBlock *))dlsym((void *)dynlib, "BSAGetObject");
    XBSAEndData     = (BSA_Int16(*)(BSA_UInt32))dlsym((void *)dynlib, "BSAEndData");
    XBSACreateObject       = (BSA_Int16(*)(BSA_UInt32, ObjectDescriptor *, DataBlock *))dlsym((void *)dynlib, "BSACreateObject");
    XBSAMarkObjectInactive = (BSA_Int16(*)(BSA_UInt32, ObjectName *))dlsym((void *)dynlib, "BSAMarkObjectInactive");
    XBSADeleteObject       = (BSA_Int16(*)(BSA_UInt32, CopyType, ObjectName *, CopyId *))dlsym((void *)dynlib, "BSADeleteObject");
    XBSASendData           = (BSA_Int16(*)(BSA_UInt32, DataBlock *))dlsym((void *)dynlib, "BSASendData");
    XBSAGetData            = (BSA_Int16(*)(BSA_UInt32, DataBlock *))dlsym((void *)dynlib, "BSAGetData");
    XBSAQueryApiVersion    = (BSA_Int16(*)(ApiVersion *))dlsym((void *)dynlib, "BSAQueryApiVersion");
    XBSAGetEnvironment     = (BSA_Int16(*)(BSA_UInt32, ObjectOwner *, char **))dlsym((void *)dynlib, "BSAGetEnvironment");
	 ...
	 */
	ELog(0,"xbsa_MountLibrary: The XBSAQueryApiVersion call returned an incompatible version, %d %d %d\n",
	     info->apiVersion.version, info->apiVersion.release, info->apiVersion.level);
	...
	if ((info->apiVersion.version > XBSA_ADSM_NO_MULT_SERVER_VERSION)   ||
	    ...
	    (info->apiVersion.version == XBSA_ADSM_NO_MULT_SERVER_VERSION &&
	     info->apiVersion.release > XBSA_ADSM_NO_MULT_SERVER_RELEASE)   ||
 ...
 * BSAGetEnvironment
 */
afs_int32 xbsa_Initialize(struct butx_transactionInfo *info,
		      ...
		      char *secToken, char *serverName)
{
    ...
    if (debugLevel > 98) {
       printf("\nxbsa_Initialize bsaObjectOwner='%s' appObjectOwner='%s' "
	      ...
	      GOODSTR(bsaObjectOwner), GOODSTR(appObjectOwner), GOODSTR(serverName));
    }

    if (info->bsaHandle != 0) {
	/* We already have a handle */
	ELog(0,"xbsa_Initialize: The dlopen call to load the XBSA shared library failed\n");
    ...
    if (rc != BSA_RC_SUCCESS) {
	ELog(0,"xbsa_Initialize: The XBSAGetEnvironment call failed with %d\n", rc);
	xbsa_error(rc, info);
	...
	if (info->maxObjects == 0) {
	    ELog(0,"xbsa_Initialize: The XBSAGetEnvironment call failed to return the MAXOBJ string\n");
	    return(BUTX_GETENVFAIL);
	}
        break;
    }
 ...
 * BSABeginTxn
 */
afs_int32 xbsa_BeginTrans(struct butx_transactionInfo *info)
{
    int rc;
    ...
    if (debugLevel > 98) {
       ...
       printf("serverName='%s' ; bsaObjectOwner='%s' ; appObjectOwner='%s' ; sectoken=xxxxxx\n",
	      GOODSTR(info->serverName), GOODSTR(info->objOwner.bsaObjectOwner),
 ...
 * BSAEndTxn
 */
afs_int32 xbsa_EndTrans(struct butx_transactionInfo *info)
{
    int rc;
 ...
 * BSATerminate
 */
afs_int32 xbsa_Finalize(struct butx_transactionInfo *info)
{
    int rc;
 ...
 * BSAQueryObject
 */
afs_int32 xbsa_QueryObject(struct butx_transactionInfo *info,
		       ...
		       char *pathName)
{
    ...
    if (info->bsaHandle == 0) {
	/* We haven't initialized yet! */
	ELog(0,"xbsa_QueryObject: No current handle, butx not initialized\n");
	return(BUTX_NOHANDLE);
    }
    ...
    rc = (int)XBSAQueryObject(info->bsaHandle, &queryDescriptor, &info->curObject);
    if (rc == BSA_RC_NO_MORE_DATA) rc = BSA_RC_SUCCESS; /* This is actually a success! */
    if (rc != BSA_RC_SUCCESS) {
 ...
 * BSAGetObject
 */
afs_int32 xbsa_ReadObjectBegin(struct butx_transactionInfo *info, char *dataBuffer,
			   afs_int32 bufferSize, afs_int32 *count, afs_int32 *endOfData)
{
    int rc;
    DataBlock dataBlock;
    ...
    if (info->bsaHandle == 0) {
	/* We haven't initialized yet! */
	ELog(0,"xbsa_ReadObjectBegin: No current handle, butx not initialized\n");
	return(BUTX_NOHANDLE);
    }
    ...
    *endOfData = 0;
    ...
    if ((rc != BSA_RC_MORE_DATA) && (rc != BSA_RC_NO_MORE_DATA)) {
        ELog(0,"xbsa_ReadObjectBegin: The XBSAGetObject call failed with %d\n", rc);
	xbsa_error(rc, info);
	return(BUTX_GETOBJFAIL);
    }
    *count = dataBlock.numBytes;
    if (rc == BSA_RC_NO_MORE_DATA) *endOfData = 1;
    return(XBSA_SUCCESS);
}
 ...
 * BSAEndData()
 */
afs_int32 xbsa_ReadObjectEnd(struct butx_transactionInfo *info)
{
    int rc;
    ...
    if (info->bsaHandle == 0) {
	/* We haven't initialized yet! */
	ELog(0,"xbsa_ReadObjectEnd: No current handle, butx not initialized\n");
	return(BUTX_NOHANDLE);
    }
 ...
 * BSACreateObject
 */
afs_int32 xbsa_WriteObjectBegin(struct butx_transactionInfo *info,
			    char *objectSpaceName, char *pathName,
			    char *lGName, afs_hyper_t estimatedSize,
			    char *objectDescription, char *objectInfo)
{
    int rc;
    int length;
    DataBlock dataBlock;
    ...
    if (info->bsaHandle == 0) {
	/* We haven't initialized yet! */
	ELog(0,"xbsa_WriteObjectBegin: No current handle, butx not initialized\n");
	return(BUTX_NOHANDLE);
    }
	...
	if ( ( (XBSA_GET_SERVER_TYPE(info->serverType) == XBSA_SERVER_TYPE_ADSM) &&
	    ...
	    ELog(0,"xbsa_WriteObjectBegin: The objectDescription is too long; size = %d; name = %s\n",
	...
	if ( ( (XBSA_GET_SERVER_TYPE(info->serverType) == XBSA_SERVER_TYPE_ADSM) &&
	    ...
	    ELog(0,"xbsa_WriteObjectBegin: The objectInfo is too long; size = %d; name = %s\n",
    ...
    if (rc != BSA_RC_SUCCESS) {
        ELog(0,"xbsa_WriteObjectBegin: The XBSACreateObject call failed with %d\n", rc);
	xbsa_error(rc, info);
 ...
 * BSADeleteObject()
 */
afs_int32 xbsa_DeleteObject(struct butx_transactionInfo *info,
			char *objectSpaceName, char *pathName)
{
    int rc;
    ...
    if (info->bsaHandle == 0) {
	/* We haven't initialized yet! */
	ELog(0,"xbsa_DeleteObject: No current handle, butx not initialized\n");
	return(BUTX_NOHANDLE);
    }
    ...
    if (rc != BSA_RC_SUCCESS ) {
	ELog(0,"xbsa_DeleteObject: XBSAMarkObjectInactive call failed with %d\n", rc);
	xbsa_error(rc, info);
	return((rc==BSA_RC_ABORT_ACTIVE_NOT_FOUND) ? BUTX_DELETENOVOL : BUTX_DELETEOBJFAIL);
    }
 ...
 * BSAEndData()
 */
afs_int32 xbsa_WriteObjectEnd(struct butx_transactionInfo *info)
{
    int rc;
    ...
    if (info->bsaHandle == 0) {
	/* We haven't initialized yet! */
	ELog(0,"xbsa_WriteObjectEnd: No current handle, butx not initialized\n");
	return(BUTX_NOHANDLE);
    }
    rc = (int)XBSAEndData(info->bsaHandle);
    if (rc != BSA_RC_SUCCESS) {
 ...
 * BSASendData
 */
afs_int32 xbsa_WriteObjectData(struct butx_transactionInfo *info,
			   char *dataBuffer, afs_int32 bufferSize, afs_int32 *count)
{
    int rc;
    DataBlock dataBlock;
    ...
    if (info->bsaHandle == 0) {
	/* We haven't initialized yet! */
	ELog(0,"xbsa_WriteObjectData: No current handle, butx not initialized\n");
	return(BUTX_NOHANDLE);
    }
 ...
 * BSAGetData
 */
afs_int32 xbsa_ReadObjectData(struct butx_transactionInfo *info, char *dataBuffer,
			  afs_int32 bufferSize, afs_int32 *count, afs_int32 *endOfData)
{
    int rc;
    DataBlock dataBlock;
    ...
    if (info->bsaHandle == 0) {
	/* We haven't initialized yet! */
	ELog(0,"xbsa_ReadObjectData: No current handle, butx not initialized\n");
	return(BUTX_NOHANDLE);
    }
