...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/butc/read_tape.c,v 1.1.1.2 2002-12-13 20:41:52 zacheiss Exp $");
  ...
  char *buffer;
{
  ...
  while (total < BUTM_BLOCKSIZE) {
	...
	return(rc);
        ...
        if (verbose) fprintf(stderr, "Hardware file mark\n");
	if (++fmcount > 3) {
	   if (verbose)
	      fprintf(stderr, "Greater than 3 hardware file marks in a row - done\n");
	   return -1;
	}
     }
     else if (nread == 0) {
  ...
  struct tapeLabel *tapeLabelPtr;
{
  ...
  tapeLabelPtr->label.creationTime   = ntohl(tapeLabelPtr->label.creationTime);
  tapeLabelPtr->label.expirationDate = ntohl(tapeLabelPtr->label.expirationDate);
  tapeLabelPtr->label.structVersion  = ntohl(tapeLabelPtr->label.structVersion);
  ...
  afs_int32 *isvolheader;
{
  ...
  *isvolheader = 0;
  ...
  if (headerPtr->magic == TC_VOLBEGINMAGIC) {
     *isvolheader = 1;
     if (verbose) fprintf(stderr, "Volume header\n");
     ...
     if (verbose) fprintf(stderr, "Volume Trailer\n");
  }
  else {
  ...
  struct volumeHeader *headerPtr;
{
  ...
  if (outfile) {
     ...
     if ((len>7) && (strcmp(".backup",filename+len-7)==0)) {
        filename[len-7] = 0;
     } else if ((len>9) && (strcmp(".readonly",filename+len-9)==0)) {
        filename[len-9] = 0;
     }
  }
  ...
  if (ask) {
     first = 1;
     skip  = 0;
     printf("Press return to retreive volume %s (%u) to file %s; ""s"" to skip\n",
...
int writeLastBlocks(char *lastblock, char *lastblock2)
{
  int rc;
  ...
  char *arock;
{
  ...
  nrestore     = (as->parms[1].items ? atol(as->parms[1].items->data) : 0x7fffffff);
  nskip        = (as->parms[2].items ? atol(as->parms[2].items->data) : 0);
  if (as->parms[4].items) nskip = 0x7fffffff;         /* -scan     */
  ...
  do {    /* while ((nskip!=0) && (nrestore!=0)) */
	...
	if (isheader) {
      ...
      */
	   ...
	   if (verbose) fprintf(stderr, "File mark volume begin\n");
	} else if (fmtype == FILE_END) {
	   if (verbose) fprintf(stderr, "File mark volume end\n");
	} else if (fmtype == FILE_EOD) {
	   if (verbose) fprintf(stderr, "File mark end-of-dump\n");
	   eod = 1;
	}
     }
     ...
     /* A dump label */
        ...
        if (verbose) fprintf(stderr, "Dump label\n");
        printLabel(label);
	eod = 0;
     }
  ...
  char **argv;
{
  ...
  cmd_AddParm(ts, "-noask",   CMD_FLAG,   CMD_OPTIONAL, "Prompt for each volume");
  cmd_AddParm(ts, "-label",   CMD_FLAG,   CMD_OPTIONAL, "Display dump label");
  cmd_AddParm(ts, "-vheaders",CMD_FLAG,   CMD_OPTIONAL, "Display volume headers");
