...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/vfsck/proplist.c,v 1.1.1.1 2002-01-31 21:50:35 zacheiss Exp $");
...
int proplist_blkscan(dp, idesc, entry_list)
   struct dinode *dp;
   struct inodesc *idesc;
   ...
   *entry_list = NULL;
   ...
   for (;;) {
	 ...
	 if (proplist_chkblock(bp, blksize)) {
	    bp->b_flags &= ~B_INUSE;
	    pwarn("PROPERTY LIST BLOCK CORRUPTED I=%u",
		  idesc->id_number);
	    if (preen)
	       printf(" (CLEARED)\n");
	 ...
	 if (valueresid < 0 ||
	     (valueresid && strcmp((char *)&pl_descp[1],
		    entry->name))) {
	    entry->flags |= PROP_ENTRY_BAD;
	    valueresid = 0;
	 }
	 if (valueresid == 0) {
	    entry = (struct prop_entry_desc *)malloc(sizeof(struct prop_entry_desc));
	    if (entry == NULL)
	       return(SKIP);
	    entry->next = NULL;
	    entry->flags = 0;
...
int proplist_markdup(entry_list)
   struct prop_entry_desc *entry_list;
{
...
int proplist_updateblks(dp, idesc, entry_list)
   struct dinode *dp;
   struct inodesc *idesc;
   struct prop_entry_desc *entry_list;
{
...
int proplist_chkblock(bp, blksize)
   struct bufarea *bp;
   int blksize;
{
   struct proplist_desc1 *pl_descp;
   ...
   for (offsetinbuf = 0; offsetinbuf < blksize;) {
	 ...
	 return(1);
      ...
      if (pl_descp->pl_entrysize % 8 ||
	  pl_descp->pl_nextentry % 8 ||
...
int proplist_blkdel(dp, idesc, nextblk)
   struct dinode *dp;
   struct inodesc *idesc;
   daddr_t nextblk;
{
