...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/vlserver/sascnvldb.c,v 1.2 2003-03-20 00:09:45 zacheiss Exp $");
...
pos=0;
     ...
     for (i=1;i<argc;i++) {
	   ...
	   case 'n' : /* -name */
	     if (pathname) {
	       fprintf(stderr, "%s: -name specified (or implied) twice!\n",pn);
	       exit(-1);
	     }
	     pathname = argv[++i];
	     break;
...
if (fromvers != tovers ) {  /* only supports 1 and 2 currently */
  ...
  for (i=0;i<3;i++)
    vlentry->nextIdHash[i] = htonl(ntohl(vlentry->nextIdHash[i]) + diff);
