...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/vlserver/vlutils.c,v 1.1.1.1 2002-01-31 21:32:09 zacheiss Exp $");
...
afs_int32 vlwrite (trans, offset, buffer, length)
...
{   afs_int32	errorcode;
    ...
    if (errorcode = ubik_Seek(trans, 0, offset)) return errorcode;
    return(ubik_Write(trans, buffer, length));
}


/* Package up seek and read into one procedure for ease of use */
afs_int32 vlread (trans, offset, buffer, length)
struct ubik_trans   *trans;
afs_int32		    offset;
char		    *buffer;
afs_int32		    length;
...
afs_int32 vlentrywrite(trans, offset, buffer, length)
struct ubik_trans   *trans;
afs_int32		    offset;
char		    *buffer;
afs_int32		    length; {
    ...
    if (length != sizeof(oentry)) return -1;
    if (maxnservers == 13) {
...
afs_int32 vlentryread(trans, offset, buffer, length)
struct ubik_trans   *trans;
afs_int32		    offset;
char		    *buffer;
afs_int32		    length; {
    ...
    if (length != sizeof(vlentry)) return -1;
...
{
    if (vlwrite(trans, 0, (char *) &cheader.vital_header, sizeof(vital_vlheader))) return VL_IO;
    return 0;
}
  ...
  struct ubik_trans *trans;
{
  ...
  for (i=1; i < VL_MAX_ADDREXTBLKS; i++) {
	...
	if (!ex_addr[i])
     ...
     code = vlread(trans, ntohl(ex_addr[0]->ex_contaddrs[i]),
...
afs_int32 CheckInit (trans, builddb)
    struct ubik_trans   *trans;
    int builddb;
{
	...
	else ERROR_EXIT(VL_EMPTY);
    }
    else if ((vldbversion != VLDBVERSION)  &&
	     (vldbversion != OVLDBVERSION) &&
...
afs_int32 GetExtentBlock(trans, base) 
    register struct ubik_trans  *trans;
    register afs_int32 base;
{
	...
	code = vlwrite(trans, blockindex, (char *)ex_addr[base], VL_ADDREXTBLK_SIZE);
	...
	if (base == 0) {
	    ...
	    code = vlwrite(trans, DOFFSET(0,&cheader,&cheader.SIT), 
	...
	code = vlwrite(trans, ntohl(cheader.SIT), ex_addr[0], sizeof(struct extentaddr));
	if (code) ERROR_EXIT(VL_IO);
    }
...
afs_int32 FindExtentBlock(trans, uuidp, createit, hostslot, expp, basep)
    register struct ubik_trans  *trans;
    afsUUID *uuidp;
    ...
    struct extentaddr **expp;
{
    ...
    *basep = 0;
	...
	if (hostslot == -1) {
	    for (i=0; i<MAXSERVERID+1; i++) {
		if (!HostAddress[i]) break;
	    }
	    if (i > MAXSERVERID) ERROR_EXIT(VL_REPSFULL);
	} else {
	    i = hostslot;
	}
	...
	for (base=0; base<VL_MAX_ADDREXTBLKS; base++) {
		...
		if (afs_uuid_is_nil(&tuuid)) {
		    ...
		    code = vlwrite(trans, DOFFSET(ntohl(ex_addr[0]->ex_contaddrs[base]), 
						  (char *)ex_addr[base], (char *)exp),
				   (char *) &tuuid, sizeof(tuuid));
		    if (code) ERROR_EXIT(VL_IO);
		    HostAddress[i] = 0xff000000 | ((base << 16) & 0xff0000) | (j & 0xffff);
		    *expp = exp;
		    *basep = base;
		    ...
		    code = vlwrite(trans, DOFFSET(0, &cheader, &cheader.IpMappedAddr[i]),
				   (char *) &cheader.IpMappedAddr[i], sizeof(afs_int32));
		    if (code) ERROR_EXIT(VL_IO);
		    ERROR_EXIT(0);
		}
	    }
...
afs_int32 AllocBlock (trans, tentry)
register struct ubik_trans  *trans;
...
{   struct nvlentry	tentry;
    ...
    if (write_vital_vlheader(trans))   return VL_IO;
    return 0;
}
...
afs_int32 FindByID (trans, volid, voltype, tentry, error)
struct ubik_trans   *trans;
afs_int32		    volid;
afs_int32		    voltype;
struct nvlentry	    *tentry;
...
/* Should we have one big hash table for volids as opposed to the three ones? */
	for (typeindex = 0; typeindex < MAXTYPES; typeindex++) {
	    for (blockindex = ntohl(cheader.VolidHash[typeindex][hashindex]); blockindex != NULLO; blockindex = tentry->nextIdHash[typeindex]) {
	...
	for (blockindex = ntohl(cheader.VolidHash[voltype][hashindex]); blockindex != NULLO; blockindex = tentry->nextIdHash[voltype]) {
	    if (vlentryread(trans, blockindex, (char *) tentry, sizeof(nvlentry))) {
		*error = VL_IO;
		return 0;
	    }
...
afs_int32 FindByName (trans, volname, tentry, error)
struct ubik_trans   *trans;
char		    *volname;
struct nvlentry	    *tentry;
afs_int32		    *error;
{   register afs_int32   hashindex;
    ...
    else if (hashindex >= 10 && strcmp(volname+hashindex-9, ".readonly")==0) {
	/* this is a readonly volume */
	strcpy(tname, volname);
	tname[hashindex-9] = 0; /* zap extension */
    }
    else strcpy(tname, volname);

    *error = 0;
    hashindex = NameHash(tname);
    for (blockindex = ntohl(cheader.VolnameHash[hashindex]); blockindex != NULLO; blockindex = tentry->nextNameHash) {
	if (vlentryread(trans, blockindex, (char *) tentry, sizeof(nvlentry))) {
	    *error = VL_IO;
	    return 0;
	}
...
{   register int   i=0;
    ...
    for (blockindex = ntohl(cheader.VolnameHash[hashindex]); blockindex != NULLO; blockindex = tentry.nextNameHash) {
	if (vlentryread(trans, blockindex, (char *) &tentry, sizeof(nvlentry))) return 0;
	i++;
...
int HashIdDump (trans, hashindex)
struct ubik_trans   *trans;
int		    hashindex;
...
{   register int   i=0;
    ...
    for (blockindex = ntohl(cheader.VolidHash[0][hashindex]); blockindex != NULLO; blockindex = tentry.nextIdHash[0]) {
	if (vlentryread(trans, blockindex, (char *) &tentry, sizeof(nvlentry))) return 0;
	i++;
	VLog(0, ("[%d]#%d: %10d %d %d (%s)\n", hashindex, i, tentry.volumeId[0],
	      tentry.nextIdHash[0],  tentry.nextNameHash, tentry.name));
...
{   int		    errorcode;
    ...
    if (vlentrywrite(trans, blockindex, (char *)tentry, sizeof(nvlentry))) return VL_IO;
    return 0;
}
...
int HashVolid(trans, voltype, blockindex, tentry)
struct ubik_trans   *trans;
afs_int32		    voltype;
afs_int32		    blockindex;
struct nvlentry	    *tentry;
{   afs_int32 hashindex, errorcode;
    ...
    if (FindByID(trans, tentry->volumeId[voltype], voltype, &ventry, &errorcode))
	return VL_IDALREADYHASHED;
    else if (errorcode) return errorcode;
    hashindex = IDHash(tentry->volumeId[voltype]);
    tentry->nextIdHash[voltype] = ntohl(cheader.VolidHash[voltype][hashindex]);
    cheader.VolidHash[voltype][hashindex] = htonl(blockindex);
    if (vlwrite(trans, DOFFSET(0, &cheader, &cheader.VolidHash[voltype][hashindex]), (char *) &cheader.VolidHash[voltype][hashindex], sizeof(afs_int32))) return VL_IO;
    return 0;
}


/* cheader must have be read before this routine is called. */
int UnhashVolid(trans, voltype, blockindex, aentry)
struct ubik_trans   *trans;
afs_int32		    voltype;
afs_int32		    blockindex;
struct nvlentry	    *aentry;
{   int hashindex, nextblockindex, prevblockindex;
	...
	cheader.VolidHash[voltype][hashindex] = htonl(aentry->nextIdHash[voltype]);
	code = vlwrite(trans, DOFFSET(0, &cheader, &cheader.VolidHash[voltype][hashindex]), (char *) &cheader.VolidHash[voltype][hashindex], sizeof(afs_int32));
	if (code) return VL_IO;
    } else {
	while (nextblockindex != blockindex) {
	    prevblockindex = nextblockindex;	/* always done once */
	    if (vlentryread(trans, nextblockindex, (char *)&tentry, sizeof(nvlentry))) 
		return VL_IO;
	...
	if (vlwrite(trans, DOFFSET(prevblockindex, &tentry, &tentry.nextIdHash[voltype]), (char *)&temp, sizeof(afs_int32)))
	    return VL_IO;
    }
...
{   register afs_int32   hashindex;
    ...
    code = vlwrite(trans, DOFFSET(0, &cheader, &cheader.VolnameHash[hashindex]), (char *) &cheader.VolnameHash[hashindex], sizeof(afs_int32));
    if (code) return VL_IO;
    return 0;
}
...
{   register afs_int32   hashindex, nextblockindex, prevblockindex;
	...
	if (vlwrite(trans, DOFFSET(0, &cheader, &cheader.VolnameHash[hashindex]), (char *) &cheader.VolnameHash[hashindex], sizeof(afs_int32))) return VL_IO;
    } else {
	while (nextblockindex != blockindex) {
	...
	if (vlwrite(trans, DOFFSET(prevblockindex, &tentry, &tentry.nextNameHash), (char *)&temp, sizeof(afs_int32)))
	    return VL_IO;
    }
...
afs_int32 NextEntry (trans, blockindex, tentry, remaining)
struct ubik_trans   *trans;
afs_int32		    blockindex;
struct nvlentry	    *tentry;
afs_int32		    *remaining;
{   register afs_int32    lastblockindex;
    ...
    for (lastblockindex = ntohl(cheader.vital_header.eofPtr); blockindex < lastblockindex;) {
	if (vlentryread(trans, blockindex, (char *)tentry, sizeof(nvlentry))) {
	    *remaining = -1;
	    return 0;
	}
	if (tentry->flags == VLCONTBLOCK) {
		...
		*remaining = (lastblockindex - blockindex) / sizeof(nvlentry) - 1;
		return blockindex;
	    }
	    blockindex += sizeof(nvlentry);
	}
...
afs_int32		    blockindex;
{
    if ((blockindex < sizeof(cheader)) || (blockindex >= ntohl(cheader.vital_header.eofPtr))) 
	return 0;
    return 1;
}
