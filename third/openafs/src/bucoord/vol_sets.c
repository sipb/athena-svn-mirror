...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/bucoord/vol_sets.c,v 1.1.1.1 2002-01-31 21:50:04 zacheiss Exp $");
...
afs_int32 bc_AddVolSetCmd(as, arock)
     struct cmd_syndesc *as;
     char *arock; 
{
    /* parm 0 is vol set name */
    ...
    code = bc_CreateVolumeSet(bc_globalConfig, (ti=as->parms[0].items)->data, flags);
    if (code) {
	if (code == -1)
	    com_err(whoami,0,"Volume set '%s' already exists", ti->data);
	else com_err(whoami,0,"Unknown problem");
    }
...
afs_int32 bc_DeleteVolEntryCmd(as, arock)
     struct cmd_syndesc *as;
     char *arock;
{
    register afs_int32 code;
    ...
    if (code) {
	   ...
	   com_err(whoami,0,"No such volume set as '%s'", vsname);
	    ...
	    com_err(whoami,0,"There aren't %d volume items for this volume set", entry);
	    com_err(whoami,0,"Use the 'listvolsets' command to examine the volume set");
	}
	ERROR(code);
    }
...
afs_int32 bc_DeleteVolSetCmd(as, arock)
     struct cmd_syndesc *as;
     char *arock; 
{
    /* parm 0 is vol set name */
   ...
   /* lock schedules and check validity */
	...
	if (code) {
	    if (code == -1) 
	    	...
	    	com_err(whoami, code, "; Unknown problem deleting volume set '%s'", ti->data);
	} else {
	    if (!(flags & VSFLAG_TEMPORARY)) tosave = 1;
...
afs_int32 bc_ListVolSetCmd(as, arock)
     struct cmd_syndesc *as;
     char *arock; 
{
    ...
    struct bc_volumeSet *aset; 
{
	...
	printf("    Entry %3d: server %s, partition %s, volumes: %s\n",
	       i, tentry->serverName, tentry->partname, tentry->name);
    }
    return 0;
}
...
bc_ParseVolumeSet()
{
	...
	{ /*e*/
	    ...
	    if (!tve->serverName)
	    {
		com_err(whoami,0,"Can't malloc() a new volume spec server name field!");
		return(-1);
	    }
	    ...
	    if (!tve->partname)
	    {
		com_err(whoami,0, "Can't malloc() a new volume spec partition pattern field!");
		return(-1);
	    }
	    ...
	    if (!tp) 
	    {
		com_err(whoami,0, "Can't malloc() a new volume spec volume pattern field!");
		return(-1);
	    }
	    strcpy(tp, vsname);
	    tve->name =	tp;
...
bc_SaveVolumeSet()
{
    ...
    for(tset = bc_globalConfig->vset; tset != 0; tset=tset->next) {
        if (tset->flags & VSFLAG_TEMPORARY) continue;  /* skip temporary entries */
...
afs_int32 bc_UpdateVolumeSet()
{
    ...
    code = bc_openTextFile(ctPtr, &bc_globalConfig->tmpTextFileNames[TB_VOLUMESET][0]);
    if ( code )
      ERROR(code);
