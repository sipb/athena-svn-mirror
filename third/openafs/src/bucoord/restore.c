...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/bucoord/restore.c,v 1.1.1.1.4.1 2003-08-20 16:52:46 ghudson Exp $");
     ...
     afs_int32 aindex; 
{
     ...
     */
    for (tvol = dumpTaskPtr->volumes; tvol; tvol = tvol->next) 
    { /*tvol*/
	...
	{
	    com_err(whoami, code, "; Can't find any dump for volume %s", tvol->name);
	    continue;
	}
	...
	{
	    if      (di->DumpId <  dumpDescr->id) { di = 0; break; }
	    else if (di->DumpId == dumpDescr->id) { break; }
	}
	...
	if (!di->lastinlist) di->volinfolist = vi;
	else di->lastinlist->next = vi;
	di->lastinlist = vi;
    } /*tvol*/
	     ...
	     parent=dlevels[lvl].parentDumpId, lvl++)
	{
		...
		{
		    com_err(whoami, code, "; Can't find parent DumpID %u for volume %s",
			    parent, vi->volname);
		}
		break;
	    }
	...
	for (vi=di->volinfolist; vi; vi=vi->next)
	{
	    ...
	    for (lv=(lvl-1); lv>=0; lv--)
	    {
	        ...
	        tapeid = (dlevels[lv].initialDumpId ? dlevels[lv].initialDumpId :
			                              dlevels[lv].DumpId);

	        /* Cycle through the volume fragments for this volume on the tape */
	        for (last=0,c=0; last>=0; last=next,c++)
		{
		    code = bcdb_FindVolumes(dlevels[lv].DumpId, vi->volname, volumeEntries, 
		        ...
		        if (code == BUDB_ENDOFLIST)
			{
		        ...
		        com_err(whoami, code, "; Can't find volume %s in DumpID %u",
			...
			{
			    tid = (tle->initialDumpID ? tle->initialDumpID : tle->dumpID);
			    ...
			    {
			        if (tlid != tid)
				    { tle = 0; break; }   /* Allocate and insert a tape entry */
			    }
				...
				if (tapeseq == tle->tapeNumber) 
				    { break; }            /* Don't allocate tape entry */
			...
			if (!tle)
			{
			    tle = (struct bc_tapeList *) malloc(sizeof(struct bc_tapeList));
			    if (!tle)
			    {
			        com_err(whoami,BC_NOMEM,"");
				return(BC_NOMEM);
			...
			tlid = (tle->initialDumpID ? tle->initialDumpID : tle->dumpID);

			/* Now place the volume fragment into the correct position on 
			 * this tapelist entry. Duplicate entries are ignored.
			 */
			for (pti = 0, ti=tle->restoreList; ti; pti=ti, ti=ti->next)
			{
			    if (volumeEntries[ve].position < ti->position) 
			        { ti = 0; break; }     /* Allocate an entry */

			    else if (volumeEntries[ve].position == ti->position) 
			...
			if (!ti)
			{
			    ti = (struct bc_tapeItem *) malloc(sizeof(struct bc_tapeItem));
			    if (!ti)
			    {
			        com_err(whoami,BC_NOMEM,"");
				return(BC_NOMEM);
        ...
        for (tle=tapeList; tle; tle=tle->next)
	{
	    ...
	    {
	        tapedumpid = (tle->initialDumpID ? tle->initialDumpID : tle->dumpID);
		strcpy(vname, ti->volumeName);
		StripBackup(vname);
		if (dumpTaskPtr->newExt) strcat(vname, dumpTaskPtr->newExt);

		/* print volume to restore and the tape and position its on */
	        if (serverAll)
		{
		    printf("Restore volume %s (ID %d) from tape %s (%u), position %d",
			   ti->volumeName, ti->oid, tle->tapeName, tapedumpid, ti->position);
		...
		{
		    haddr = ti->server;
		    hostPtr = gethostbyaddr((char *) &haddr, sizeof(haddr), AF_INET);
		    if (hostPtr) printf("%s", hostPtr->h_name);
		    else         printf("%u", ti->server);
		    
		    printf(" /vicep");
		    viceName(ti->partition);

		    printf(" %s # as %s; %s (%u); pos %d;", 
			   ti->volumeName, vname, tle->tapeName, tapedumpid, ti->position);
    ...
    tcarray = (struct tc_restoreDesc *) malloc(nentries*sizeof(struct tc_restoreDesc));
    if (!tcarray)
    {
        com_err(whoami,BC_NOMEM,"");
	ERROR(BC_NOMEM);
	...
	for (ti=tle->restoreList; ti; ti=ti->next) 
	{
	    ...
	    if (dumpTaskPtr->newExt) strcat(tcarray[i].newName, dumpTaskPtr->newExt);
	...
	else 
	{
	    ...
	    port  = dumpTaskPtr->portOffset[(level <= dumpTaskPtr->portCount-1 ? 
				       level : dumpTaskPtr->portCount-1)];

	    for (todump=1; ((startentry+todump) < nentries); todump++)
	    {
		level     = tcarray[startentry+todump].dumpLevel;
		nextport  = dumpTaskPtr->portOffset[(level <= dumpTaskPtr->portCount-1 ? 
					       level : dumpTaskPtr->portCount-1)];
		if (port != nextport) break;          /* break if we change ports */
	    }
	}
	...
	code = TC_PerformRestore(aconn, "DumpSetName", &rpcArray, &dumpTaskPtr->dumpID);
	if (code)
	{
	...
	while (1)
	{
	    ...
	    if (!newStatusPtr || (newStatusPtr->flags &
				  (ABORT_REQUEST|ABORT_SENT|ABORT_DONE|TASK_ERROR)) )
	    {
	        unlock_Status();
		ERROR(0);
	    }
  ...
  error_exit:
	...
	{
	    nti = ti->next;
	    if (ti->volumeName) free(ti->volumeName);
	    free(ti);
	}
	ntle = tle->next;
	if (tle->tapeName) free(tle->tapeName);
	free(tle);
    }
