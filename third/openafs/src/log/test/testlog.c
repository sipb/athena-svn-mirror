...
RCSID("$Header: /afs/dev.mit.edu/source/repository/third/openafs/src/log/test/testlog.c,v 1.1.1.1 2002-01-31 21:50:13 zacheiss Exp $");
...
{ /*Main routine*/
    ...
    /*
         ...
         */
    fprintf(stderr, "Getting tokens from the Cache Manager (non-cellular call)\n");
    U_GetLocalTokens(&testCTok, &testSTok);

    for (cellNum = 0; cellNum < 100; cellNum++)
	origInfo[cellNum].valid = 0;
    fprintf(stderr, "Getting tokens from the Cache Manager (cellular call)\n");
    useCellEntry = 1;
    cellNum = 0;
    do {
	printf("\t[%3d] ", cellNum);
        rc = U_CellGetLocalTokens(useCellEntry, cellNum, &origInfo[cellNum].cTok, &origInfo[cellNum].sTok, origInfo[cellNum].cellName, &isPrimary);
        if (rc) {
	...
	if (strcmp(argv[currArg], "-x") == 0) {
	    /*
	         ...
	         */
	    if (currArg != 1) {
		fprintf(stderr, "-x switch must appear before all other arguments.\n");
		usageError = TRUE;
		break;
		}
	    doLookup = FALSE;
	...
	if (pw == NULL) {
	    ...
	    fprintf (stderr, "Usage: testlog [[-x] user [password] [-c cellname]]\n\n");
	    exit (1);
	}
	foundUser = TRUE;
	doLookup = FALSE;
   ...
   if (usageError || !foundUser) {
	...
	sprintf(buf, "Password for user '%s' in cell '%s': ",
		lclpw->pw_name, cellID);
    ...
    cToken.ViceId = 123;
#endif DB_CELLS
    if ((rc = U_CellAuthenticate(pw->pw_name, passwd, cellID, &cToken, &sToken))
		!= AUTH_SUCCESS)
    ...
    fprintf(stderr, "Giving tokens to Cache Manager (cellular call, cell = '%s')\n",
	     cellID);
    notPrimary = 0;
    if (rc = U_CellSetLocalTokens(0, &cToken, &sToken, cellID, notPrimary) !=
         AUTH_SUCCESS)
        fprintf(stderr, "\tError: code %d ('%s')\n", rc, U_Error(rc));
