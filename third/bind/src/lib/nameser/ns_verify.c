 ...
 *	- TSIG verification fails with BADID (-ns_r_badid)
 ...
 *	- TSIG verification succeeds, error set to BADID (ns_r_badid)
 */
int
	  ...
	  time_t *timesigned, int nostrip)
{
	...
	if (n < 0)
		return (NS_TSIG_ERROR_FORMERR);
	if (strcasecmp(alg, NS_TSIG_ALG_HMAC_MD5) != 0)
		return (-ns_r_badkey);
	cp += n;
	...
	if (key != NULL) {
		...
		if (error != ns_r_badsig && error != ns_r_badkey) {
			if (strcmp(key->dk_key_name, name) != 0)
				return(-ns_r_badkey);
		}
	}
	      ...
	      int required)
{
	...
	/* Verify that the key used is OK */
	if ((strcmp(state->key->dk_key_name, name) != 0 ||
