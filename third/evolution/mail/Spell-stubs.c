				    ...
				    CORBA_Environment * ev)
{
   ...
   if (_obj->servant && _obj->vepv && GNOME_Spell_Dictionary__classid) {
	 ...
	 ((POA_GNOME_Spell_Dictionary__epv *) _obj->
	  vepv[GNOME_Spell_Dictionary__classid])->getLanguages(_obj->servant,
							       ev);
 ...
 _ORBIT_retry_request:
   ...
   {				/* demarshalling */
	 ...
	 goto _ORBIT_msg_exception;
      _ORBIT_retval = GNOME_Spell_LanguageSeq__alloc();
      _ORBIT_curptr = GIOP_RECV_BUFFER(_ORBIT_recv_buffer)->cur;
      if (giop_msg_conversion_needed(GIOP_MESSAGE_BUFFER(_ORBIT_recv_buffer))) {
	 _ORBIT_curptr = ALIGN_ADDRESS(_ORBIT_curptr, 4);
	 ...
	 (*_ORBIT_retval)._buffer =
	    CORBA_sequence_GNOME_Spell_Language_allocbuf((*_ORBIT_retval).
							 _length);
	 ...
	 for (_ORBIT_tmpvar_5 = 0; _ORBIT_tmpvar_5 < (*_ORBIT_retval)._length;
	    ...
	    (*((guint32 *) & (_ORBIT_tmpvar_9))) =
	       GUINT32_SWAP_LE_BE(*((guint32 *) _ORBIT_curptr));
	    _ORBIT_curptr += 4;
	    (*_ORBIT_retval)._buffer[_ORBIT_tmpvar_5].abrev =
	       CORBA_string_alloc(_ORBIT_tmpvar_9);
	    memcpy((*_ORBIT_retval)._buffer[_ORBIT_tmpvar_5].abrev,
		   _ORBIT_curptr,
		   sizeof((*_ORBIT_retval)._buffer[_ORBIT_tmpvar_5].
			  abrev[_ORBIT_tmpvar_8]) * _ORBIT_tmpvar_9);
	    _ORBIT_curptr +=
	       sizeof((*_ORBIT_retval)._buffer[_ORBIT_tmpvar_5].
		      abrev[_ORBIT_tmpvar_8]) * _ORBIT_tmpvar_9;
	 }
				   ...
				   CORBA_Environment * ev)
{
   ...
   if (_obj->servant && _obj->vepv && GNOME_Spell_Dictionary__classid) {
      ((POA_GNOME_Spell_Dictionary__epv *) _obj->
       vepv[GNOME_Spell_Dictionary__classid])->setLanguage(_obj->servant,
							   language, ev);
      return;
   }
   _cnx = ORBit_object_get_connection(_obj);
 _ORBIT_retry_request:
   ...
   {				/* marshalling */
	 ...
	 giop_send_request_buffer_use(_cnx, NULL, _ORBIT_request_id,
      ...
      if (!_ORBIT_send_buffer)
	 goto _ORBIT_system_exception;
      _ORBIT_tmpvar_1 = strlen(language) + 1;
