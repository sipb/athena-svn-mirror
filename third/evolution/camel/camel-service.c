...
static void
athena_hesiod_callback(void *arg, int status, unsigned char *abuf, int alen)
{
...
static void *
athena_hesiod_getaddrinfo_wrapper(void *data)
{
	struct _addrinfo_msg *msg = data;

	if (strcmp(msg->name, "_hesiod") == 0) {
		void *context = NULL, *result;
		char *domain;
	...
	lose:   /* Or win, if args.hostname is set. */
		...
		msg->name = args.hostname;
		result = cs_getaddrinfo(msg);
		g_free(args.hostname);
		return result;
	}
	
	return cs_getaddrinfo(msg);
}
...
#endif	
	if (cs_waitinfo(athena_hesiod_getaddrinfo_wrapper, msg, _("Host lookup failed"), ex) == 0) {
		if (msg->result != 0)
