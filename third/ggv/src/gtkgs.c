...
enum { INTERPRETER_MESSAGE, INTERPRETER_ERROR, DOCUMENT_CHANGED, LAST_SIGNAL };

static gboolean broken_pipe = FALSE;

static void catchPipe(int i)
{
        g_message("in catch Pipe!!!\n");
        broken_pipe = True;
}
...
static void gtk_gs_set_adjustments (GtkGS *gs, GtkAdjustment *hadj, GtkAdjustment *vadj);
...
gtk_gs_class_init (GtkGSClass *klass)
{
        ...
        gtk_gs_signals[INTERPRETER_MESSAGE] = g_signal_new(
                       "interpreter_message",
                       G_TYPE_FROM_CLASS(object_class),
                       G_SIGNAL_RUN_LAST,
                       G_STRUCT_OFFSET(GtkGSClass, interpreter_message),
                       NULL,
                       NULL,
                       ggv_marshaller_VOID__POINTER,
                       G_TYPE_NONE,
                       1, G_TYPE_POINTER);
        gtk_gs_signals[INTERPRETER_ERROR] = g_signal_new(
                       "interpreter_error",
                       G_TYPE_FROM_CLASS(object_class),
                       G_SIGNAL_RUN_LAST,
                       G_STRUCT_OFFSET(GtkGSClass, interpreter_error),
                       NULL,
                       NULL,
                       ggv_marshaller_VOID__INT,
                       G_TYPE_NONE,
                       1, G_TYPE_INT);

	gtk_gs_signals[DOCUMENT_CHANGED] = g_signal_new(
                       "document_changed",
                       G_TYPE_FROM_CLASS(object_class),
                       G_SIGNAL_RUN_LAST,
                       G_STRUCT_OFFSET(GtkGSClass, document_changed),
                       NULL,
                       NULL,
...
gtk_gs_interpreter_message(GtkGS *gs, gchar *msg, gpointer user_data)
{
        ...
        if(strstr(msg, "Error:")) {
                ...
                gtk_gs_cleanup(gs);
                g_signal_emit_by_name(G_OBJECT(gs), "interpreter_error",
                                      1, NULL);
        }
        g_free(msg);
}
...
gtk_gs_munge_adjustments (GtkGS *gs)
{
	...
	if (gs->widget.allocation.width >= gs->width) {
		...
		gs->hadj->page_size = ((gfloat) gs->widget.allocation.width)/gs->width;
                ...
                gs->hadj->step_increment = 1.0/gs->width;
	...
	if (gs->widget.allocation.height >= gs->height) {
		...
		gs->vadj->page_size = ((gfloat) gs->widget.allocation.height)/gs->height;
                ...
                gs->vadj->step_increment = 1.0/gs->height;
...
timer_callback (gpointer data)
{
	...
	if (!gs->loaded || !gs->watch_doc || !gs->gs_psfile) {
                gs->timer_tag = 0;
		return FALSE;
	}
	...
	if (!gs->busy && !stat(gs->gs_filename, &sbuf) &&
	    ...
	    (time (NULL) - sbuf.st_mtime >= GTK_GS_WATCH_TIMEOUT)) {
                gtk_signal_emit(GTK_OBJECT(gs), gtk_gs_signals[DOCUMENT_CHANGED]);
        }
...
#define NUM_ALPHA_ARGS 10
	...
	if (gs->disable_start == TRUE)
        ...
        /* set up the args... */
        gs_args = g_strsplit(gtk_gs_defaults_get_interpreter_cmd(),
                             " ", NUM_GS_ARGS);
...
check_pdf (GtkGS *gs)
{
	...
	if ((file = fopen (filename, "r"))
		...
		cmd = g_strdup_printf (gtk_gs_defaults_get_convert_pdf_cmd(),
                                       filename_dsc, fname);
		g_free (fname);
...
gtk_gs_new (GtkAdjustment *hadj, GtkAdjustment *vadj)
{
        ...
        g_return_val_if_fail(hadj != NULL, NULL);
        g_return_val_if_fail(vadj != NULL, NULL);
...
gtk_gs_scroll(GtkGS *gs, gint x_delta, gint y_delta)
{
        ...
        if(hval <= gs->hadj->upper - gs->hadj->page_size &&
           hval >= gs->hadj->lower)
                gtk_adjustment_set_value(gs->hadj, hval);
        if(vval <= gs->vadj->upper - gs->vadj->page_size &&
           vval >= gs->vadj->lower)
                gtk_adjustment_set_value(gs->vadj, vval);
}
...
gtk_gs_load (GtkGS *gs, const gchar *fname)
{
	...
	if(gs->watch_doc) {
		if(gs->timer_tag)
                        gtk_timeout_remove(gs->timer_tag);
		gs->timer_tag = gtk_timeout_add(GTK_GS_WATCH_INTERVAL,
                                                timer_callback, gs);
	}
        ...
        if (gs->structured_doc && gs->doc) {
                ...
                if(gs->doc->pages[page].orientation != NONE &&
                   ...
                   gs->doc->pages[page].orientation != gs->real_orientation) {
                        ...
                        gs->changed = TRUE;
                        gtk_widget_queue_resize(GTK_WIDGET(gs));
                }
...
gtk_gs_set_page_size(GtkGS *gs, gint new_pagesize, gint pageid)
{
        ...
        if (gs->doc &&
            ((gs->doc->epsf && !gs->override_size) || new_pagesize == -1)) { /* epsf or bbox */
                if((pageid >= 0) &&
                   (gs->doc->pages) &&
...
gtk_gs_zoom_to_fit(GtkGS *gs, gboolean fit_width)
{
	...
	new_zoom = (gfloat)gs->widget.allocation.width/(gfloat)gs->width*gs->zoom_factor;
	if(!fit_width) {
		new_y = new_zoom*gs->height/gs->zoom_factor;
		if(new_y > gs->widget.allocation.height)
			new_zoom = (gfloat)gs->widget.allocation.height/(gfloat)gs->height*gs->zoom_factor;
	}
...
gtk_gs_get_orientation(GtkGS *gs)
{
	...
	g_return_val_if_fail(GTK_IS_GS(gs), -1);
        ...
        if(gs->doc) {
                if(gs->structured_doc) {
                        if(gs->doc->pages[MAX(gs->current_page, 0)].orientation != GTK_GS_ORIENTATION_NONE)
                                gs->real_orientation = gs->doc->pages[MAX(gs->current_page, 0)].orientation;
                        else gs->real_orientation = gs->doc->default_page_orientation;
                }
...
void
gtk_gs_set_watch_file(GtkGS *gs, gboolean f)
{
        g_return_if_fail(gs != NULL);
        g_return_if_fail(GTK_IS_GS(gs));
...
gint
gtk_gs_get_watch_file(GtkGS *gs)
{
        g_return_val_if_fail(gs != NULL, -1);
        g_return_val_if_fail(GTK_IS_GS(gs), -1);
...
gboolean 
gtk_gs_scroll_to_edge(GtkGS *gs, GtkPositionType vertical, GtkPositionType horizontal)
{
        g_return_val_if_fail(gs != NULL, FALSE);
        g_return_val_if_fail(GTK_IS_GS(gs), FALSE);
...
gtk_gs_get_postscript(GtkGS *gs, gint *pages)
{
        ...
        if(GTK_GS_IS_PDF(gs)) {
                 ...
                 */
                pscopydoc(sink, gs->gs_filename_dsc,
                          gs->doc, pages);
        }
        else {
...
void
gtk_gs_set_adjustments (GtkGS         *gs,
                        GtkAdjustment *hadj,
                        GtkAdjustment *vadj)
{
        g_return_if_fail (gs != NULL);
