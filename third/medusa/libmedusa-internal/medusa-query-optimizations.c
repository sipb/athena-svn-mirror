...
static gboolean                     result_is_constant_query             (MedusaOptimizationResult *result);
                                                  ...
                                                  MedusaUnparsedCriteria *criteria)
{
        ...
        while ((optimization_list = g_list_find_custom (optimization_list,
                ...
                if (result_is_constant_query (result)) {
                        return result;
                }
        }
...
static gboolean                 
result_is_constant_query (MedusaOptimizationResult *result)
{
        return result->query_is_always_true || result->query_is_always_false;
}
...
optimization_result_new (void)
{
        ...
        optimization_result->syntax_error_found = FALSE;
        optimization_result->query_is_always_true = FALSE;
        optimization_result->query_is_always_false = FALSE;
...
replace_username_with_uid_criterion (MedusaUnparsedCriteria *criteria) 
{
        ...
        g_print ("replacing user name with uid\n");
        ...
        if (query_relation == NULL) {
                result->syntax_error_found = TRUE;
                return result;
        }
        ...
        if (medusa_username_to_uid (query_object,
                ...
                if (query_relation_is_negative (query_relation)) {
                        result->query_is_always_true = TRUE;
                }
                else {
                        result->query_is_always_false = TRUE;
                }
        }
...
replace_group_with_gid_criterion (MedusaUnparsedCriteria *criteria)
{
        ...
        result = optimization_result_new ();
        g_print ("replacing group name with gid\n");        
...
{
        return query_criterion_get_by_category (criteria,
                                                "modified") != NULL;
}
...
replace_modified_with_mtime_criteria (MedusaUnparsedCriteria *criteria)
 {
        ...
        g_print ("replacing modified with mtime\n");
