...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Mrmwcrw.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:42 $"
#endif
#endif
...
Pixel			fgint = -1 ;	/* foreground for pixmaps. -1 means
					   not set */
Pixel			bgint = -1 ;	/* background for pixmaps */
 ...
 */ 
		      ...
		      /* CR 7568: Assume parent is not NULL. */
...
/*
 * Fix for CR 3281 - check to see if the resource_name is XmNaccelerators.
 *                   If it is, run XtParseAcceleratorTable and assign the
 *                   output to val.
 */
...

long Urm__CW_EvaluateValOrOffset 
     ...
     switch(cvttype){
       ...
       case MrmRtypeIconImage:
            if(reptype == MrmRtypeInteger)
               sprintf(err_msg,
                         "Couldn't convert integer '%ld' to icon image",
                         (*val)) ;
	    else sprintf(err_msg,
			 "Couldn't convert to icon image");
...
switch ( reptype )
	...
	for (cnt=0; cnt<vec_count; cnt++)
	    {
	    cstr_table_dst[cnt] = (XmString) ((char *) cstr_table_dst + 
				    ((unsigned long) cstr_table_src[cnt] - 
				     (unsigned long) cstr_table_src)) ;
	    }
	...
	for (cnt=0; cnt<vec_count; cnt++)
	    {
	    char8_table_dst[cnt] = (String) ((char *) char8_table_dst + 
