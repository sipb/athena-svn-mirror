...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: TextOut.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:17:04 $"
#endif
#endif
/*
...
#define UNSET_COLUMNS -1       /* Flag indicating unset resource */

#define DEFAULT_ROWS 1         /* Resource default number of rows */
#define UNSET_ROWS -1          /* Flag indicating unset resource */
...
static void RefigureRowInfo() ;
static void RefigureColumnInfo() ;
...
static XtResource output_resources[] =
{
...

    {
      ...
      XmRImmediate, (XtPointer) UNSET_ROWS
    },
...
#endif /* _NO_PROTO */
{
    XSetClipMask(XtDisplay(tw), gc, None);
}
...
#endif /* _NO_PROTO */
{
  ...
  while (start < end && x <= rightedge) {
	...
	if ((int)widget->text.char_size == 1){
	  ...
	  for (i=0, num_bytes = mblen(block.ptr, (int)widget->text.char_size); 
    ...
    if (!ShouldWordWrap(data, widget) && PosToXY(widget, position, &x, &y)) {
      ...
      if (x <= data->leftmargin) {
        if (widget->text.edit_mode == XmSINGLE_LINE_EDIT) {
          if (position == widget->text.bottom_position)
	  position = MAX(position - data->columns/2, 0);
        } else {
          line_num = _XmTextGetTableIndex(widget, position);
          if (position == widget->text.bottom_position ||
              (line_num < widget->text.total_lines && 
               position == widget->text.line_table[line_num+1].start_pos - 1))
	  position = MAX(position - data->columns/2,
                         line_num ? widget->text.line_table[line_num].start_pos : 0);
        }
        (void)PosToXY(widget, position, &x, &y);
      }
        x += data->hoffset;
...
static void
#ifdef _NO_PROTO
RefigureColumnInfo( widget, data )
        XmTextWidget widget ;
        OutputData data ;
#else
RefigureColumnInfo(
...
#endif /* _NO_PROTO */
{
    ...
    {
      if (data->rows_set == UNSET_ROWS)
         rows = data->rows;
      else
         rows = data->rows_set;
    }

    if (data->columns_set == UNSET_COLUMNS)
    {
       if (widget->core.width != 0)
          *width = widget->core.width;
       else
          *width = (Dimension) ((data->columns * data->averagecharwidth) +
			        data->leftmargin + data->rightmargin);
    }
    else
    ...
    if (widget->text.source->data->length > 0 && data->resizewidth)  {
      XmTextPosition nextpos;
      LineTableExtra extra = NULL;
      Boolean past_end = False;
      int i;

      for (i = 0 ; i < widget->text.number_lines && !past_end ; i++) {
	past_end = !MeasureLine(widget, i, widget->text.line[i].start, 
				&nextpos, &extra);
	if (extra) {
    ...
    if (data->rows_set == UNSET_ROWS && widget->core.height != 0)
...
#endif /* _NO_PROTO */
{
   ...
   XmResetSaveGC(widget, data->save_gc);
   /*
...
#endif /* _NO_PROTO */
{
    ...
    Boolean columns_changed, rows_changed;
...
*/
   ...
   /* needed for query geometry if width changes */
    if (new_width != oldtw->core.width)
    {
       RefigureColumnInfo(newtw, newdata);
       data->columns_set = UNSET_COLUMNS;
    } 
    else
       data->columns_set = newdata->columns;

   /* needed for query geometry if height changes */
    if (new_height != oldtw->core.height)
    {
       RefigureRowInfo(newtw, newdata);
       data->rows_set = UNSET_ROWS;
    }
    else
       data->rows_set = newdata->rows;

    if (!(new_width != oldtw->core.width &&
	  new_height != oldtw->core.height)) {
       if (columns_changed || rows_changed || newsize) {
...
#endif /* _NO_PROTO */
{
    ...
    _XmTextInvalidate(widget, (XmTextPosition) widget->text.top_character,
		      (XmTextPosition) widget->text.top_character, LONG_MAX);
...
#endif /* _NO_PROTO */
{
    ...
    if (data->resizewidth && tw->core.width != *width) {
       TextFindNewWidth(tw, width);
       if (*width < data->minwidth) *width = data->minwidth;
    }

    if (data->resizeheight && tw->core.height != *height)
...
#endif /* _NO_PROTO */
{
  ...
  /* Don't grow in width if word wrap is on */
    ...
    */
    ...
    if (data->rows == UNSET_ROWS)
    {
       data->rows_set = UNSET_ROWS;
       data->rows = DEFAULT_ROWS;
    }
    else
