...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: LabelG.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:51 $"
#endif
#endif
/*
 ...
 */
   ...
   if (LabG__label(new_w) == NULL)
    ...
    /* CR 6267:  Supress highlight thickness before sizing also. */
...
#endif /* _NO_PROTO */
{
    ...
    XGetGeometry (XtDisplay(w), Pix(lw),
		  &root,	/* returned root window */
		  &x, &y,	/* returned x, y of pixmap */
		  &wid, &hei,	/* returned width, height of pixmap */
		  &border,	/* returned border width */
		  &d);          /* returned depth */
...
#endif /* _NO_PROTO */
{
   ...
   /* Prevent button cording from issuing a drag.  This is needed to prevent
    * lost implicit grabs for original button pressed.  Include Button2Mask
    * because this may be called using different translations.  The
    * mask isn't set on the button event invoking this action.
    */
    if (event->type == ButtonPress &&
        (event->xbutton.state & (Button1Mask | Button2Mask | Button3Mask|
