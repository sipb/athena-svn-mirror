...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: FileSB.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:49 $"
#endif
#endif
/*
...
static void FSBSetValuesHook() ;
static void FSBGetValuesHook() ;
static void GetTextWithDirText() ;
#else
static void ClassInitialize() ;
static void FilterFix( 
                        XmGeoMatrix geoSpec,
...
#ifdef CDE_FILESB
	/* set_values_hook	    */	(XtArgsFunc)FSBSetValuesHook,
...
static void
#ifdef _NO_PROTO
FSBSetValuesHook(w, args, num_args)
      Widget w;
      ArgList args;
      Cardinal *num_args;
#else
...
#endif
{
      ...
      for (i = 0; i < *num_args; i++)  {
	      if (strcmp(args[i].name, "pathMode") == 0) {
		      FS_PathMode(w) = (unsigned char)args[i].value;
	      }
	      else if (strcmp(args[i].name, "fileFilterStyle") == 0) {
		      FS_FileFilterStyle(w)=(unsigned char)args[i].value;
	      }
      }
}  /* end of FSBSetValuesHook() */
...
#endif
{
	...
	for (i = 0; i < *num_args; i++)  {
		...
		else if (strcmp(args[i].name, "dirSpec") == 0) {
  			if (FS_PathMode( w) == XmPATH_MODE_RELATIVE)  {
				XmStringFree(*(XmString *)args[i].value);
				GetTextWithDirText(w, (XmString *)args[i].value);
			}
		}
	}
...
/****************/
    ...
    if(    SB_Text(fsb)    )
    {   
        fname = XmTextFieldGetString( SB_Text(fsb)) ;
        if(    *fname == '/'    )
          {   
            dirfilename = fname ;
          } 
          ...
          { 
            ...
            dname = XmTextFieldGetString( FS_DirText( fsb)) ;
            dlen = strlen( dname) ;
        ...
        if(    FS_PathMode( fs)  ==  XmPATH_MODE_FULL   )
          ...
          {   
            ...
            if(    (fileStr == NULL)
              ...
              {   
                String dirStr = XmTextFieldGetString( FS_DirText( fs));
                unsigned dirLen = strlen( dirStr) ;

		/*  Check that the dirStr ends in a / */
		if (    dirLen  &&  (dirStr[dirLen-1] != '/')    )  {
                	valueString = XtMalloc( dirLen + strlen( fileStr) + 2) ;
                	strcpy( valueString, dirStr) ;
                	valueString[dirLen] = '/' ;
                	strcpy( &valueString[dirLen+1], fileStr) ;
		}
		else  {
...
#ifdef CDE_FILESB
	if ( FS_FileFilterStyle(fs) == XmFILTER_NONE &&
	     FS_PathMode(fs) == XmPATH_MODE_FULL ) {
#endif
	    ...
	    XmListDeselectAllItems( FS_DirList( fs));
#endif
    ...
    {   
        ...
        {   
            ...
            {   
                ...
                {   
#ifdef CDE_FILESB
		    /*  Always update the DirText field, in case the user edited it */
  		    if(    FS_PathMode( wid)  ==  XmPATH_MODE_RELATIVE    )  {
                        if ((dirString = _XmStringGetTextConcat(FS_Directory(fsb))) != NULL)
                        {   
                          XmTextFieldSetString( FS_DirText( fsb), dirString) ;
                          XmTextFieldSetCursorPosition( FS_DirText( fsb),
                               XmTextFieldGetLastPosition( FS_DirText( fsb))) ;
