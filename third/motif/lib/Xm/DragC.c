...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: DragC.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:47 $"
#endif
#endif
/*
...
#endif /* _NO_PROTO */
{
    ...
    if((XGetWindowProperty(dpy, win, atom, 0, 0, False, AnyPropertyType,
	...
	for (i = nchildren - 1; i >= 0; i--) {
	    if ((inf = GetClientWindow(dpy, children[i], atom)) != None) {
              XFree(children);
	      return inf;
            }
	}
	...
	((xrmTarget = XrmStringToQuark(targetName)) == xrmFailure)) ||
	  ...
	  *maxLengthRtn = 0;
	  ...
	  DragDropFinish(dc);
				    ...
				    "_MOTIF_CANCEL_DROP_EFFECT", False)) 
      {
