...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Region.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:56 $"
#endif
#endif
/*
...
#ifdef I18N_MSG
#define MESSAGE1	catgets(Xm_catd,MS_Region,MSG_RE_1,_XmMsgRegion_0000)
#else
#define MESSAGE1	_XmMsgRegion_0000
#endif
...
#ifdef _NO_PROTO
typedef void (*XmOverlapProc)() ;
typedef void (*XmNonOverlapProc)() ;
#else
...
void
#ifdef _NO_PROTO
_XmRegionSetGCRegion( dpy, gc, x_origin, y_origin, r )
    Display		*dpy ;
    GC			gc ;
    int			x_origin ;
    int			y_origin ;
...
miCoalesce( pReg, prevStart, curStart )
    XmRegion	pReg;	    	/* XmRegion to coalesce */
    long    	  	prevStart;  	/* Index of start of previous band */
    long    	  	curStart;   	/* Index of start of current band */
#else
miCoalesce(
    XmRegion	pReg,
    long    	  	prevStart,
...
miRegionOp( newReg, reg1, reg2, overlapFunc,  nonOverlap1Func, nonOverlap2Func )
    register XmRegion 	newReg;	    	    	/* Place to store result */
    XmRegion	  	reg1;	    	    	/* First region in operation */
    XmRegion	  	reg2;	    	    	/* 2d region in operation */
    XmOverlapProc  	overlapFunc;   		/* Function to call for over-
						 * lapping bands */
    XmNonOverlapProc  	nonOverlap1Func;	/* Function to call for non-
						 * overlapping bands in region
						 * 1 */
    XmNonOverlapProc  	nonOverlap2Func;	/* Function to call for non-
						 * overlapping bands in region
						 * 2 */
#else
miRegionOp(
...
void
#ifdef _NO_PROTO
_XmRegionIntersect( reg1, reg2, newReg )
    XmRegion 	  	reg1;
    XmRegion	  	reg2;          /* source regions     */
    XmRegion 		newReg;        /* dest XmRegion (overwritten) */
#else
_XmRegionIntersect(
    XmRegion 	  	reg1,
    XmRegion	  	reg2,
...
void
#ifdef _NO_PROTO
_XmRegionIntersectRectWithRegion( rect, source, dest )
    XRectangle		*rect;
    XmRegion		source;
    XmRegion		dest;	/* overwritten */
#else
_XmRegionIntersectRectWithRegion(
    XRectangle		*rect,
    XmRegion		source,
...
void
#ifdef _NO_PROTO
_XmRegionUnion( reg1, reg2, newReg )
    XmRegion 	  reg1;
    XmRegion	  reg2;             /* source regions     */
    XmRegion 	  newReg;           /* dest XmRegion (overwritten) */
#else
...
void
#ifdef _NO_PROTO
_XmRegionUnionRectWithRegion( rect, source, dest )
    XRectangle		*rect;
    XmRegion		source;
    XmRegion		dest;	/* overwritten */
#else
...
void
#ifdef _NO_PROTO
_XmRegionSubtract( regM, regS, regD )
    XmRegion 	  	regM ;
    XmRegion	  	regS ;          
    XmRegion		regD ;	/* overwritten */
#else
_XmRegionSubtract(
    XmRegion 	  	regM,
    XmRegion	  	regS,          
...
Compress( r, s, t, dx, xdir, grow )
    XmRegion		r ;
    XmRegion		s ;	/* scratch region */
    XmRegion		t ;	/* scratch region */
    unsigned		dx ;	/* delta */
...
static void
#ifdef _NO_PROTO
CreateLeftShadow( here, mask, segml, segmc, segmi )
    XmRegionBox		*here ;
    unsigned long	mask ;
    XSegment		**segml ;
    int			*segmc ;
...
static void
#ifdef _NO_PROTO
CreateRightShadow( here, mask, segml, segmc, segmi )
    XmRegionBox		*here ;
    unsigned long	mask ;
    XSegment		**segml ;
    int			*segmc ;
...
static void
#ifdef _NO_PROTO
CreateTopShadow( start_x, end_x, here, mask, segml, segmc, segmi )
    Position		start_x ;
    Position		end_x ;
    XmRegionBox		*here ;
    unsigned long	mask ;
...
static void
#ifdef _NO_PROTO
CreateBottomShadow( start_x, end_x, here, mask, segml, segmc, segmi )
    Position		start_x ;
    Position		end_x ;
    XmRegionBox		*here ;
    unsigned long	mask ;
...
void
#ifdef _NO_PROTO
_XmRegionDrawShadow( display, d, top_gc, bottom_gc, region,
		     border_thick, shadow_thick, shadow_type )
    Display		*display ;
    Drawable		d ;
    GC			top_gc ;
    GC			bottom_gc ;
