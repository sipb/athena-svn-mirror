...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: RowColumn.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:58 $"
#endif
#endif
/*
...
#endif /* _NO_PROTO */
{
   ...
   if (XmIsCascadeButtonGadget(w))
   {
          ...
          (IsBar(XtParent(w))))
      {
         if (mode == XmADD)
         {
            ...
            if (LabG_Mnemonic(c) != (KeySym) NULL)
            {
            ...
            XmIsPushButtonGadget(w))
   {
      ...
      if (mode == XmADD)
      {
         ...
         if (LabG_Mnemonic(l) != (KeySym) NULL)
         {
...
#endif /* _NO_PROTO */
{
   ...
   else
   {
      ...
      /* if the last cascade to post the submenu is breaking the link, clear
       * it from from the submenu's record.
       *
       * CR 5550 fix begin 
       */
      if (    ( m != NULL )
...
#endif /* _NO_PROTO */
{
   ...
   if (IsBar(rc) && !RC_IsArmed(rc) && !gadget)
   {
      ...
      * XmProcessTraversal.  (This might be better served by sending in a flag+       * to MenuArm() that avoids the call completely - hint, hint).  Then we
...
#endif /* _NO_PROTO */
{
   XmMenuState mst = _XmGetMenuState((Widget)rc);
	...
	/* release all grabs for mwm to move the focus properly */
	XtUngrabKeyboard(rc, CurrentTime);

	/* do this before popdown since ptr is ungrabbed there */
