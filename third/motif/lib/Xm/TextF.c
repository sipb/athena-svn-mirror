...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: TextF.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:59 $"
#endif
#endif
/*
...
#define DEFAULT_COLUMNS 20         /* Resource default number of columns */
#define UNSET_COLUMNS -1           /* Flag indicating columns resource */
...
static short GetColumnsFromWidth() ;
...
static XtResource resources[] =
{
...

    {
      ...
      XmRImmediate, (XtPointer) UNSET_COLUMNS
    },
...
#endif /* _NO_PROTO */
{
    ...
    if (TextF_Columns(tf) == UNSET_COLUMNS)
       (*value) = (XtArgVal) DEFAULT_COLUMNS;
    else
       (*value) = (XtArgVal) TextF_Columns(tf);
}
...
#endif /* _NO_PROTO */
{
    ...
    if (TextF_Columns(tf) != UNSET_COLUMNS)
       columns = TextF_Columns(tf);
    else
       columns = DEFAULT_COLUMNS;

    if (TextF_ResizeWidth(tf) &&
	columns < tf->text.string_length) {
	...
	*width = columns * tf->text.average_char_width +
...
static short
#ifdef _NO_PROTO
GetColumnsFromWidth( tf )
        XmTextFieldWidget tf ;
#else
GetColumnsFromWidth( XmTextFieldWidget tf )
#endif /* _NO_PROTO */
{

   return((short) ((XtWidth(tf) - (2 * (TextF_MarginWidth(tf) +
                                  tf->primitive.shadow_thickness +
                                  tf->primitive.highlight_thickness)))
                   / tf->text.average_char_width));
}
...
#endif /* _NO_PROTO */
{
  ...
  /*CR 6971 clearing a text field leaves data */
...
/* if positions aren't adjacent, return False */
   ...
   if (tf->text.max_char_size == 1) { /* data is char* and one-byte per char */
      if (isspace((int)(unsigned char)TextF_Value(tf)[pos1]) || 
	  isspace((int)(unsigned char)TextF_Value(tf)[pos2])) return True;
   } else {
...
#endif /* _NO_PROTO */
{
    ...
    if (tf->text.max_char_size == 1) {
       for (start = begin; start > 0; start--) {
          if (isspace((int)(unsigned char)TextF_Value(tf)[start - 1])) {
	     break;
          }
       }
       *left = start;
       ...
       for (end = begin; end <= tf->text.string_length; end++) {
          if (isspace((int)(unsigned char)TextF_Value(tf)[end])) {
             end++;
             break;
          }
       }
...
#endif /* _NO_PROTO */
{
    ...
    if (tf->text.max_char_size == 1) {
       if ((start > 0) && 
	   (isspace((int)(unsigned char)TextF_Value(tf)[start - 1]))) {
           for (; start > 0; start--) {
               if (!isspace((int)(unsigned char)TextF_Value(tf)[start - 1])) {
                  start--;
                  break;
    ...
    if(tf->text.max_char_size == 1) {
       ...
       */
       while (*right < tf->text.string_length &&
              isspace((int)(unsigned char)TextF_Value(tf)[(int)*right]))
             *right = *right + 1;
	...
	_XmIsKPKey(dpy, event->xkey.keycode, &keysym_return)) {
		...
		if(keysym_return != XK_KP_Enter) 
			insert_length = _XmTranslateKPKeySym(keysym_return,
					 insert_string, TEXT_MAX_INSERT_SIZE);	
    }
#endif
        ...
        if (pending_delete) {
           XmTextFieldSetSelection(w, TextF_CursorPosition(tf),
                 ...
                 TextF_CursorPosition(tf) == tf->text.string_length)
             {
                if (TextF_Columns(tf) != UNSET_COLUMNS)
                {
                   if (nextPos - cursorPos > TextF_Columns(tf))
                      AdjustText(tf, TextF_CursorPosition(tf)-1, True);
                }
                else
                {
                   short columns = GetColumnsFromWidth(tf);

                   if (nextPos - cursorPos > columns)
                      AdjustText(tf, TextF_CursorPosition(tf)-1, True);
...
#endif /* _NO_PROTO */
{
    ...
    if (cursorPos < tf->text.string_length) {
       if (tf->text.max_char_size == 1) {
         if (isspace((int)(unsigned char)TextF_Value(tf)[cursorPos]))
	    FindWord(tf, cursorPos, &dummy, &position);
         else
            FindNextWord(tf, &dummy, &position);
          if(isspace((int)(unsigned char)TextF_Value(tf)[position])){
...
#endif /* _NO_PROTO */
{
   ...
   */
   ...
   if (supports_locale_data)
      prim_select->target = targets[0] = CS_OF_LOCALE;
   else if (supports_CT)
...
#endif /* _NO_PROTO */
{
    ...
    if ( ( (tf->primitive.navigation_type == XmNONE )
             || ( (tf->primitive.navigation_type == XmTAB_GROUP)
                     && _XmShellIsExclusive(w) ) )
       && VerifyLeave(tf, event)) {
...
#endif /* _NO_PROTO */
{
    ...
    if (TextF_Columns(tf) <= 0 && TextF_Columns(tf) != UNSET_COLUMNS) {
...
#endif /* _NO_PROTO */
{
    ...
    if (tf->text.max_char_size > 1 && !have_font_set){
      ...
      * If LoadFontMetrics is called from SetValues and set
      ...
      *wcs_temp_str = (wchar_t)0L; /* terminate with a wchar_t NULL */
      ...
      if (tf->text.max_char_size == 1) { /* Need to store data as char* */
         ...
         ret_val = wcstombs(TextF_Value(tf), wcs_start_temp,
			    tf->text.size_allocd);
	 if (ret_val < 0) tf->text.value[0] = (char) NULL;
         TextF_WcValue(tf) = NULL;
...
#endif /* _NO_PROTO */
{
   ...
   /* width was not set by application */
    if (req_tf->core.width == 0)
       new_tf->core.width = width;
    else
      /* Setting width takes priority over column setting and
       * columns needs to be reset as a flag to SetValues for this
       * condition.
       */
       TextF_Columns(new_tf) = UNSET_COLUMNS;

   /* height was not set by application */
...
#endif /* _NO_PROTO */
{
    ...
    * the i-beam. */
    ...
    if (TextF_CursorPosition(new_tf) != TextF_CursorPosition(old_tf)) {
      ...
      /*
       * Reset old pixmaps (which may have been destroyed)
       * before turning off cursor, since we may be in a
       * nested SetValues.  We need to use the new widget
       * in case the cursor pixmaps have been regenerated
       * in an earlier SetValues in the nest.
       */
       if (old_tf->text.cursor != new_tf->text.cursor)
       {
	  old_tf->text.ibeam_off = new_tf->text.ibeam_off;
	  old_tf->text.cursor = new_tf->text.cursor;
	  old_tf->text.add_mode_cursor = new_tf->text.add_mode_cursor;
	  old_tf->text.image_clip = new_tf->text.image_clip;
       }
   ...
   CheckHasRect(new_tf);
    ...
    if (!(new_width != old_tf->core.width &&
	  ...
	  AdjustText(new_tf, 0, False);
         ...
         /* Determine whether width was set by application or by
          * computation.  UNSET_COLUMNS is used when the columns
          * were not explicitly set and the Width was set by the
          * application.
          */
	  if (new_width == old_tf->core.width &&
              TextF_Columns(new_tf) != UNSET_COLUMNS)
	     new_w->core.width = width;
             ...
             TextF_Columns(new_tf) = UNSET_COLUMNS; /* Reset Default */
	  ...
	  redisplay = True;
       ...
       TextF_Columns(new_tf) = UNSET_COLUMNS; /* Reset Default Condition */
    }
       ...
       if (XtSensitive(new_w)) {
         ...
         _XmTextFieldDrawInsertionPoint(new_tf, True);
          ...
          if (new_tf->text.has_focus) {
             ...
             new_tf->text.blink_on = True;
             _XmTextFieldDrawInsertionPoint(new_tf, True);
             /* CR 6459 Call the callback from SetValues - however,
                setting doit flag to false will have no effect. */
...
#endif /* _NO_PROTO */
{
   ...
   if (tf->text.string_length > 0) {
	 ...
	 if (ret_val < 0) temp_str[0] = (char) NULL;
	 return temp_str;
      }
   } else
      return(XtNewString(""));
