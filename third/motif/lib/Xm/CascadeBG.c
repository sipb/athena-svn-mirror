...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: CascadeBG.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:45 $"
#endif
#endif
/*
...
#ifdef I18N_MSG
#define WRONGPARENT	catgets(Xm_catd,MS_CButton,MSG_CB_1,_XmMsgCascadeB_0000)
...
#endif /* _NO_PROTO */
{
    ...
    XSaveContext( XtDisplay(mw), (XID) cb, extension_context,
    ...
    cb_extension->normal_GC = XtGetGC( (Widget) mw,valueMask,&values);

    valueMask = GCForeground | GCBackground | GCGraphicsExposures;
    values.foreground = select_pixel;
    values.background = select_pixel;
    values.graphics_exposures = False;
    cb_extension->arm_GC = XtGetGC( (Widget) mw,valueMask,&values);
}
...
#endif /* _NO_PROTO */
{
      ...
      (LabG_MenuType(cb) == XmMENU_OPTION))
   {
      if (XtIsRealized(XtParent(cb)))
      {
          ...
          XtVaGetValues((Widget)XmGetXmDisplay(XtDisplayOfObject((Widget)cb)), "enableEtchedInMenu", &etched_in, NULL);
	  _XmDrawShadows (XtDisplay (cb), XtWindow (XtParent(cb)),
			...
			cb->gadget.shadow_thickness,
                        (etched_in && (LabG_MenuType(cb) != XmMENU_OPTION)) ?
...
#endif /* _NO_PROTO */
{
...
#ifdef CDE_VISUAL	/* etched in menu button */
   {
	...
	if (etched_in) {
	    GetExtension((Widget) cb);
	    XFillRectangle (XtDisplay(cb), XtWindow(XtParent(cb)),
	      CBG_IsArmed(cb) ? cb_extension->arm_GC : cb_extension->normal_GC,
...
#endif /* _NO_PROTO */
{
...

   {
	 ...
	 ArmAndPost (cb, event);
      ...
      {
         /* CR 8172 - the following two lines were reversed.
