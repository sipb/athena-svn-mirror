...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: ToggleBG.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:17:07 $"
#endif
#endif
/*
...
#endif /* _NO_PROTO */
{
    ...
    XSaveContext( XtDisplay(mw), (XID) tb, extension_context,
    ...
    tb_extension->normal_GC = XtGetGC( (Widget) mw,valueMask,&values);
 
    valueMask = GCForeground | GCBackground | GCGraphicsExposures;
    values.foreground = select_pixel;
    values.background = select_pixel;
    values.graphics_exposures = False;
    tb_extension->arm_GC = XtGetGC( (Widget) mw,valueMask,&values);
}
...
#endif /* _NO_PROTO */
{
       ...
       LabG_MenuType(w) == XmMENU_POPUP)
   {
	 ...
	 TBG_Armed(w) = TRUE;
         {
         Boolean etched_in = False;
         XtVaGetValues((Widget)XmGetXmDisplay(XtDisplayOfObject((Widget)w)),
			 "enableEtchedInMenu", &etched_in, NULL);
         if (etched_in)
         {
             DrawToggleLabel(w);
...
#endif /* _NO_PROTO */
{
   ...
   {
   Boolean etched_in = False;
   XtVaGetValues((Widget)XmGetXmDisplay(XtDisplayOfObject((Widget)tb)), 
		"enableEtchedInMenu", &etched_in, NULL);
   if (etched_in) {
       DrawToggleLabel(tb);
       if (TBG_IndOn(tb))
...
been overridden and should be left alone. */
       ...
       XtVaGetValues((Widget)XmGetXmDisplay(XtDisplayOfObject((Widget) tw)), "enableToggleColor", 
           &toggle_color, NULL);
       if (toggle_color &&
	   (TBG_IndType(tw) == XmONE_OF_MANY) &&
...
static void
#ifdef _NO_PROTO
DrawRadio(w, x, y, length, fill)
    XmToggleButtonGadget w;
    int x, y, length;
    Boolean fill;
#else
DrawRadio(XmToggleButtonGadget w,
	      int x,
	      int y,
	      int length,
	  Boolean fill)
#endif /* _NO_PROTO */
{
	...
	XDrawArc(XtDisplay ((Widget) w),
	     XtWindow ((Widget) w),
	     IsOn(w) ? XmParentBottomShadowGC(w) : XmParentTopShadowGC(w),
	     x, y, length, length,
	     45 * 64,
	     180 * 64);
	XDrawArc(XtDisplay ((Widget) w),
	     XtWindow ((Widget) w),
	     IsOn(w) ? XmParentTopShadowGC(w) : XmParentBottomShadowGC(w),
	     x, y, length, length,
	     45 * 64,
	     -180 * 64);
	...
	XFillArc(XtDisplay ((Widget) w),
	     XtWindow ((Widget) w),
	     IsOn(w) ? TBG_SelectGC(w) : TBG_BackgroundGC(w),
...
DrawCheck(w, x, y, length)
    ...
    int x, y, length;
#else
DrawCheck(XmToggleButtonGadget w,
	      int x,
	      int y,
	      int length)
#endif /* _NO_PROTO */
{
    ...
    {
        GetExtension((Widget)w);
        XFillRectangle(XtDisplay(w), XtWindow(w), TBG_Armed(w)?
...
#endif /* _NO_PROTO */
{
  ...
  if ((TBG_IndType(w)) == XmN_OF_MANY)
  {
	...
	to a diamond shaped indicator of the same width and height */
	 ...
	 length = edge - shadow_thickness * 2;
         ...
         _XmDrawShadows(XtDisplay (w), XtWindow (w),
                    ...
                    ((IsOn(w)) ?
                               XmParentTopShadowGC(w) :
                               XmParentBottomShadowGC(w)),
                    x, y, edge, edge, shadow_thickness, XmSHADOW_OUT);
	 ...
	 if (IsOn(w) && toggle_visual)
	     DrawCheck(w, x + shadow_thickness, y + shadow_thickness,
  ...
  else
  {
   ...
   if ((!TBG_Visible(w)) && (!IsOn(w)))
   {
	  ...
	  XFillRectangle( XtDisplay ((Widget) w),
			 XtWindow ((Widget) w),
			 (etched_in && TBG_Armed(w)) ? tb_extension->arm_GC:
			     TBG_BackgroundGC(w),
			 x, y, edge, edge);
      }
#else
...
#ifdef CDE_VISUAL	/* etched in menu button */
      Boolean etched_in = False;
      XtVaGetValues((Widget)XmGetXmDisplay(XtDisplayOfObject((Widget)tb)), "enableEtchedInMenu", &etched_in, NULL);
      if (etched_in)
      {
...
#endif /* _NO_PROTO */
{
    ...
    {   
        ...
        XFillRectangle (XtDisplay(tb), XtWindow(tb), 
			TBG_Armed(tb)? tb_extension->arm_GC:
...
#ifdef CDE_VISUAL
        LabG_NormalGC(tb) = TBG_Armed(tb) ? tb_extension->arm_GC:
				TBG_BackgroundGC(tb);
#else
