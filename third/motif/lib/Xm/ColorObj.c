...
#ifdef HP_MOTIF
static Boolean _XmGetEnableCDEColorFactors() ;
static void DefaultPrimaryColorSetId() ;
static void DefaultSecondaryColorSetId() ;
static void DefaultUseTextColor() ;
#endif /* HP_MOTIF */
static void Destroy() ;
...
static XtResource resources[] = {
...
#ifdef HP_MOTIF
	XmRCallProc, (XtPointer) DefaultPrimaryColorSetId,
#else
	XmRImmediate,
	(XtPointer) 5,
#endif /* HP_MOTIF */
...
#ifdef HP_MOTIF
	XmRCallProc, (XtPointer) DefaultSecondaryColorSetId,
#else
	XmRImmediate,
	(XtPointer) 6,
#endif /* HP_MOTIF */
...
#ifdef HP_MOTIF
	XmRCallProc, (XtPointer) DefaultUseTextColor,
#else
	XmRImmediate,
	(XtPointer) True,
#endif /* HP_MOTIF */
    ...
    /* To avoid the string to bitmap converter warning for wmShellClassRec,
     * we'll use a dummy resource declaration that does "nothing".
     */
    {   XmNiconPixmap,
         XmCIconPixmap,
         XmRString,
...
#ifdef HP_MOTIF
/**********************************************************************/
/** _XmGetEnableCDEColorFactors() - if the resource,		     **/
/**     XmNenableCDEColorFactor exists, get it's value to determine  **/
/** 	default values.						     **/
/**********************************************************************/
/*ARGSUSED*/
static Boolean 
#ifdef _NO_PROTO
_XmGetEnableCDEColorFactors( wid )
        Widget wid ;
#else
_XmGetEnableCDEColorFactors( Widget wid )
#endif /* _NO_PROTO */
{
  XrmName names[2];
  XrmClass classes[2];
  ...
  names[0] = XrmStringToQuark("enableCDEColorFactors");
  names[1] = NULLQUARK;

  classes[0] = XrmStringToQuark("EnableCDEColorFactors");
  classes[1] = NULLQUARK;

  if(    XrmQGetResource( XtScreenDatabase( XtScreen(wid) ), names, classes,
                                                          &rep, &db_value)    )
   {
      Boolean bool_value;

      to_value.size = sizeof(Boolean);
      to_value.addr = (XPointer) &bool_value;

      if(    XtConvertAndStore( wid, XmRString, &db_value,
                                                    XmRBoolean, &to_value)    )
       {
         return( bool_value );
       }
...
/**********************************************************************/
/** DefaultPrimaryColorSetId() - set the default color set to 3 for  **/
/** 	OSF Motif compatability or 5 for CDE environment             **/
/**********************************************************************/
/*ARGSUSED*/
static void
#ifdef _NO_PROTO
DefaultPrimaryColorSetId( widget, offset, value )
        Widget widget ;
        int offset ;
        XrmValue *value ;
#else
DefaultPrimaryColorSetId(
        Widget widget,
        int offset,
        XrmValue *value )
#endif /* _NO_PROTO */
{
  static int primaryColorSetId;

  if( _XmGetEnableCDEColorFactors(widget) )
   {
     primaryColorSetId = 5;
   }
   else
  ...
  value->addr = (XPointer)&primaryColorSetId;
}

/**********************************************************************/
/** DefaultSecondaryColorSetId() - set the default color set to 3    **/
/** 	for OSF Motif compatability or 5 for CDE environment	     **/
/**********************************************************************/
/*ARGSUSED*/
static void
#ifdef _NO_PROTO
DefaultSecondaryColorSetId( widget, offset, value )
        Widget widget ;
        int offset ;
        XrmValue *value ;
#else
DefaultSecondaryColorSetId(
        Widget widget,
        int offset,
        XrmValue *value )
#endif /* _NO_PROTO */
{
  static int secondaryColorSetId;

  if( _XmGetEnableCDEColorFactors(widget) )
   {
     secondaryColorSetId = 6;
   }
   else
    {
      secondaryColorSetId = 4;
    }

  value->addr = (XPointer)&secondaryColorSetId;
}

/**********************************************************************/
/** DefaultUseTextColor() - set the default color set to 3 for       **/
/** 	OSF Motif compatability or 5 for CDE environment	     **/
/**********************************************************************/
/*ARGSUSED*/
static void
#ifdef _NO_PROTO
DefaultUseTextColor( widget, offset, value )
        Widget widget ;
        int offset ;
        XrmValue *value ;
#else
...
#endif /* _NO_PROTO */
{
  ...
  value->addr = (XPointer)&useTextColor;
}
#endif /* HP_MOTIF */
...
#endif /* _NO_PROTO */
{
	...
	/* Get screen number for this widget */
        screen = XtScreen(w);
        scr = XScreenNumberOfScreen(screen);

	/** if background didn't default to ColorObj, don't overwrite colors **/
	/** Use the correct color set for this screen, myColors is for the default screen only **/
	if (defaultBackground != tmpColorObj->color_obj.colors[scr][colorPrim].bg)
	    return;

        ac = 0;
	/* Don't use myColors, they're for the default screen only */
        XtSetArg (al[ac], XmNbackground, 
                  tmpColorObj->color_obj.colors[scr][color].bg);        ac++;
        XtSetArg (al[ac], XmNforeground, 
                  tmpColorObj->color_obj.colors[scr][color].fg);        ac++;
        XtSetArg (al[ac], XmNtopShadowColor, 
                  tmpColorObj->color_obj.colors[scr][color].ts);        ac++;
        XtSetArg (al[ac], XmNbottomShadowColor, 
                  tmpColorObj->color_obj.colors[scr][color].bs);        ac++;

        /** put dithers for top shadows if needed **/
	/* Use the correct screen, instead of myScreen */
        if (DitherTopShadow(tmpColorObj->color_obj.display, 
			    scr,
                            &tmpColorObj->color_obj.colors[scr][color]))
        {
            if (mono)
                ...
                ditherPix = XmGetPixmap (screen, "50_foreground",
                            tmpColorObj->color_obj.colors[scr][color].bg,
                                         WhitePixelOfScreen(screen));
        ...
        if (DitherTopShadow(tmpColorObj->color_obj.display, 
			    scr, 
                            &tmpColorObj->color_obj.colors[scr][colorPrim]))
...
#endif /* _NO_PROTO */
{
    ...
    cacheRec.screen = ScreenOfDisplay(tmpColorObj->color_obj.display, screen);
    ...
    /** R5 update **/
    db = XtScreenDatabase(
	    ScreenOfDisplay(tmpColorObj->color_obj.display, screen));
...
#ifdef HP_MOTIF
    if (_XmGetEnableCDEColorFactors((Widget)tmpColorObj))
    {
#endif /* HP_MOTIF */
