...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: CutPaste.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:46 $"
#endif
#endif
/*
	 ...
	 */
	    ...
	    next_length = MIN( length - length_sent,
            ...
            */
            XChangeProperty( display,
                             ...
                             (int) (next_length / BYTELENGTH(1,format)) );
        ...
        */
        XChangeProperty( display,
                         ...
                         PropModeReplace,
                         (unsigned char *) value,
                         (int) (length / BYTELENGTH(1,format)) );
...
#endif /* _NO_PROTO */
{
	...
	*/
	...
	/* allocate storage for list of target atoms 
	   plus the necessary  CR 9526 */
	ptr = (Atom *)_XmClipboardAlloc((count + 2) * sizeof(Atom) );
	save_ptr = ptr;
	...
	*length = (n - 1) * sizeof(Atom);
	...
	timestamp = (timeStamp *)_XmClipboardAlloc(sizeof(timeStamp));
...
#endif /* _NO_PROTO */
{
    Window rootwindow;
    Atom formatatom;
    long new_length = (long)format_length;
    int stored_len;
...
#endif /* _NO_PROTO */
{
    ...
    */
    ...
    /* length is passed in bytes. Convert to count of elements for 
     * XChangeProperty by dividing by the byte length of one element.
     */
...
#endif /* _NO_PROTO */
{
	...
	case 1: item = "_MOTIF_CLIP_NEXT_ID";
	    break;
	default:
    	    sprintf( atomname, "_MOTIF_CLIP_ITEM_%d", (int)itemid );
    	    item = atomname;
	    break;	
    }
...
#endif /* _NO_PROTO */
{
    ...
    */
    if ( formatptr->windowId == window )
    {
    	...
    	*/
    	_XmClipboardEventHandler( XtWindowToWidget(display,
...
#endif /* _NO_PROTO */
{
    ...
    */
    ...
    if ( itemheader->cutByNameWindow != 0L )
    {
	...
	*/
	    ...
	    */
	    for( i = 0; i < loc_count; i++ )
	    {
		    ...
		    if ((*atomptr) != (Atom)0)
		    {
  		        str = XmGetAtomName( display, *atomptr );
			temp = strlen(str);
			XFree(str);
	...
	*/
	if ( matchformat != 0 )
	{
	    if ( CONVERT_32_FACTOR != 1 )
	      {
		  _XmClipboardGetLenFromFormat( display,
					       format,
					       &format_len );
