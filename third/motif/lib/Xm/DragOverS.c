...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: DragOverS.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:48 $"
#endif
#endif
/*
...
extern	Boolean	_XmTextualDragFlag;
...
#endif /* _NO_PROTO */
{
	...
	if (cache_ptr->depth == depth &&
	    ...
	    cache_ptr->sourceX == sourceX &&
	    cache_ptr->sourceY == sourceY &&
            ((cache_ptr->statePixmap == None && !stateIcon) ||
             (stateIcon && cache_ptr->statePixmap == stateIcon->drag.pixmap &&
			   cache_ptr->stateMask == stateIcon->drag.mask &&
	    		   cache_ptr->stateX == stateX &&
	    		   cache_ptr->stateY == stateY)) &&
            ((cache_ptr->opPixmap == None && !opIcon) ||
 ...
 *  If this is a Textual Drag, use the offsets specified.
 *
 ***********************************************************************/

static void
#ifdef _NO_PROTO
GetIconPosition( dos, icon, sourceIcon, iconX, iconY, offsetX, offsetY)
    XmDragOverShellWidget	dos;
    XmDragIconObject		icon;
    XmDragIconObject		sourceIcon;
    Position			*iconX;
