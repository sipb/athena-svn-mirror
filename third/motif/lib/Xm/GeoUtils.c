...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: GeoUtils.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:50 $"
#endif
#endif
/*
...
#define MESSAGE1 _XmMsgGeoUtils_0000
...
XtGeometryResult 
#ifdef _NO_PROTO
_XmHandleQueryGeometry(widget, intended, desired, resize_policy, 
		       createMatrix )
        Widget widget ;
...
XtGeometryResult 
#ifdef _NO_PROTO
_XmHandleGeometryManager( wid, instigator, desired, allowed, policy, cachePtr, createMatrix )
        Widget wid ;
        Widget instigator ;
        XtWidgetGeometry *desired ;
    ...
    {   
        ...
        {   
            ...
            *cachePtr = geoSpec ;
            ...
            else /* 7142: change For fixing OSF CR 5956 */ {
		allowed = NULL ;
		*cachePtr = NULL ;
		result = XtGeometryNo ;
	    }
...
#endif /* _NO_PROTO */              /*   XmGET_ACTUAL_SIZE. */
{
...
static void 
#ifdef _NO_PROTO
_XmGeoCalcFill( fillSpace, margin, numBoxes, endSpec, betweenSpec, pEndSpace, pBetweenSpace )
        Dimension fillSpace ;
        Dimension margin ;
        unsigned int numBoxes ;
        Dimension endSpec ;
...
static void 
#ifdef _NO_PROTO
SegmentFill( rowBoxes, numBoxes, layoutPtr, x, width, marginW, endX, maxX, endSpace, betweenSpace )
        XmKidGeometry rowBoxes ;
        unsigned int numBoxes ;
        XmGeoRowLayout layoutPtr ;
        Position x ;
...
static Position 
#ifdef _NO_PROTO
_XmGeoLayoutWrap( rowPtr, layoutPtr, x, y, endSpace, betweenSpace, maxX, width, marginW )
        XmKidGeometry rowPtr ;
        XmGeoRowLayout layoutPtr ;
        Position x ;
        Position y ;
...
void
#ifdef _NO_PROTO
_XmMenuBarFix( geoSpec, action, layoutPtr, rowPtr )
        XmGeoMatrix geoSpec ;
        int action ;
        XmGeoMajorLayout layoutPtr ;
        XmKidGeometry rowPtr ;
...
XmKidGeometry 
#ifdef _NO_PROTO
_XmGetKidGeo( wid, instigator, request, uniform_border, border, uniform_width_margins, uniform_height_margins, help, geo_type )
        Widget wid ;
        Widget instigator ;
        XtWidgetGeometry *request ;
        int uniform_border ;
...
/****************/
    if(!geoA){ /* For fixing OSF CR 5956 */
         _XmWarning( wid, "Invalid order found in XmSelectionBox"); 
         return(False);
    }
...
#ifdef I18N_MSG
	  _XmWarning( w, catgets(Xm_catd,MS_GeoUtil,MSG_GEO_1,
		MESSAGE1)) ;
#else
	  _XmWarning( w, MESSAGE1) ;
#endif
