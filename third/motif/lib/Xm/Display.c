...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Display.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:46 $"
#endif
#endif
/*
...
static XtResource cde_resources[] = {
  ...
  { "enableThinThickness", "EnableThinThickness", XmRBoolean, sizeof(Boolean),
    XtOffsetOf(struct _CDEDisplay_InstanceExtRec, enable_thin_thickness),
    XmRImmediate, (XtPointer) False
  },
};
...
MSG_DSP_1, MESSAGE1)); 
	...
	for (i = 0; i < num_screens; i++)  {
	    ...
	    new_db = XrmGetStringDatabase(_CDEBaseTranslationsOverride);
...
#endif
{
	...
	for (i = 0; i < *num_args; i++)  {
		...
		if (strcmp(args[i].name, "enableThinThickness") == 0) {
			*(Boolean *)args[i].value = rec_cache->enable_thin_thickness;
		}
	}
