...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: ResInd.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:58 $"
#endif
#endif
/*
 ...
 * to default the shadow and highlight thickness resource.  If
 * enablethinThickness is true (CDE apps), the value is "1 pixel",
 * else defaults to "2".  
 **********************************************************************/
void 
#ifdef _NO_PROTO
_XmSetThickness( widget, offset, value )
        Widget widget ;
        int offset ;
        XrmValue *value ;
#else
...
#endif /* _NO_PROTO */
{
	...
	Boolean thinthickness;

	XtVaGetValues((Widget)XmGetXmDisplay(XtDisplay(widget)), "enableThinThickness", &thinthickness, NULL);
...
void 
#ifdef _NO_PROTO
_XmSetThicknessDefault0( widget, offset, value )
        Widget widget ;
        int offset ;
        XrmValue *value ;
#else
