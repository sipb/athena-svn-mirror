...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Visual.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:17:11 $"
#endif
#endif
/*
...
#ifdef CDE_COLOR_CALC
static int xm_intensity_factor = 75;
static int xm_luminosity_factor = 25;
static int xm_color_lite_bs_factor = 40;
static int xm_color_hi_bs_factor = 40;
static int xm_color_hi_ts_factor = 60;
static int xm_color_lo_ts_factor = 50;
static int xm_default_dark_threshold = 20;
static int xm_default_light_threshold = 90;
static int xm_default_foreground_threshold = 70;
#endif /* CDE_COLOR_CALC */
  ...
  */
...
#ifdef CDE_COLOR_CALC
  {
  Boolean useCDEcolors = False; /* default to use 1.2 colors */
  ...
  if (XrmQGetResource(XtScreenDatabase(screen), names, classes,
		      &rep, &db_value))  
    {
      ...
      to_value.addr = (XPointer) &useCDEcolors;

      XtConvertAndStore( &widget, XmRString, &db_value, XmRBoolean, &to_value);
    }
  if (!useCDEcolors)
    {
      /* set the global values to the old 1.2 defined values. */
      xm_intensity_factor = XmINTENSITY_FACTOR;
      xm_luminosity_factor = XmLUMINOSITY_FACTOR;
      xm_color_lite_bs_factor = XmCOLOR_LITE_BS_FACTOR;
      xm_color_hi_bs_factor = XmCOLOR_HI_BS_FACTOR;
      xm_color_hi_ts_factor = XmCOLOR_HI_TS_FACTOR;
      xm_color_lo_ts_factor = XmCOLOR_LO_TS_FACTOR;
      xm_default_dark_threshold = XmDEFAULT_DARK_THRESHOLD;
      xm_default_light_threshold = XmDEFAULT_LIGHT_THRESHOLD;
      xm_default_foreground_threshold = XmDEFAULT_FOREGROUND_THRESHOLD;
    }
  }
#endif /* CDE_COLOR_CALC */
  ...
  if (XrmQGetResource(XtScreenDatabase(screen), names, classes,
	...
	if ( (default_light_threshold_spec >= 0) && 
	  ...
	  ;
#ifdef CDE_COLOR_CALC
	else default_light_threshold_spec = xm_default_light_threshold;
      }
      else default_light_threshold_spec = xm_default_light_threshold;
    }
  else default_light_threshold_spec = xm_default_light_threshold; 
  ...
  if (XrmQGetResource(XtScreenDatabase(screen), names, classes,
	...
	if ( (default_dark_threshold_spec >= 0) && 
	  ...
	  ;
#ifdef CDE_COLOR_CALC
	else default_dark_threshold_spec = xm_default_dark_threshold;
       }
       else default_dark_threshold_spec = xm_default_dark_threshold;
    }
  else default_dark_threshold_spec = xm_default_dark_threshold;
  ...
  if (XrmQGetResource(XtScreenDatabase(screen), names, classes,
	...
	if ( (default_foreground_threshold_spec >= 0) && 
	  ...
	  ;
#ifdef CDE_COLOR_CALC
	else default_foreground_threshold_spec = xm_default_foreground_threshold;
      }
      else default_foreground_threshold_spec = xm_default_foreground_threshold;
    }
  else default_foreground_threshold_spec = xm_default_foreground_threshold;
...
#ifdef CDE_COLOR_CALC
	brightness = ( (intensity * xm_intensity_factor) +
				   (light * XmLIGHT_FACTOR) +
				   (luminosity * xm_luminosity_factor) ) / 100;
#else
 ...
 */
	...
	{
#ifdef CDE_COLOR_CALC
		int bs_factor = xm_color_lite_bs_factor;
#else
		int bs_factor = XmCOLOR_LITE_BS_FACTOR;
#endif /* CDE_COLOR_CALC */

		color_value = bg_color->red;
		color_value -= (color_value * bs_factor) / 100;
...
#endif /* _NO_PROTO */
{
	...
	if (bs_color)
	{
		...
		f = XmCOLOR_LO_BS_FACTOR + (brightness 
			* ( xm_color_hi_bs_factor - XmCOLOR_LO_BS_FACTOR )
			/ XmMAX_SHORT);
#else
		f = XmCOLOR_LO_BS_FACTOR + (brightness 
	...
	{
#ifdef CDE_COLOR_CALC
		f = xm_color_lo_ts_factor + (brightness
			* ( xm_color_hi_ts_factor - xm_color_lo_ts_factor )
			/ XmMAX_SHORT);
#else
