...
#pragma ident "@(#)regexp.c	1.4 94/03/04 SMI"

/*
 * _sun_regcomp and _sun_regexec
 *
 ...
 * that _sun_regcomp() supplies a regmust only if the r.e. contains something
 * potentially expensive (at present, the only such thing detected is * or +
 * at the start of the r.e., which can involve a lot of backup).  Regmlen is
 * supplied because the test in _sun_regexec() needs it and _sun_regcomp() is computing
 * it anyway.
 */
...
/*
 * Global work variables for _sun_regcomp().
 */
...
/*
 * Forward declarations for _sun_regcomp()'s friends.
 */
...
/*
 - _sun_regcomp - compile a regular expression into internal code
 *
...
_sun_regexp *
_sun_regcomp(char *exp)
{
	register _sun_regexp *r;
	register char	*scan;
	...
	/* Allocate space. */
	r = (_sun_regexp * )malloc(sizeof(_sun_regexp) + (unsigned)regsize);
	if (r == NULL)
		FAIL("out of space");
...
/*
 * _sun_regexec and friends
 */

/*
 * Global work variables for _sun_regexec().
 */
 ...
 * Forwards.
 */
static int regtry(_sun_regexp *prog, char *);
static int regmatch(char *prog);
static int regrepeat(char *p);

/*
 - _sun_regexec - match a regexp against a string
 */
int
_sun_regexec(_sun_regexp *prog, char *string)
{
	register char	*s;
	extern char	*strchr();
...
regtry(_sun_regexp *prog, char *string)
{
	register int	i;
	register char	**sp;
	register char	**ep;
