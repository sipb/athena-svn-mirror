...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: List.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:52 $"
#endif
#endif
/*
...
static void ClearList(XmListWidget lw);
...
#endif /* _NO_PROTO */
{
    ...
    XmListWidget lw;
    KeySym cur_ksm;
    static KeySym ksm_osfCancel = NULL;

    /* fix to restore proper ScrolledList behavior on cancel drag event*/
    if (lcd->reason == XmCR_VALUE_CHANGED) {
        if (!ksm_osfCancel)
                ksm_osfCancel = XStringToKeysym("osfCancel");

        if(lcd->event && lcd->event->type == KeyPress){
            cur_ksm = XtGetActionKeysym(lcd->event, NULL);
            if ( cur_ksm==ksm_osfCancel )
                /* the drag cancel operation, so DrawList to back the change */
                ;
            else {
                /* It is not osfCancel so not the drag cancel operatio,
                 * let us updateHighlight only
                 */
                UpdateHighlight(w,NULL,lcd);
                return;
            }
        }
        else {
            /* Not the drag cancel operation, so normal action */
            UpdateHighlight(w,NULL,lcd);
            return;
        }
    }
 ...
 ****************/
    ...
    {
        lw->list.InternalList = (ElementPtr *)XtMalloc((sizeof(Element *) * 
			        lw->list.itemCount));
        for (i = 0; i < lw->list.itemCount; i++)
	    AddInternalElement(lw, lw->list.items[i], 0,
			       OnSelectedList(lw,lw->list.items[i]), FALSE);

      /* CR 9375: Preserve initial selectedItems a little longer. */
      if (!lw->list.selectedItemCount)
	{
	/* CR 5560 : Before building selected list, clear selected list to 
	   avoid  memory leak.  */
...
/*	BuildSelectedList(newlw);*/
	 ...
	 * fix for bug # 5300
	 */
    if (!newlw->list.FromSetNewSize && (newlw->list.SizePolicy != XmVARIABLE))
    {
...
#endif /* _NO_PROTO */
{
   ...
   if (lw->list.items && lw->list.itemCount)
   {
	...
	lw->list.BaseY = BaseY;
        ...
        if (all)
	    /* clear the list so there are no stray pixels hanging around */
	    ClearList(lw);
...
/***************************************************************************
 *                                                                         *
 * SortList ( void *base, size_t n_mem, size_t size,                       *
 *            int (*compare)(XmConst void *, XmConst void *))              *
 *                                                                         *
 * Performs a bubble sort on the list for use in deleing internal items.   *
 * This procedure takes the same parameters as qsor and could be replaced  *
 * with qsort.                                                             *
 ***************************************************************************/

static void
#ifdef _NO_PROTO
SortList( base, n_mem, size, compare)
char * base;
size_t n_mem;
size_t size;
int (*compare)();
...
#endif /* _NO_PROTO */
{
  ...
  /* Currently this routine is only used to sort
   * list in DeleteInternalElementPositions, */
  /* which are just integer pointers. */
...
/***************************************************************************
 *                                                                         *
 * ReverseListCompare ( XmConst void *A. XmConst void *B )                 *
 *                                                                         *
 * This function is used with SortList to reorder the list of items to be  *
 * deleted in reverse order so that the list does not become stale.        *
 ***************************************************************************/
static int
#ifdef _NO_PROTO
ReverseListCompare ( A, B )
        XmConst void *A;
        XmConst void *B;
#else
ReverseListCompare (
...
*/
    p_list = (int *) XmStackAlloc(position_count, pos_list);
    memcpy(p_list, position_list, position_count * sizeof(int));

   /* put list into a last item first order to prevent stale list deletes */
    SortList(p_list, position_count, sizeof(int), ReverseListCompare);

   /* used to check multiple deletes of same item */
    last_item_pos = -1;

    for (ix = 0; ix < position_count; ix++)
    { 
        item_pos = p_list[ ix ] - 1;
        if ( item_pos >= 0 && item_pos < oldItemCount &&
             item_pos != last_item_pos) /* Do not delete item already deleted */
	{
...
/* END OSF Fix CR 4656 */
    ...
    XmStackFree((char *)p_list, pos_list);
}
...
#endif /* _NO_PROTO */
{
    ...
    for (i = 0; i < item_count; i++)
    {
		...
		/*
		 * fix for bug# 5331
		 * change < to <= as item_pos starts with 1 and not 0
		 */
...
#endif /* _NO_PROTO */
{
    ...
    /* the max font height might have changed */
    ResetHeight(lw);
...
static void ListSetValuesAlmost(old, new, request, reply)
Widget              old;
Widget              new;
XtWidgetGeometry    *request;
XtWidgetGeometry    *reply;
{
                                        ...
                                        request->height != lw->core.height) ) )
    {
         ...
         * set visible count and redraw list if different
         */
        viz = SetVizCount(lw);
        ...
        /*
         * coreProc
	 * call Superclass method -- if we subclass from List and this function
	 * is inherited a change should be made to prevent this from being
	 * called twice
         */
            superclass = (WidgetClass)XtSuperclass(new);
            superclass->core_class.set_values_almost(old, new, request, reply) ;
