...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: UilSymStor.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:39 $"
#endif
#endif
...
#else
Uil_sym_cleanup_storage (freealloc)
  boolean freealloc;
#endif
{
if ( freealloc )
...
#else
sym_make_forward_ref ( az_id_frame, l_widget_type, a_location)
...
#else
sym_make_value_forward_ref (az_value_frame, a_location, fwd_ref_flags)
...
#else
sym_dump_symbol (az_symbol_entry)
...
#else
sym_dump_widget (az_widget_entry)
...
#else
sym_dump_argument (az_argument_entry)
...
#else
sym_dump_control (az_control_entry)
...
#else
sym_dump_callback(az_callback_entry)
...
#else
sym_dump_module(az_module_entry)
...
#else
sym_dump_color_item(az_color_item_entry)
...
#else
sym_dump_parent_list_item (az_parent_list_item)
...
void
#ifndef _NO_PROTO
sym_dump_external_def(XmConst sym_external_def_entry_type *az_external_def_entry)
#else
sym_dump_external_def(az_external_def_entry)
...
#else
sym_dump_proc_def(az_proc_def_entry)
...
#else
sym_dump_proc_ref(az_proc_ref_entry)
...
#else
sym_dump_forward_ref(az_forward_ref_entry)
...
void
#ifndef _NO_PROTO
output_text(XmConst int   length, 
            XmConst char *text)
#else
output_text(length, text)
  XmConst int   length;
  XmConst char *text;
#endif
{
...
#else
sym_dump_obj_header (az_obj_entry)
...
sym_dump_include_file (sym_include_file_entry_type *az_symbol_entry)
#else
sym_dump_include_file (az_symbol_entry)
...
sym_dump_section (sym_section_entry_type *az_symbol_entry)
#else
sym_dump_section (az_symbol_entry)
...
sym_dump_object_variant (sym_def_obj_entry_type *az_symbol_entry)
#else
sym_dump_object_variant (az_symbol_entry)
...
sym_dump_root_entry (sym_root_entry_type *az_symbol_entry)
#else
sym_dump_root_entry (az_symbol_entry)
