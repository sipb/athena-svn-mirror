...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: UilP2Out.c,v $ $Revision: 1.1.1.1 $ $Date: 1999-01-30 03:16:37 $"
#endif
#endif
...
/*
 * out_k_sym_entries is used to define the size of the out_queue_type that is
 * overlayed on the src_source_buffer_type
 */
#define out_k_sym_entries ((sizeof( src_source_buffer_type) \
			    - sizeof( struct _out_queue_type  *)) \
			   / sizeof( sym_entry_type *) )

typedef	struct	_out_queue_type
{
    struct _out_queue_type  *az_prior_queue;
    sym_entry_type	    *entry[out_k_sym_entries];
} out_queue_type;
...

{
    ...
    */
    ...
    if (out_l_next_offset >= out_k_sym_entries)
    {
...

{
	...
	out_l_next_offset = out_k_sym_entries - 1;
    }
...
#else
    text_offset = (unsigned long)&extern_arg_compr->entry[arg_value_count]
	- (unsigned long)extern_arg_compr;
#endif
...
#else
    text_offset = (unsigned long)&extern_class_compr->entry[class_value_count]
	- (unsigned long)extern_class_compr;
#endif
    comp_code = UilMrmMinValidCode;
    for ( i = 0;
	i <= uil_max_object;
