...
namespace aspeller {
  ...
  struct EditDistanceWeights {
    ...
    int del2;    // the cost of inserting a character or deleting a char
                    // in the next string
    int swap;    // the cost of a swapping two adjesent letters
    int sub;     // the cost of a replasing one letter with another
    int similar; // the cost of a "similar" but not exact match for
                    // two characters
    int min;     // the max of del1, del2, swap and sub.
    int max;     // the min of del1, del2, swap and sub.
    EditDistanceWeights()
