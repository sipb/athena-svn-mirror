 ...
 * Revision 1.15  1999/03/11 21:46:09  kenh
 * Don't allow requests from people who have REQUIRES_HW_AUTH set.
 *
 * Revision 1.14  1998/03/17 20:26:03  kenh
 * Fixes from David Thompson for longer lifetimes and for Solaris/x86
 *
 * Revision 1.13  1998/02/25 19:35:08  vwelch
 * Added -l <facility> option to fakeka to allow direction of syslog.
 *
 * Revision 1.12  1998/01/08 19:36:44  vwelch
 * Modified fakeka to screen out null principals (""."") since they
 * were causing fakeka to crash under HPUX_10.20 deep down in the krb5
 * library.
 *
 * Revision 1.11  1997/08/25 14:41:07  vwelch
 * Tweated string returned for malformed request to indicate it probably was
 * caused by a bad password.
 *
 * Revision 1.10  1997/08/14 21:28:16  kenh
 * Check to see if the principal has been disabled before issuing a ticket
 * for it.
 *
 * Revision 1.9  1997/08/14 19:09:58  kenh
 * Changes from Von Welch to support the case where the AFS cell name !=
 * Kerberos realm name.
 *
 * Revision 1.8  1997/04/10 15:13:20  kenh
 * Add stuff to make emt/adm work
 *
 * Revision 1.7  1997/02/04 05:58:15  kenh
 * Cleanup & garbage collection
 *
 * Revision 1.6  1997/02/03 21:30:11  kenh
 * Initialize the des425 random number generator
 *
 * Revision 1.5  1997/02/03 19:07:53  kenh
 * Compiles now, at least
 *
 * Revision 1.4  1997/01/31 23:56:34  kenh
 * Initial import into my source tree
 *
 * Revision 1.3  1996/08/09  00:17:01  jhutz
 * 	Added the '-m' switch, as in the Kerberos KDC.  If this switch
 * 	is given, we prompt for the master key instead of reading it from
 * 	a file.  If startup is successful, we then fork and insure the
 * 	parent exits with a status of 0.
 * 	[1996/08/03  02:04:16  jhutz]
 *
 * Revision 1.2  1995/02/23  18:26:29  chs
 * 	Created.
 * 	[1995/02/23  18:26:01  chs]
 * 
 * $EndLog$
 */
...
#ifndef LINT
static char rcsid[]=
	"$Id: fakeka.c,v 1.1.1.1 2003-02-13 00:14:42 zacheiss Exp $";
#endif
...
#define PUT_INT(x) { int q = ntohl(x); bcopy((char *)&q, p, 4); ADVANCE(4); }
#define PUT_STR(x) { strcpy(p, x); ADVANCE(strlen(x) + 1); }
    ...
    *ticklenp = ntohl(p - ticket);
    ...
    if (rv)
	return rv;
    des_pcbc_encrypt(ticket, ticket, p - ticket, ssched, skey, ENCRYPT);
    bzero(skey, sizeof(skey));
    bzero(ssched, sizeof(ssched));
...
#undef	PUT_STR
    ...
    *enclenp = ntohl(p - enc);
    des_pcbc_encrypt(enc, enc, p - enc, sched, key, ENCRYPT);
    reply->len = n;

  error:
    bzero(new_session_key, sizeof(new_session_key));
    kadm5_free_principal_ent(handle, &cprinc);
...
#define GET_INT(x) { int q; bcopy(p, (char*)&q, 4); x = ntohl(q); ADVANCE(4); }
...
packet_t req, reply;
{
    ...
    if (rv)
	ERR(rv);
    des_pcbc_encrypt(p, p, 8, csched, ckey, DECRYPT);
...
packet_t req, reply;
{
    ...
    if (rv)
	ERR(rv);
    des_pcbc_encrypt(ticket, ticket, ticketlen, csched, ckey, DECRYPT);
    bzero(ckey, sizeof(ckey));
    bzero(csched, sizeof(csched));
    ...
    des_ecb_encrypt(enctimes, times, session_sched, DECRYPT);
  ...
  error:
    ...
    bzero(session_sched, sizeof(session_sched));
...
char **argv;
{
	...
	bzero(reply.data, sizeof(reply.data));
	req.len = packetlen;
	req.base = req.data;
	...
	if (forwarded) {
	    ...
	    bcopy(req.data, &ia.s_addr, 4);
	    from = inet_ntoa(ia);
	    /*
	     * copy the forwarder header and adjust the bases and lengths.
	     */
	    bcopy(req.data, reply.data, HEADER_LEN);
	    req.base += HEADER_LEN;
	    req.len -= HEADER_LEN;
