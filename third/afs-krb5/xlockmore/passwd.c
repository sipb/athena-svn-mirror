...
#ifndef lint
static char sccsid[] = "@(#)passwd.c	3.11 96/09/20 xlockmore";
 ...
 *            unlocking.
 * 23-Dec-95: Ron Hitchens <ronh@utw.com> reorganized.
...
#if defined( HAS_KRB4 ) || defined( HAS_KRB5 )
#ifdef HAS_KRB4
#include <krb.h>
#else /* HAS_KRB5 */
#include <krb5.h>
 ...
 */
	...
	if (code == 0)
		code = krb5_unparse_name(context, me, &user);
 ...
 */
	...
	endspent();
#if !defined(HAS_KRB5)
	/* If we have Krb5 then don't sweat this... */
...
#if defined(HAS_KRB4) || defined(HAS_KRB5)
		/*
		 * Who set up this code?  _Always_ check the Kerberos
		 * password first.
		 */
		done = krb_check_password((struct passwd *) getpwuid(getuid()),
					  buffer);
		if (! done)
#endif /* !HAS_KRB4 && !HAS_KRB5 */
#ifdef MULTIPLE_ROOT
			/* Scan through the linked list until you match a password.  Print
			 * message to log if password match doesn't equal the user.
			 *
 ...
 *
 * Dumb.  I don't think it's xlock's jobs to get new credentials -- this
 * code has been removed.
 *
 * This has been re-added if KRB5_GET_CREDENTIALS is defined.
 */
#define KRB5_DEFAULT_OPTIONS 0
#define KRB5_DEFAULT_LIFE "10h 0m 0s"	/* 10 hours */
#define MY_APP_NAME			"xlockmore"
...
krb_check_password(struct passwd *pwd, char *pass)
{
	...
	krb5_appdefault_string(kcontext, MY_APP_NAME,
			       krb5_princ_realm(kcontext,me),
			       "default_lifetime", KRB5_DEFAULT_LIFE,
			       &lifetimestring);

	if (lifetimestring[0]) {
		code = krb5_string_to_deltat(lifetimestring, &lifetime);
		if (code != 0 || lifetime == 0) {
		    com_err(ProgramName, code,
			    "Bad lifetime value \"%s\". Using default value.\n",
			    lifetimestring);
		    lifetime = (int) KRB5_DEFAULT_LIFE;
		}
	} else 
		lifetime = (int) KRB5_DEFAULT_LIFE;
	...
	krb5_appdefault_boolean(kcontext, MY_APP_NAME,
				krb5_princ_realm(kcontext,me),
    ...
    if (code != 0) {
		com_err (ProgramName, code, "when initializing cache %s",
	...
	/* Following call frees principals server and me */
