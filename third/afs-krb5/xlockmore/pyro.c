...
#ifndef lint
static char sccsid[] = "@(#)pyro.c	3.11 96/09/20 xlockmore";
...
burst(ModeInfo * mi, pyrostruct * pp, rocket * rp)
{
	...
	if (MI_WIN_IS_USE3D(mi)) {
		XDrawPoints(display, window, gc, rp->Xpointsleft[0], nstars, CoordModeOrigin);
		if (stype == DOUBLECLOUD)
			XDrawPoints(display, window, gc, rp->Xpointsleft[1], nstars, CoordModeOrigin);
	}
#endif
...
/* Stagger the stars' decay */
	...
	if (MI_WIN_IS_USE3D(mi)) {
		...
		XDrawPoints(display, window, gc, rp->Xpointsleft[0], nstars, CoordModeOrigin);
		if (stype == DOUBLECLOUD) {
			XDrawPoints(display, window, gc, rp->Xpointsleft[1], nstars,
				    CoordModeOrigin);
		}
		if (MI_WIN_IS_INSTALL(mi))
			XSetFunction(display, gc, GXcopy);
