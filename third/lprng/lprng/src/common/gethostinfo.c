...
"$Id: gethostinfo.c,v 1.2 2001-03-07 01:19:22 ghudson Exp $";
			...
			for( i = 0; result && i < c1; ++i ){
				...
				for( j = 0; result && j < c2; ++j ){
					...
					if(DEBUGL4){
						...
						ls[0] = 0; rs[0] = 0;
						for( n = 0; n < l1; ++n ){
							plp_snprintf( ls + strlen(ls), 6, "%02x", h1[n] );
						}
						for( n = 0; n < l1; ++n ){
							plp_snprintf( rs + strlen(rs), 6, "%02x", h2[n] );
						}
		...
		*s++ = '/';
		...
		if( t == 0 || *t ){
			...
			*t = 0;
			...
			if( bitcount ){
				if( buffer[0] ) *t++ = '.';
				plp_snprintf(t,6, "%d", (~((1<<(8-bitcount))-1))&0xFF);
				t += strlen(t);
				*t = 0;
			}
			for( ; i < addrlen; ++i ){
