 ...
 **************************************************************************/
/**** HEADER *****/
static char *const _id = "$Id: stty.c,v 1.1.1.3 1999-05-04 18:50:34 mwhitson Exp $";
...
void Do_stty( int fd, char *Stty_command )
{
	...
	DEBUG4("Do_Do_stty: using SGTTYB, fd %d", fd );
...
void Do_stty( int fd, char *Stty_command )
{
	...
	DEBUG4("Do_Do_stty: using TERMIO, fd %d", fd );
...
void Do_stty( int fd, char *Stty_command )
{
	...
	DEBUG4("Do_Do_stty: using TERMIOS, fd %d", fd );
	...
	for( argcount = 0; argcount < l.count; ++argcount ){
		...
		if( bauds[i].string ){
			...
			DEBUG4("Do_Do_stty: using cfsetispeed/cfsetospeed, speed %s, spd %d",
				arg, bauds[i].speed );
			/* POSIX baudrate manipulation */
			cfsetispeed( &t_dat, bauds[i].speed);
			cfsetospeed( &t_dat, bauds[i].speed);
#else
			DEBUG4("Do_Do_stty: setting tdat.c_cflag");
			t_dat.c_cflag &= ~CBAUD;
