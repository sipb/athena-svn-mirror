...
test_format ()
{
  ...
  GstElement *pipeline, *src, *filter, *conv, *enc, *mux, *sink;

  g_print ("creating sink\n");
  sink = gst_element_factory_make ("multifilesink", "sink");
  g_object_set (G_OBJECT (sink), "location", "format%d.ogg", NULL);
  ...
  enc = gst_element_factory_make ("rawvorbisenc", "enc");
  mux = gst_element_factory_make ("oggmux", "mux");

  pipeline = gst_pipeline_new ("pipeline");
  gst_bin_add_many (GST_BIN (pipeline), src, filter, conv, enc, mux, sink,
      NULL);
  gst_element_link_many (src, filter, conv, enc, mux, sink, NULL);
  ...
  for (i = 0; i < 9; i++) {
    ...
    tempfile = g_strdup_printf ("format%d.ogg", i);
...
newfile_signal (GstElement * element, gpointer user_data)
{
  ...
  filename = g_strdup_printf ("signal%d.ogg", *curindex);
...
test_signal ()
{
  ...
  g_print ("creating sink\n");
  sink = gst_element_factory_make ("multifilesink", "sink");
  g_object_set (G_OBJECT (sink), "location", "signal0.ogg", NULL);
  curindex = 1;
  ...
  for (i = 0; i < 9; i++) {
    ...
    tempfile = g_strdup_printf ("signal%d.ogg", i);
    retval = retval && g_file_test (tempfile, G_FILE_TEST_EXISTS);
    /* yuck but it works */
    unlink (tempfile);
    g_free (tempfile);
...
main (int argc, char **argv)
{
  ...
  if (!test_format ()) {
    g_print ("Test with location as format%%d.ogg failed\n");
    exit (-1);
  }
  if (!test_signal ()) {
