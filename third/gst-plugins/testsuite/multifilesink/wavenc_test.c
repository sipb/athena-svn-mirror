...
test_format ()
{
  ...
  GstElement *pipeline, *src, *filter, *enc, *sink, *conv;

  g_print ("creating sink\n");
  sink = gst_element_factory_make ("multifilesink", "sink");
  g_object_set (G_OBJECT (sink), "location", "format%d.wav", NULL);
  ...
  enc = gst_element_factory_make ("wavenc", "enc");
  conv = gst_element_factory_make ("audioconvert", "conv");

  pipeline = gst_pipeline_new ("pipeline");
  gst_bin_add_many (GST_BIN (pipeline), src, conv, filter, enc, sink, NULL);
  gst_element_link_many (src, conv, filter, enc, sink, NULL);
  ...
  for (i = 0; i < 9; i++) {
    ...
    tempfile = g_strdup_printf ("format%d.wav", i);
...
newfile_signal (GstElement * element, gpointer user_data)
{
  ...
  filename = g_strdup_printf ("signal%d.wav", *curindex);
...
test_signal ()
{
  ...
  GstElement *pipeline, *src, *filter, *enc, *sink;

  g_print ("creating sink\n");
  sink = gst_element_factory_make ("multifilesink", "sink");
  g_object_set (G_OBJECT (sink), "location", "signal0.wav", NULL);
  curindex = 1;
  ...
  pipeline = gst_pipeline_new ("pipeline");
  gst_bin_add_many (GST_BIN (pipeline), src, filter, enc, sink, NULL);
  gst_element_link_many (src, filter, enc, sink, NULL);
  ...
  for (i = 0; i < 9; i++) {
    ...
    tempfile = g_strdup_printf ("signal%d.wav", i);
    retval = retval && g_file_test (tempfile, G_FILE_TEST_EXISTS);
    /* yuck but it works */
    unlink (tempfile);
    g_free (tempfile);
...
main (int argc, char **argv)
{
  ...
  if (!test_format ()) {
    g_print ("Test with location as format%%d.wav failed\n");
    exit (-1);
  }
  if (!test_signal ()) {
