...
test_format ()
{
  ...
  GstElement *pipeline, *src, *filter, *enc, *mux, *sink;
  ...
  enc = gst_element_factory_make ("theoraenc", "enc");
  mux = gst_element_factory_make ("oggmux", "mux");

  pipeline = gst_pipeline_new ("pipeline");
  gst_bin_add_many (GST_BIN (pipeline), src, filter, enc, mux, sink, NULL);
  gst_element_link_many (src, filter, enc, mux, sink, NULL);
