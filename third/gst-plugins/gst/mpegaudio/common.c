...
int
mpegaudio_pick_table (fr_ps)    /* choose table, load if necess, return # sb's */
     frame_params *fr_ps;
{
...
void
mpegaudio_hdr_to_frps (fr_ps)   /* interpret data in hdr str to fields in fr_ps */
     frame_params *fr_ps;
{
  ...
  if (hdr->lay == 2)
    fr_ps->sblimit = mpegaudio_pick_table (fr_ps);
  else
     ...
     FILE *s;
{
  ...
  fprintf (s, "layer=%s, tot bitrate=%d, sfrq=%.1f, mode=%s, ",
      mpegaudio_layer_names[info->lay - 1],
      mpegaudio_bitrate[info->lay - 1][info->bitrate_index],
      mpegaudio_s_freq[info->sampling_frequency],
      mpegaudio_mode_names[info->mode]);
  fprintf (s, "sblim=%d, jsbd=%d, ch=%d\n", fr_ps->sblimit, fr_ps->jsbound,
      fr_ps->stereo);
  fflush (s);
}

void
WriteBitAlloc (bit_alloc, f_p, s)
...
void
WriteScale (bit_alloc, scfsi, scalar, fr_ps, s)
...
void
WriteSamples (ch, sample, bit_alloc, fr_ps, s)
     int ch;
...
int
NumericQ (s)                    /* see if a string lookd like a numeric argument */
     char *s;
{
  char c;
...
int
mpegaudio_BitrateIndex (layr, bRate)    /* convert bitrate in kbps to index */
     int layr;                  /* 1 or 2 */
...
int
mpegaudio_SmpFrqIndex (sRate)   /* convert samp frq in Hz to index */
...
void FAR *
mpegaudio_mem_alloc (block, item)
     unsigned long block;
     char *item;
{
...
int
mpegaudio_memcheck (array, test, num)
     char *array;
...
void
mpegaudio_double_to_extended (pd, ps)
     double *pd;
     char ps[10];
{
...
void
mpegaudio_extended_to_double (ps, pd)
     char ps[10];
     double *pd;
{
...
void
mpegaudio_I_CRC_calc (fr_ps, bit_alloc, crc)
     frame_params *fr_ps;
     unsigned int bit_alloc[2][SBLIMIT];
     unsigned int *crc;
{
...
void
mpegaudio_II_CRC_calc (fr_ps, bit_alloc, scfsi, crc)
     frame_params *fr_ps;
     unsigned int bit_alloc[2][SBLIMIT], scfsi[2][SBLIMIT];
     unsigned int *crc;
{
...
void
mpegaudio_update_CRC (data, length, crc)
...
void
mpegaudio_set_mac_file_attr (fileName, vRefNum, creator, fileType)
...
------------------------------------------------------------------------ */
char *
mpegaudio_new_ext (char *filename, char *extname)
{
  int found, dotpos;
  char newname[80];
