...
void
mpegaudio_psycho_anal (buffer, savebuf, chn, lay, snr32, sfreq)
     short int *buffer;
...
/* a corresponding mem_free() call at the end of this function.           */
  ...
  grouped_c = (FLOAT *) mpegaudio_mem_alloc (sizeof (FCB), "grouped_c");
  grouped_e = (FLOAT *) mpegaudio_mem_alloc (sizeof (FCB), "grouped_e");
  nb = (FLOAT *) mpegaudio_mem_alloc (sizeof (FCB), "nb");
  cb = (FLOAT *) mpegaudio_mem_alloc (sizeof (FCB), "cb");
  ecb = (FLOAT *) mpegaudio_mem_alloc (sizeof (FCB), "ecb");
  bc = (FLOAT *) mpegaudio_mem_alloc (sizeof (FCB), "bc");
  wsamp_r = (FLOAT *) mpegaudio_mem_alloc (sizeof (FBLK), "wsamp_r");
  wsamp_i = (FLOAT *) mpegaudio_mem_alloc (sizeof (FBLK), "wsamp_i");
  phi = (FLOAT *) mpegaudio_mem_alloc (sizeof (FBLK), "phi");
  energy = (FLOAT *) mpegaudio_mem_alloc (sizeof (FBLK), "energy");
  c = (FLOAT *) mpegaudio_mem_alloc (sizeof (FHBLK), "c");
  fthr = (FLOAT *) mpegaudio_mem_alloc (sizeof (FHBLK), "fthr");
  snrtmp = (F32 *) mpegaudio_mem_alloc (sizeof (F2_32), "snrtmp");
...
/* initialization time.  The mem_free() function must not be called.      */
    ...
    numlines = (int *) mpegaudio_mem_alloc (sizeof (ICB), "numlines");
    partition = (int *) mpegaudio_mem_alloc (sizeof (IHBLK), "partition");
    cbval = (FLOAT *) mpegaudio_mem_alloc (sizeof (FCB), "cbval");
    rnorm = (FLOAT *) mpegaudio_mem_alloc (sizeof (FCB), "rnorm");
    window = (FLOAT *) mpegaudio_mem_alloc (sizeof (FBLK), "window");
    absthr = (FLOAT *) mpegaudio_mem_alloc (sizeof (FHBLK), "absthr");
    tmn = (double *) mpegaudio_mem_alloc (sizeof (DCB), "tmn");
    s = (FCB *) mpegaudio_mem_alloc (sizeof (FCBCB), "s");
    lthr = (FHBLK *) mpegaudio_mem_alloc (sizeof (F2HBLK), "lthr");
    r = (F2HBLK *) mpegaudio_mem_alloc (sizeof (F22HBLK), "r");
    phi_sav = (F2HBLK *) mpegaudio_mem_alloc (sizeof (F22HBLK), "phi_sav");
...
/*        exit(-1); */
    ...
    mpegaudio_read_absthr (absthr, sfreq_idx);
    if (lay == 1) {
      flush = 384;
      syncsize = 1024;
      sync_flush = 576;
...
/* calculate fft frequency, then bval of each line (use fthr[] as tmp storage)*/
    for (i = 0; i < HBLKSIZE; i++) {
      ...
      while (temp1 > crit_band[j])
        j++;
      fthr[i] =
          j - 1 + (temp1 - crit_band[j - 1]) / (crit_band[j] - crit_band[j -
              1]);
    }
    partition[0] = 0;
 ...
 ************************************************************************/
    for (j = 0; j < CBANDS; j++) {
      for (i = 0; i < CBANDS; i++) {
        ...
        temp1 += 0.474;
        temp3 =
            15.811389 + 7.5 * temp1 - 17.5 * sqrt ((double) (1.0 +
                temp1 * temp1));
        if (temp3 <= -100)
          s[i][j] = 0;
        else {
...
/**Compute FFT****************************************************************/
        mpegaudio_fft (wsamp_r, wsamp_i, energy, phi, 1024);
/*****************************************************************************
 ...
 *****************************************************************************/
      ...
      for (i = 0; i < 32; i++) {
        if (lay == 2)
          snr32[i] =
              (snrtmp[0][i] > snrtmp[1][i]) ? snrtmp[0][i] : snrtmp[1][i];
        else
          snr32[i] = snrtmp[0][i];
      }
      break;
...
/* variables placed on the stack.                                        */
  ...
  mpegaudio_mem_free (grouped_c);
  mpegaudio_mem_free (grouped_e);
  mpegaudio_mem_free (nb);
  mpegaudio_mem_free (cb);
  mpegaudio_mem_free (ecb);
  mpegaudio_mem_free (bc);
  mpegaudio_mem_free (wsamp_r);
  mpegaudio_mem_free (wsamp_i);
  mpegaudio_mem_free (phi);
  mpegaudio_mem_free (energy);
  mpegaudio_mem_free (c);
  mpegaudio_mem_free (fthr);
  mpegaudio_mem_free (snrtmp);
}
