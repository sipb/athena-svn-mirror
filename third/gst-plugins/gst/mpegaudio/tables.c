...
int
mpegaudio_read_bit_alloc (table, alloc) /* read in table, return # subbands */
     int table;
     al_table *alloc;
{
  int i, j, n;
...
void
mpegaudio_read_ana_window (ana_win)
     double FAR ana_win[HAN_SIZE];
{
  int i;

  for (i = 0; i < 512; i++)
    ana_win[i] = mpegaudio_enwindow_tab[i];
}
...
void
mpegaudio_read_absthr (absthr, table)
     FLOAT *absthr;
     int table;
{
  int i;
  ...
  for (i = 0; i < HBLKSIZE; i++)
    absthr[i] = mpegaudio_absthr_tab[table][i];
}
...
void
mpegaudio_read_cbound (lay, freq)       /* this function reads in critical */
     int lay, freq;             /* band boundaries                 */
{
  ...
  mpegaudio_cbound =
      (int FAR *) mpegaudio_mem_alloc (sizeof (int) * mpegaudio_crit_band,
      "cbound");
  for (i = 0; i < mpegaudio_crit_band; i++)
    mpegaudio_cbound[i] = mpegaudio_cb_tab[n][i];
...
void
mpegaudio_read_freq_band (ltg, lay, freq)       /* this function reads in   */
     ...
     g_ptr FAR *ltg;            /* values                   */
{
  ...
  *ltg =
      (g_ptr FAR) mpegaudio_mem_alloc (sizeof (g_thres) * mpegaudio_sub_size,
      "ltg");
