...
GST_ELEMENT_DETAILS ("Volume Envelope",
    "Filter/Effect/Audio",
...
static GstStaticPadTemplate gst_volenv_src_template =
...
static GstStaticPadTemplate gst_volenv_sink_template =
...
static void gst_volenv_base_init (gpointer g_class);
...
static void gst_volenv_chain (GstPad * pad, GstData * _data);
static void inline gst_volenv_fast_16bit_chain (gint16 * data,
    gint16 * out_data, guint numsamples, GstVolEnv * filter);
...
volenv_link (GstPad * pad, const GstCaps * caps)
{
  GstVolEnv *filter;
  GstPad *otherpad;
...
gst_volenv_get_type (void)
{
  ...
  if (!volenv_type) {
    static const GTypeInfo volenv_info = {
      sizeof (GstVolEnvClass),
      gst_volenv_base_init,
      NULL,
    ...
    volenv_type =
        g_type_register_static (GST_TYPE_ELEMENT, "GstVolEnv", &volenv_info, 0);
  }
  return volenv_type;
}

static void
gst_volenv_base_init (gpointer g_class)
{
  GstElementClass *element_class = GST_ELEMENT_CLASS (g_class);

  gst_element_class_set_details (element_class, &volenv_details);

  gst_element_class_add_pad_template (element_class,
      gst_static_pad_template_get (&gst_volenv_src_template));
  gst_element_class_add_pad_template (element_class,
      gst_static_pad_template_get (&gst_volenv_sink_template));
}
static void
gst_volenv_class_init (GstVolEnvClass * klass)
{
  ...
  g_object_class_install_property (G_OBJECT_CLASS (klass), ARG_RISE, g_param_spec_double ("rise", "rise", "rise", -G_MAXDOUBLE, G_MAXDOUBLE, 0, G_PARAM_READWRITE));    /* CHECKME */
  g_object_class_install_property (G_OBJECT_CLASS (klass), ARG_LEVEL, g_param_spec_double ("level", "level", "level", -G_MAXDOUBLE, G_MAXDOUBLE, 0, G_PARAM_READWRITE));        /* CHECKME */
  g_object_class_install_property (G_OBJECT_CLASS (klass), ARG_CONTROLPOINT, g_param_spec_string ("controlpoint", "controlpoint", "controlpoint", NULL, G_PARAM_READWRITE));    /* CHECKME */
...
gst_volenv_init (GstVolEnv * filter)
{
  filter->sinkpad =
      ...
      (&gst_volenv_sink_template), "sink");
  filter->srcpad =
      gst_pad_new_from_template (gst_static_pad_template_get
      (&gst_volenv_src_template), "src");
  ...
  gst_pad_set_link_function (filter->sinkpad, volenv_link);
  gst_pad_set_getcaps_function (filter->sinkpad, gst_pad_proxy_getcaps);
  gst_element_add_pad (GST_ELEMENT (filter), filter->srcpad);
  gst_pad_set_link_function (filter->srcpad, volenv_link);
  gst_pad_set_getcaps_function (filter->srcpad, gst_pad_proxy_getcaps);
}

static void
gst_volenv_chain (GstPad * pad, GstData * _data)
{
...
/*  printf ("DEBUG : time %f, level %f, increase %e\n", filter->run_time, filter->level, filter->increase); */
  ...
  GST_BUFFER_DATA (outbuf) =
      (gchar *) g_new (gint16, GST_BUFFER_SIZE (buf) / 2);
  ...
  switch (width) {
    case 16:
      gst_volenv_fast_16bit_chain (in_data, out_data, num_samples, filter);
      break;
    case 8:
      num_samples *= 2;
...
#include "filter.func"
     static void
         gst_volenv_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  gint rate = 0;
  GstVolEnv *filter;
  ...
  switch (prop_id) {
    case ARG_RISE:
      ...
      GST_DEBUG ("filter increase set to %e", filter->increase);
      ...
      /* register control point */
    {
      ...
      GST_DEBUG ("volenv : added cp : level %f at time %f", cp_level, cp_time);
      *pcp_time = cp_time;
      *pcp_level = cp_level;
      ...
      if (g_list_length (filter->envelope) == 4) {
        ...
        GST_DEBUG ("second cp registered at %f level %f", cp_time, cp_level);
        GST_DEBUG ("first cp registered at %f level %f", *pst, *psl);
        GST_DEBUG ("second cp registered with rise %e", filter->rise);
...
static void
gst_volenv_get_property (GObject * object, guint prop_id, GValue * value,
    GParamSpec * pspec)
{
  GstVolEnv *filter;
...
{
  return gst_element_register (plugin, "volenv", GST_RANK_NONE,
      GST_TYPE_VOLENV);
}
