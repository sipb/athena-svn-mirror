...
void inithuff(mpeg1encoder_VidStream *vid_stream)
{
  ...
  int **MyCoef;
  Huffman_tables *huff = &vid_stream->huff;

  MyCoef = (int **) calloc(vid_stream->FrameInterval+1,sizeof (int *));
...
*/
  ...
  huff->MBADHuff = MakeDhuff();
  huff->MVDDHuff = MakeDhuff();
  huff->CBPDHuff = MakeDhuff();
  huff->T1DHuff = MakeDhuff();
  huff->T2DHuff = MakeDhuff();
  huff->IntraDHuff = MakeDhuff();
  huff->PredictedDHuff = MakeDhuff();
  huff->InterpolatedDHuff = MakeDhuff();
  huff->DCLumDHuff = MakeDhuff();
  huff->DCChromDHuff = MakeDhuff();
  huff->ModuloDHuff = (DHUFF **) calloc(vid_stream->FrameInterval+1,sizeof(DHUFF *));

  for(i=2;i<=vid_stream->FrameInterval;i++)
    {
      huff->ModuloDHuff[i] = MakeDhuff();
    }

  huff->MBAEHuff = MakeEhuff(40);
  huff->MVDEHuff = MakeEhuff(40);
  huff->CBPEHuff = MakeEhuff(70);
  huff->T1EHuff = MakeEhuff(8192);
  huff->T2EHuff = MakeEhuff(8192);

  huff->IntraEHuff = MakeEhuff(20);
  huff->PredictedEHuff = MakeEhuff(20);
  huff->InterpolatedEHuff = MakeEhuff(20);
  huff->DCLumEHuff = MakeEhuff(20);
  huff->DCChromEHuff = MakeEhuff(20);
  huff->ModuloEHuff = (EHUFF **) calloc(vid_stream->FrameInterval+1,sizeof(EHUFF *));
  for(i=2;i<=vid_stream->FrameInterval;i++)
    {
      huff->ModuloEHuff[i] = MakeEhuff(i+2);
    }

  LoadDTable(MBACoeff,huff->MBADHuff);
  LoadETable(MBACoeff,huff->MBAEHuff);
  LoadDTable(MVDCoeff,huff->MVDDHuff);
  LoadETable(MVDCoeff,huff->MVDEHuff);
  LoadDTable(CBPCoeff,huff->CBPDHuff);
  LoadETable(CBPCoeff,huff->CBPEHuff);
  LoadDTable(TCoeff1,huff->T1DHuff);
  LoadETable(TCoeff1,huff->T1EHuff);
  LoadDTable(TCoeff2,huff->T2DHuff);
  LoadETable(TCoeff2,huff->T2EHuff);

  LoadDTable(IntraTypeCoeff,huff->IntraDHuff);
  LoadETable(IntraTypeCoeff,huff->IntraEHuff);
  LoadDTable(PredictedTypeCoeff,huff->PredictedDHuff);
  LoadETable(PredictedTypeCoeff,huff->PredictedEHuff);
  LoadDTable(InterpolatedTypeCoeff,huff->InterpolatedDHuff);
  LoadETable(InterpolatedTypeCoeff,huff->InterpolatedEHuff);
  LoadDTable(DCLumCoeff,huff->DCLumDHuff);
  LoadETable(DCLumCoeff,huff->DCLumEHuff);
  LoadDTable(DCChromCoeff,huff->DCChromDHuff);
  LoadETable(DCChromCoeff,huff->DCChromEHuff);
  for(i=2;i<=vid_stream->FrameInterval;i++)
    {
      LoadDTable(MyCoef[i],huff->ModuloDHuff[i]);
      LoadETable(MyCoef[i],huff->ModuloEHuff[i]);
    }
}
...
int Encode(vid_stream,val,huff)
...
/*      printf("Value: %d|%x  Length: %d  Code: %d\n",
      ...
      vid_stream->NumberBitsCoded+=huff->Hlen[val];
      fputv(vid_stream,huff->Hlen[val],huff->Hcode[val]);
