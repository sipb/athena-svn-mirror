...
static void predict_mb _ANSI_ARGS_((
  mpeg2enc_vid_stream *vid_stream,unsigned char *oldref[], unsigned char *newref[], unsigned char *cur[],
...
static void pred _ANSI_ARGS_((mpeg2enc_vid_stream *vid_stream,unsigned char *src[], int sfield,
  unsigned char *dst[], int dfield,
  int lx, int w, int h, int x, int y, int dx, int dy, int addflag));

static void pred_comp_C _ANSI_ARGS_((unsigned char *src, unsigned char *dst,
  int lx, int w, int h, int x, int y, int dx, int dy, int addflag));
extern void pred_comp_mmx _ANSI_ARGS_((unsigned char *src, unsigned char *dst,
  int lx, int w, int h, int x, int y, int dx, int dy, int addflag));
extern void pred_comp_sse _ANSI_ARGS_((unsigned char *src, unsigned char *dst,
  int lx, int w, int h, int x, int y, int dx, int dy, int addflag));

static void (*pred_comp) _ANSI_ARGS_((unsigned char *src, unsigned char *dst,
  int lx, int w, int h, int x, int y, int dx, int dy, int addflag));

static void calc_DMV _ANSI_ARGS_((mpeg2enc_vid_stream *vid_stream,int DMV[][2], int *dmvector, int mvx,
  int mvy));

static void clearblock _ANSI_ARGS_((mpeg2enc_vid_stream *vid_stream,unsigned char *cur[], int i0, int j0));


void predict_init(vid_stream)
...
void predict(vid_stream,reff,refb,cur,secondfield,mbi)
mpeg2enc_vid_stream *vid_stream;
...
struct mbinfo *mbi;
{
  ...
  for (j=0; j<vid_stream->seq.height2; j+=16)
    ...
    {
      predict_mb(vid_stream,reff,refb,cur,vid_stream->seq.width,i,j,vid_stream->picture.pict_type,vid_stream->picture.pict_struct,
...
static void predict_mb(vid_stream,oldref,newref,cur,lx,bx,by,pict_type,pict_struct,
...
int PMV[2][2][2], mv_field_sel[2][2], dmvector[2];
{
  ...
  if (mb_type&MB_INTRA)
  {
    clearblock(vid_stream,cur,bx,by);
    return;
  }
  ...
  if ((mb_type & MB_FORWARD) || (pict_type==P_TYPE))
  {
    ...
    if (pict_struct==FRAME_PICTURE)
    {
      ...
      if ((motion_type==MC_FRAME) || !(mb_type & MB_FORWARD))
      {
        ...
        pred(vid_stream,oldref,0,cur,0,
      ...
      else if (motion_type==MC_FIELD)
      {
        ...
        pred(vid_stream,oldref,mv_field_sel[0][0],cur,0,
          lx<<1,16,8,bx,by>>1,PMV[0][0][0],PMV[0][0][1]>>1,0);

        /* bottom field prediction */
        pred(vid_stream,oldref,mv_field_sel[1][0],cur,1,
      ...
      else if (motion_type==MC_DMV)
      {
        ...
        calc_DMV(vid_stream,DMV,dmvector,PMV[0][0][0],PMV[0][0][1]>>1);
        ...
        pred(vid_stream,oldref,1,cur,1,
          lx<<1,16,8,bx,by>>1,PMV[0][0][0],PMV[0][0][1]>>1,0);

        /* predict and add to top field from bottom field */
        pred(vid_stream,oldref,1,cur,0,
          lx<<1,16,8,bx,by>>1,DMV[0][0],DMV[0][1],1);

        /* predict and add to bottom field from top field */
        pred(vid_stream,oldref,0,cur,1,
    ...
    else /* TOP_FIELD or BOTTOM_FIELD */
    {
      ...
      if ((motion_type==MC_FIELD) || !(mb_type & MB_FORWARD))
      {
        ...
        pred(vid_stream,predframe,mv_field_sel[0][0],cur,currentfield,
      ...
      else if (motion_type==MC_16X8)
      {
        ...
        /* lower half */
        pred(vid_stream,predframe,mv_field_sel[1][0],cur,currentfield,
      ...
      else if (motion_type==MC_DMV)
      {
        ...
        /* calculate derived motion vectors */
        calc_DMV(vid_stream,DMV,dmvector,PMV[0][0][0],PMV[0][0][1]);

        /* predict from field of same parity */
        pred(vid_stream,oldref,currentfield,cur,currentfield,
          lx<<1,16,16,bx,by,PMV[0][0][0],PMV[0][0][1],0);

        /* predict from field of opposite parity */
        pred(vid_stream,predframe,!currentfield,cur,currentfield,
  ...
  if (mb_type & MB_BACKWARD)
  {
    ...
    if (pict_struct==FRAME_PICTURE)
    {
      ...
      {
        /* frame-based prediction in frame picture */
        pred(vid_stream,newref,0,cur,0,
      ...
      else
      {
        ...
        /* top field prediction */
        pred(vid_stream,newref,mv_field_sel[0][1],cur,0,
          lx<<1,16,8,bx,by>>1,PMV[0][1][0],PMV[0][1][1]>>1,addflag);

        /* bottom field prediction */
        pred(vid_stream,newref,mv_field_sel[1][1],cur,1,
    ...
    else /* TOP_FIELD or BOTTOM_FIELD */
    {
      ...
      {
        /* field-based prediction in field picture */
        pred(vid_stream,newref,mv_field_sel[0][1],cur,currentfield,
      ...
      else if (motion_type==MC_16X8)
      {
        ...
        /* lower half */
        pred(vid_stream,newref,mv_field_sel[1][1],cur,currentfield,
 ...
 */
static void pred(vid_stream,src,sfield,dst,dfield,lx,w,h,x,y,dx,dy,addflag)
mpeg2enc_vid_stream *vid_stream;
unsigned char *src[];
int sfield;
unsigned char *dst[];
...
static void pred_comp_C(src,dst,lx,w,h,x,y,dx,dy,addflag)
unsigned char *src;
unsigned char *dst;
int lx;
int w, h;
...
static void calc_DMV(vid_stream,DMV,dmvector,mvx,mvy)
mpeg2enc_vid_stream *vid_stream;
int DMV[][2];
int *dmvector;
int mvx, mvy;
{
  ...
  else
  {
    ...
    if (vid_stream->picture.pict_struct==TOP_FIELD)
      DMV[0][1]--;
    else
      DMV[0][1]++;
  }
...
static void clearblock(vid_stream,cur,i0,j0)
mpeg2enc_vid_stream *vid_stream;
unsigned char *cur[];
int i0, j0;
{
  ...
  p = cur[0] + ((vid_stream->picture.pict_struct==BOTTOM_FIELD) ? vid_stream->seq.width : 0) + i0 + vid_stream->seq.width2*j0;

  for (j=0; j<16; j++)
  {
    for (i=0; i<16; i++)
      p[i] = 128;
    p+= vid_stream->seq.width2;
  }
  ...
  p = cur[1] + ((vid_stream->picture.pict_struct==BOTTOM_FIELD) ? vid_stream->seq.chrom_width : 0) + i0
             + vid_stream->seq.chrom_width2*j0;

  for (j=0; j<h; j++)
  {
    for (i=0; i<w; i++)
      p[i] = 128;
    p+= vid_stream->seq.chrom_width2;
  }

  p = cur[2] + ((vid_stream->picture.pict_struct==BOTTOM_FIELD) ? vid_stream->seq.chrom_width : 0) + i0
             + vid_stream->seq.chrom_width2*j0;
