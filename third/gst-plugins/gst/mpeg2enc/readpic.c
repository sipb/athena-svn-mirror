...
static void border_extend _ANSI_ARGS_((unsigned char *frame, int w1, int h1,
  int w2, int h2));
static void conv444to422 _ANSI_ARGS_((unsigned char *src, unsigned char *dst, int width, int height, int mpeg1, unsigned char*clp));
static void conv422to420 _ANSI_ARGS_((unsigned char *src, unsigned char *dst, int width, int height, int prog_frame, unsigned char*clp));
...
void convertRGBtoYUV(mpeg2enc_vid_stream *vid_stream, unsigned char *source, unsigned char *dest)
{
  ...
  int width = vid_stream->seq.width;
  int height = vid_stream->seq.height;

  i = vid_stream->seq.matrix_coefficients;
  if (i>8)
    i = 3;

  frame[0] = dest;
  frame[1] = dest+width*height;
  frame[2] = dest+width*height+((width>>1)*height);
  ...
  for (i=0; i<vid_stream->seq.vertical_size; i++)
  {
    yp = frame[0] + i*width;
    up = u444 + i*width;
    vp = v444 + i*width;

    for (j=0; j<vid_stream->seq.horizontal_size; j++)
    {
      r=*source++;
      g=*source++;
      b=*source++;
  ...
  border_extend(frame[0],vid_stream->seq.horizontal_size,vid_stream->seq.vertical_size,width,height);
  border_extend(u444,vid_stream->seq.horizontal_size,vid_stream->seq.vertical_size,width,height);
  border_extend(v444,vid_stream->seq.horizontal_size,vid_stream->seq.vertical_size,width,height);

  if (vid_stream->seq.chroma_format==CHROMA422)
  {
    conv444to422(u444,frame[1], width, height, vid_stream->mpeg1, vid_stream->clp);
    conv444to422(v444,frame[2], width, height, vid_stream->mpeg1, vid_stream->clp);
  }

  if (vid_stream->seq.chroma_format==CHROMA420)
  {
    conv444to422(u444,u422, width, height, vid_stream->mpeg1, vid_stream->clp);
    conv444to422(v444,v422, width, height, vid_stream->mpeg1, vid_stream->clp);
    conv422to420(u422,frame[1], width, height, vid_stream->picture.prog_frame, vid_stream->clp);
    conv422to420(v422,frame[2], width, height, vid_stream->picture.prog_frame, vid_stream->clp);
  }
}
...
static void conv444to422(src,dst, width, height, mpeg1, clp)
...
static void conv422to420(src,dst, width, height, prog_frame, clp)
unsigned char *src, *dst;
int width, height;
int prog_frame;
unsigned char *clp;
