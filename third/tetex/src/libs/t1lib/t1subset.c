		     ...
		     unsigned long *bufsize)
{
  ...
  while (retval>0) {
	...
	sprintf( &(filebuf[i]), "readonly def\n");
        i+=strlen( &(filebuf[i]));
	...
	nocharstrings++;
      ...
      sprintf( &(filebuf[i]), "2 index /CharStrings %d dict dup begin\n",
		  nocharstrings);
      i+=strlen( &(filebuf[i]));
      break;
    }
  ...
  */
  ...
  sprintf( &(filebuf[i]), "/.notdef %d %s ", charstringL, rdstring);
  i+=strlen( &(filebuf[i]));
  ...
  for ( j=0; j<256; j++) {
	...
	k=i;
	sprintf( &(filebuf[i]), "/%s %d %s ", charnameP, charstringL, rdstring);
        i+=strlen( &(filebuf[i]));
	memcpy(&(filebuf[i]), charstringP, charstringL);
	i+=charstringL;
	sprintf( &(filebuf[i]), " %s\n", ndstring);
  ...
  sprintf( &(trailerbuf[l]), "cleartomark\n");
  l+=strlen( &(trailerbuf[l]));
  if ( !((tr_len==1) && ((linebuf[0]=='\n') || (linebuf[0]=='\r')) ) ) {
    sprintf( err_warn_msg_buf,
    ...
    T1_PrintLog( "T1_SubsetFont()", err_warn_msg_buf,
		 T1LOG_DEBUG);
    sprintf( &(trailerbuf[l]), linebuf); /* contains the PostScript trailer */
    l+=strlen( &(trailerbuf[l]));
  }
