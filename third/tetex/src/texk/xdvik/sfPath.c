...
SFfindFile(SFDir *dir, char *str)
{
    ...
    else if (str[len - 1] == DIR_SEPARATOR) {
	len--;
    }
...
SFupdatePath(void)
{
	...
	begin = end;
	while ((*end) && (*end++ != DIR_SEPARATOR)) {
	    ;
	}
	if ((end - SFcurrentPath <= SFtextPos) && (*(end - 1) == DIR_SEPARATOR)) {
	    SFdirPtr = i - 1;
	    if (SFdirPtr < 0) {
		SFdirPtr = 0;
	    }
