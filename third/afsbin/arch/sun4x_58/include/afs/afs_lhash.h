...
/*
 * copied from
 * /afs/tr/proj/dce/rcs/file/util/RCS/hash.h,v 1.12 1999/10/22 14:58:42 ota
 *
 * The 'unsigned int' quantities are assumed to be 32 bits wide.
 */

unsigned int afs_HashBytes(
  const void   *key,			/* the key */
  unsigned int	length,			/* the length of the key */
  unsigned int	initval);
unsigned int afs_HashString(
  const char   *key,			/* the key: a null terminated string */
  unsigned int	initval);		/* the prev. hash or arbitrary value */
unsigned int afs_HashWords(
  const void   *key,			/* the key */
  unsigned int	length,			/* the length of the key */
  unsigned int	initval);
unsigned int afs_HashWords3(unsigned int a, unsigned int b, unsigned int c);
