...
component_details_get_info (ComponentDetails *comp_details, gchar *iid) {
	...
	comp_details->priv->components = bonobo_browser_get_components_list (
		query);
	g_free (query);
	...
	/* Build the General tab */
	info = (BonoboComponentInfo *) comp_details->priv->components->data;
	...
	repo_ids = bonobo_component_get_repoids (info);
	...
	/* Clean up time... */
	bonobo_component_info_free (info);
