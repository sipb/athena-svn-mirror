...
* Fix all warnings.
*
* Revision 1.3  1998/02/12 03:07:53  unammx
...
static int  LZWStreamGetCode(LZWStream *lzw);
...
/*****
* Name: 		LZWStreamGetCode
* Return Type: 	int
* Description: 	gets next code signal from the LZW stream
* In: 
*	lzw:		current LZWStream
* Returns:
*	code signal or -1 on end of data.
*****/
static int 
LZWStreamGetCode(LZWStream *lzw)
{
	int code = 0;
	register int i, j;
	...
	if((lzw->curBit + lzw->codeBits) >= lzw->lastBit)
	{
		...
		{
			/*
			* when lzw->curBit >= lzw->lastBit there was less data than
			* required. This is a common occurance for gif images.
			*/
			return(-1);
		}

		/*
		* Keep two last codes of current buffer, they haven't been processed
		* yet.
		*/
...
LZWStreamConvertBelow8(LZWStream *lzw)
{
	...
	*/
	while((inCode = LZWStreamGetCode(lzw)) != -1 && !eod)
	{
		/*****
