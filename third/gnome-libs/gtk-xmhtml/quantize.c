...
#define _WANT_TIMINGS	/* we want timings, see XmHTMLfuncs.h */
...
{
	unsigned long colors[MAX_IMAGE_COLORS],col;
...
QuickQuantize(Byte *rgb, XmHTMLRawImageData *img_data)
{
	...
	AllocRawImageCmap(img_data, MAX_IMAGE_COLORS);
	cols = img_data->cmap;

	/* fill colormap */
	for(i = 0; i < MAX_IMAGE_COLORS; i++)
	{
		cols[i].red  =(((i << RSHIFT) & RMASK)*255 + 0.5*RMASK)/RMASK;
		cols[i].green=(((i << GSHIFT) & GMASK)*255 + 0.5*GMASK)/GMASK;
		cols[i].blue =(((i << BSHIFT) & BMASK)*255 + 0.5*BMASK)/BMASK;
	}
...
*	more than MAX_IMAGE_COLORS colors) and the data field has been pixelized. 
*****/
void
...
{
	int r, g, b, er, eg, eb;
	int i, j, size, ex, used[MAX_IMAGE_COLORS];
