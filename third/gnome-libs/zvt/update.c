...
void vt_update_rect(struct _vtx *vx, int fill, int csx, int csy, int cex, int cey)
{
  ...
  if (wn) {
	...
	bl->data[i] = fillin;
      ...
      /* this is an optimisation so we dont try and render text we've
	 already rendered.  since we only keep track of if it was modified,
	 and not the modification range, we can only do it on a whole line */
      if (csx <=0 && cex >= bl->width)
	      wn->modcount = 0;
