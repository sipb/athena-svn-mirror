...
create_user_gnome_directories (void)
{
	...
	if(
	   ...
	   && errno != EEXIST)
	  g_warning("Could not create per-user Gnome directory <%s>\n",
		  gnome_user_dir);
	...
	if(
	   ...
	   && errno != EEXIST)
		g_warning("Could not create private per-user Gnome directory <%s>\n",
	...
	if (chmod (gnome_user_private_dir, 0700) != 0)
		g_warning (
			"Could not set mode 0700 on private per-user Gnome directory <%s>\n",
			gnome_user_private_dir);
