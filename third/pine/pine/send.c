...
static char rcsid[] = "$Id: send.c,v 1.1.1.1 2001-02-19 07:11:51 ghudson Exp $";
#endif
/*----------------------------------------------------------------------
    ...
    gf_io_t        inc_text_getc;
{
    ...
    if(!role_arg && !ps_global->anonymous && !(given_to || attach)){
	char	     file_path[MAXPATH+1], *p;
	int	     ret = 'n';
    ...
    if(!outgoing && !ps_global->anonymous && !(given_to || attach)
	...
	build_path(file_path,
		   ...
		   INTERRUPTED_MAIL, sizeof(file_path));
	if(folder_exists(NULL, file_path) & FEX_ISFILE){
	    if((stream = pine_mail_open(NULL, file_path, FCC_STREAM_MODE))
	       && !(((*(p = stream->mailbox) == '<')
		     || ((p = strindex(p, '}')) && *++p == '<'))
		    && !strcmp(p + 1, "no_mailbox>"))){
		...
		if(F_ON(F_ALT_COMPOSE_MENU, ps_global) || 
		   (ret = redraft_prompt("Interrupted",INTRPT_PMT,'n')) =='y'){
		    if(!redraft(stream, &outgoing, &body, &fcc, &lcc, &reply,
		      ...
		      return;
		    /* NOTE: "stream" closing in redraft() */
		    postponed = form = userole = 0;
		}
		else{
		    mail_close(stream);
		    if(ret == 'x'){
	...
	if(exists & FEX_ISFILE){
	    ...
	    if(stream
	       || ((stream = context_open(p_cntxt,NULL,mbox,FCC_STREAM_MODE))
		   && !(((*(p = stream->mailbox) == '<')
			 || ((p = strindex(p, '}')) && *++p == '<'))
			&& !strcmp(p + 1, "no_mailbox>")))){
		if(F_ON(F_ALT_COMPOSE_MENU, ps_global) || 
		   (ret = redraft_prompt("Postponed",PSTPND_PMT,'n')) == 'y'){
		    ...
		    /* note: "stream" closing handled in redraft() */
		    intrptd = form = userole = 0;
		}
		else{
		    if(stream != ps_global->mail_stream)
		      mail_close(stream);
	...
	if(attach)
	  create_message_body(&body, attach);
    }
...
/*----------------------------------------------------------------------
 ...
 ----*/
int
redraft(stream, outgoing, body, fcc, lcc, reply, redraft_pos, custom, role,
	flags)
    MAILSTREAM	  *stream;
    ENVELOPE	  **outgoing;
    BODY	  **body;
    ...
    int		    flags;
{
	...
	mn_set_cur(msgmap, (REDRAFT_PPND&flags) ? stream->nmsgs : 1L);
...
#define INDEX_CURSORPOS 3
#define INDEX_OUR_REPLYTO 4
#define INDEX_LCC 5		/* MUST REMAIN LAST FIELD DECLARED */
#define FIELD_COUNT 6
	...
	if(extras = pine_fetchheader_lines(stream, cont_msg, NULL,fields)){
	    ...
	    for(i = INDEX_LCC; fields[i]; i++)
		  ...
		  if(strlen(values[i]) > SIZEOF_20KBUF-1)
		    values[i][SIZEOF_20KBUF-1] = '\0';

		  p = (char *)rfc1522_decode((unsigned char*)tmp_20k_buf,
					     SIZEOF_20KBUF,values[i], &charset);
		  if(charset)
	...
	*outgoing = copy_envelope(e);
	...
	if(our_replyto == 'E'){
	    ...
	    if(add_to_role && role && (*role == NULL || from_added_to_role)){
		if(*role == NULL){
		    *role = (ACTION_S *)fs_get(sizeof(**role));
		    memset((void *)*role, 0, sizeof(**role));
		}
	...
	if((flags & REDRAFT_DEL) && !mail_elt(stream, cont_msg)->deleted)
...
Args:	stream --
 ...
 ----*/
int
redraft_cleanup(stream, problem, flags)
    MAILSTREAM *stream;
    int		problem;
    int         flags;
{
	...
	if(!stream->nmsgs){			/* close and delete folder */
	    ...
	    /*
	     * At this point, one way or another stream's invalid
	     * so reuse it and see if we can't reuse a current stream
	     * as well to delete "mbox"...
	     */

	    if(!(stream = same_stream(mbox, ps_global->mail_stream))
...
dos_valid_from()
{
	...
	if(ps_global->blank_user_id
	   && want_to(prompt, 'y', 'n', NO_HELP, WT_NORM) == 'y'){
	    set_variable(V_USER_ID, answer, 1, Main);
	}
	else{
	...
	if(rc == 0 && answer){		/* save the name */
	    ...
	    if(ps_global->blank_personal_name
	       && want_to(prompt, 'y', 'n', NO_HELP, WT_NORM) == 'y'){
		set_variable(V_PERSONAL_NAME, answer, 1, Main);
	    }
	    else{
	...
	if(!ps_global->userdomain && !ps_global->blank_user_domain
	   && want_to(prompt, 'y', 'n', NO_HELP, WT_NORM) == 'y'){
	    set_variable(V_USER_DOMAIN, answer, 1, Main);
	...
	switch(i){
	  ...
	  case 0:
	   {void (*redraw) () = ps_global->redrawer;
		...
		errbuf = NULL;
		if(build_address(*tobufp, &addr, &errbuf, &ba_fcc, NULL) >= 0){
		    int sendit = 0;
			...
			if(lmc.so){
			   ...
			   && pine_rfc822_output(&header, *body, NULL, NULL))){
				...
				fcc_result = 
				  write_fcc(fcc,fcc_cntxt,lmc.so,NULL,label);
			    }
			    else if(result == 0){
				q_status_message(SM_ORDER,3,5,
    ...
    int        sticky_fcc;
{
    ...
    struct headerentry *he, *headents, *he_to, *he_fcc, *he_news, *he_lcc,
		       *he_from = NULL;
    PINEFIELD          *pfields, *pf, *pf_nobody = NULL, *pf_ref = NULL,
		       *pf_fcc = NULL, *pf_err, *pf_uid, *pf_curpos,
    ...
    pbf->quote_str     = reply ? reply->prefix : "> ";
    pbf->edit_offset   = 0;
...
#define NN 2
	...
	static int news_order[] = {
	    ...
	    N_MSGID, N_NOBODY, N_POSTERR, N_RPLUID, N_CURPOS, N_OURREPLYTO,
	    N_OURHDRS
#if	!(defined(DOS) || defined(OS2)) || defined(NOAUTH)
	    , N_SENDER
#endif
	...
	*he             = he_template[index];
	 ...
	 */
	if(!reply && !redraft_pos && !(pbf->pine_flags & P_BODY) && he_from &&
 ...
 */
  ...
  fake_hup:
		...
		/* If there wasn't one, ":" we start in the body. */
...
----*/      
char *
send_exit_for_pico(he, redraw_pico)
    struct headerentry *he;
    void (*redraw_pico)();
{
    int	       i, rv, c, verbose_label = 0, bg_label = 0, old_suspend;
    int        dsn_label = 0, fcc_label = 0, dsn_show, no_help, lparen;
    ...
    REPLY_S *reply;
{
	...
	if(ps_global->mail_stream
	   && !strcmp(reply->mailbox, ps_global->mail_stream->mailbox))
	  stream = ps_global->mail_stream;
	else if(ps_global->inbox_stream
		&& ps_global->inbox_stream != ps_global->mail_stream
		&& !strcmp(reply->mailbox, ps_global->inbox_stream->mailbox))
	...
	if(stream->uid_validity == reply->data.uid.validity){
	    ...
	    fs_give((void **)&seq);
	    if(stream == ps_global->mail_stream)
	      check_point_change();	/* make note of state change */
	}

	if(ourstream)
	  mail_close(stream);		/* clean up dangling stream */
    ...
    char *addr;
{
	...
	so_puts((STORE_S *)body->contents.text.data, "Document request: Pine");
...
call_mailer(header, body)
    ...
    BODY    *body;
{
    char         error_buf[100], *error_mess = NULL, *postcmd;
    ADDRESS     *a;
    ...
    *fcc_cntxt		    = NULL;
	...
	if(!(*fcc_cntxt = default_save_context(ps_global->context_list)))
	  *fcc_cntxt = ps_global->context_list;
		 ...
		 */
		create_stream = context_same_stream(*fcc_cntxt,fcc,
						    ps_global->mail_stream);
		if(!create_stream
		   && ps_global->mail_stream != ps_global->inbox_stream)
		  create_stream = context_same_stream(*fcc_cntxt, fcc,
		...
		if(context_create(*fcc_cntxt, create_stream, fcc))
		  ok++;
	...
	if(exists & (FEX_ISFILE | FEX_ISDIR)){
		...
		ok = pine_mail_create(create_stream, fcc) != 0L;
...
An error message is produced if this fails.
  ----*/
int
write_fcc(fcc, fcc_cntxt, tmp_storage, stream, label)
     char	*fcc;
     CONTEXT_S	*fcc_cntxt;
     STORE_S	*tmp_storage;
     MAILSTREAM *stream;
 ...
 */
    ...
    if(!fcc_stream)
      fcc_stream = context_same_stream(cntxt, fcc, ps_global->mail_stream);

    if(!fcc_stream && ps_global->mail_stream != ps_global->inbox_stream)
      fcc_stream = context_same_stream(cntxt, fcc, ps_global->inbox_stream);
	...
	for(a = new; a; a = a->next)
	  if(a->personal && a->mailbox && !strcmp(old->mailbox, a->mailbox)
	     && a->host && !strcmp(old->host, a->host)){
	      char *charset = NULL, *p, buftmp[MAILTMPLEN];
    ...
    PATMT  *attach;
{
    ...
    set_mime_type_by_grope(&((*b)->nested.part->body), NULL);
    set_body_size(&((*b)->nested.part->body));
	...
	for(pa = attach; pa && (*pp)->body.id; pa = pa->next)
	  ...
	  if(pa->id && strcmp(pa->id, (*pp)->body.id) == 0){
		 ...
		 || strcmp(pa->description, (*pp)->body.description)){
	...
	*parmp		    = mail_newbody_parameter();
     ...
     * text/plain message.
     */
    if(!(*b)->nested.part->next && (*b)->nested.part->body.type == TYPETEXT){
    ...
    PATMT  **alist;
{
	...
	if((*alist)->filename){
		...
		dprint(1, (debugfile, "-- Can't unlink(%s): %s\n",
			   (*alist)->filename, error_description(errno)));
    ...
    char     *addition;
{
    ...
    if((p = strchr(e->message_id, '@'))
       && (alen = strlen(addition))
       && ++alen < (plen = p - e->message_id)
       && *(p - alen) != '-'){		/* already updated? */
	new = fs_get(plen + alen + strlen(p) + 1);
	sprintf(new, "%.*s-%s%s", plen, e->message_id, addition, p);
    ...
    BODY *body;
{
    ...
    switch (body->type) {
      ...
      case TYPEVIDEO:
        ...
        break;
      case TYPEMULTIPART:
        for(part = body->nested.part; part != NULL; part = part->next) 
    ...
    int            encode_all;
{
    ...
    int   in_comment = 0, some_encoding_done = 0;
    ...
    int        writehdr, localcopy;
{
    ...
    size_t   alloced = 0;
    char    *p, *delim, *ptmp, ch, buftmp[MAILTMPLEN];
    ADDRESS *atmp;
    int      i, count;
    ...
    TCPSTREAM *s;
{
     ...
     */
    {
	 ...
	 if(((stream = ps_global->inbox_stream) && stream->mailbox 
    ...
    TCPSTREAM *s;
{
	...
	if(body->type == TYPETEXT){
		...
		gf_link_filter(gf_translate, NULL);
	    }
#endif
	}
	...
	if(lmc.text_written){	/* we have some splainin' to do */
	    ...
	    sprintf(tmp, "    A %s/%.100s segment of about %s bytes.\015\012",
...
news_poster(header, body)
     ...
     BODY    *body;
{
    char *error_mess, error_buf[100], **news_servers;
    ...
    CustomType cstmtype;
{
    ...
    if(list){
		...
		if(isspace((unsigned char)*t)){
		    q_status_message1(SM_ORDER, 3, 3,
				      "Space not allowed in header name (%s)",
				      name);
		    continue;
		}
		...
		*t = '\0';
		...
		if(pine_header_forbidden(name)){
		    q_status_message1(SM_ORDER, 3, 3,
				      "Not allowed to change header \"%s\"",
				      name);
