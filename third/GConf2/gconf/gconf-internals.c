  ...
  if (gconf_use_local_locks ())
    {
       ...
       * per-session directory in /tmp, so we can run a separate
	...
	return g_build_filename (session_tmpdir, "gconfd", NULL);
	...
	session_id = "";
      ...
      subdir = g_strconcat ("gconfd-", g_get_user_name (), "-",
			    session_id, NULL);
      s = g_build_filename ("/tmp", subdir, NULL);
