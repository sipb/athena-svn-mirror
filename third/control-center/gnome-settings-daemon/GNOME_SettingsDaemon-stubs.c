...
if (ORBit_small_flags & ORBIT_SMALL_FAST_LOCALS && 
    ...
    (_ORBIT_epv = (POA_GNOME_SettingsDaemon__epv*) ORBIT_STUB_GetEpv (_obj, GNOME_SettingsDaemon__classid))->awake) {
ORBIT_STUB_PreCall (_obj);
_ORBIT_retval = _ORBIT_epv->awake (ORBIT_STUB_GetServant (_obj), service, ev);
ORBIT_STUB_PostCall (_obj);
 } else { /* remote marshal */
gpointer _args[1];
_args[0] = (gpointer)&service;
ORBit_small_invoke_stub_n (_obj, &GNOME_SettingsDaemon__iinterface.methods, 0, &_ORBIT_retval, _args, NULL, ev);
