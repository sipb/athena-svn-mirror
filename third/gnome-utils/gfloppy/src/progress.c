	       ...
	       GdkInputCondition condition)
{
	...
	if (size == 0) {
		...
		error_dialog = gtk_message_dialog_new ((GtkWindow *)progress_dialog, 0,
						       dialog_type,
						       GTK_BUTTONS_CLOSE,
						       error_label);
...
static void
progress_close_callback (GtkWidget *progress, gpointer data)
{
	GFloppy *floppy = (GFloppy *) data;
...
setup_progress_and_run (GFloppy *floppy, GtkWidget *parent)
{
	...
	gtk_box_pack_start (GTK_BOX (GTK_DIALOG (progress_dialog)->vbox), msg_label, FALSE, FALSE, 0);
	gtk_widget_show (msg_label);
	progressbar = gtk_progress_bar_new ();
	gtk_box_pack_start (GTK_BOX (GTK_DIALOG (progress_dialog)->vbox), progressbar, FALSE, FALSE, 0);
	floppy->handler_id = gdk_input_add (floppy->message[0], GDK_INPUT_READ, read_stdinput, floppy);

	gtk_signal_connect (GTK_OBJECT (progress_dialog), "close", GTK_SIGNAL_FUNC (progress_close_callback), floppy);
