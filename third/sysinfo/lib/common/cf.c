...
/*
 * List of config directories relative to the executing program which
 * we look for.
 */
static char *RelativeConfDirs[] = {
    ...
    char		       *String;
{
    ...
    if (!Path) {
        SImsg(SIM_GERR, "%s: Line %d: No valid pathname specified.",
	      PathName, LineNo);
	return(-1);
    }
    if (!FileExists(Path)) {
	SImsg(SIM_GERR, "%s: Line %d: %s file does not exist.",
...
/*
 * Do the dirty work of finding a Config Directory
 */
static char *CFchooseConfDirRel(ProgramDir, RelConfDir, OSname)
     char		       *ProgramDir;
     char		       *RelConfDir;
     char		       *OSname;
{
    static char			CfDir[MAXPATHLEN];
    static char			OScf[MAXPATHLEN];

    if (!RelConfDir || !OSname)
	return (char *) NULL;

    /*
     * This could be NULL if we're in the same directory as the program
     */
    if (ProgramDir && ProgramDir[0]) {
	if (chdir(ProgramDir) != 0) {
	    SImsg(SIM_DBG, "%s: chdir failed: %s", ProgramDir, SYSERR);
	    return (char *) NULL;
	}
    }

    /*
     * We're now in the dir where ProgramPath resides
     */

    /*
     * If the main OS .cf file exists, then this must be the place.
     */
    (void) snprintf(OScf, sizeof(OScf), "%s/%s.cf", RelConfDir, OSname);
    if (FileExists(OScf)) {
	if (chdir(RelConfDir) == 0) {
	    if (!GetCwd(CfDir, sizeof(CfDir))) {
		SImsg(SIM_GERR, "Cannot determine current working directory.");
		return (char *) NULL;
	    }
	    /*
	     * Success!
	    ...
	    SImsg(SIM_DBG, "%s: Cannot chdir from <%s>: %s", 
		  RelConfDir, ProgramDir, SYSERR);
	    return (char *) NULL;
	}
    }

    SImsg(SIM_DBG, "CFchooseConfDir: %s/%s Doesn't exist.", ProgramDir, OScf);

    return (char *) NULL;
}

/*
 * See if we can find the config directory relative to the ProgramPath
 */
extern char *CFchooseConfDir(ProgramPath)
     char		       *ProgramPath;
{
    ...
    SImsg(SIM_DBG, "Searching for Config Dir . . .");

    if (!GetCwd(StartDir, sizeof(StartDir))) {
	SImsg(SIM_GERR, "Cannot determine current working directory path.");
	return (char *) NULL;
    }

    ProgramDir = DirName(ProgramPath);

    /*
     * Get our OS name using the normal CF* call to VarSub() 
     * which will edit OSname appropriately.
     */
    OSname = VarGetVal("OSname", NULL);
    if (!OSname)
	return (char *) NULL;

    /*
     * Try each relative directory entry looking for ${OS}.cf
     */
    for (Dir = RelativeConfDirs; Dir && *Dir; ++Dir) {
	CfDir = CFchooseConfDirRel(ProgramDir, *Dir, OSname);

	/* Make sure we always wind up where we started from */
	if (chdir(StartDir) != 0) {
	    SImsg(SIM_GERR, "%s: Change directory failed: %s",
		  StartDir, SYSERR);
	    return (char *) NULL;
	}
	...
	if (CfDir) {
	    SImsg(SIM_DBG, 
		  "Found Config Dir=<%s> ProgramDir=<%s>",
		  CfDir, ProgramDir);
	    return CfDir;
	}
    }

    SImsg(SIM_DBG, "Could not find relative config directory.");
    SImsg(SIM_DBG, "Config directory defaulting to <%s>", CFDIR);
