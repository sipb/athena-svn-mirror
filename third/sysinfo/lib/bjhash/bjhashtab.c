...
htab_t *hcreate(logsize)
Word_t  logsize;    /* log base 2 of the size of the hash table */
{
  void *Ptr;
  htab_t *t;
  ub4 i;
  ...
  Ptr = (void *) xcalloc(1, sizeof(htab_t));
  t = (htab_t *) Ptr;

  len = ((ub4)1<<logsize);
  t->table = (hitem_t **)xcalloc(1, sizeof(hitem_t *)*(ub4)len);
  for (i=0; i<len; ++i) t->table[i] = (hitem_t *)0;
  t->logsize = logsize;
  t->mask = len-1;
  t->count = 0;
  t->apos = (ub4)0;
  t->ipos = (hitem_t *)0;
  t->space = bjremkroot(sizeof(hitem_t));
  t->bcount = 0;
  return t;
}
...
htab_t  *t;    /* table */
{
  register ub4     newsize;
  register ub4     newmask;
  register ub4     i;
  register hitem_t **oldtab;
  register hitem_t **newtab;

  newsize = (ub4)1 << (++t->logsize);
  newmask = newsize-1;

  oldtab = t->table;
  newtab = (hitem_t **)xcalloc(1, newsize*sizeof(hitem_t *));

  /* make sure newtab is cleared */
  for (i=0; i<newsize; ++i) newtab[i] = (hitem_t *)0;
  t->table = newtab;
  t->mask = newmask;
  ...
  for (i=newsize>>1; i--;)
  {
    register hitem_t *this, *that, **newplace;
...
Word_t   hfind( t, key, keyl )
htab_t  *t;     /* table */
ub1   *key;   /* key to find */
ub4    keyl;  /* key length */
{
  hitem_t *h;
  ub4    x = bjlookup(key,keyl,0);
  ub4    y;
 ...
 */
Word_t hadd( t, key, keyl, stuff)
htab_t  *t;      /* table */
ub1   *key;    /* key to add to hash table */
ub4    keyl;   /* key length */
void  *stuff;  /* stuff to associate with this key */
{
  register hitem_t  *h,**hp;
  register ub4     y, x = bjlookup(key,keyl,0);
...
Word_t  hdel(t)
htab_t *t;      /* the hash table */
{
  hitem_t  *h;    /* item being deleted */
  hitem_t **ip;   /* a counter */
...
htab_t  *t;
{
  ...
  double  total = 0.0;
  hitem_t  *h;
  hitem_t  *walk, *walk2, *stat = (hitem_t *)0;
