     ...
     ProbeData_t	       *ProbeData;
{
    ...
    /*
     * Query this device via kstat() to find any additional info we can.
     */
    (void) KstatQuery(DevInfo);
...
/*
 * Is Path a swap device?
 */
static int IsSwapDev(Path)
     char		       *Path;
{
#if	defined(SC_LIST)
    register int		i;
    static struct swaptable	SwapTab;

    if (swapctl(SC_LIST, &SwapTab) == 0)
	for (i = 0; i < SwapTab.swt_n; ++i)
	    if (eq(SwapTab.swt_ent[i].ste_path, Path))
		return TRUE;
...
/*
 * Find partition usage for this device
 */
static int GetPartUsage(PartInfo)
     PartInfo_t		       *PartInfo;
{
    static FILE 	       *mntFilePtr;
    static FILE 	       *vfstabFilePtr;
	...
	if (EQ(cp, PartInfo->Name)) {
	    PartInfo->MntName = strdup(MntTab.mnt_mountp);
	    PartInfo->Type = strdup(MntTab.mnt_fstype);
	    if (EQ(MntTab.mnt_fstype, "swap")) { /* Unlikely, but try anyway */
		PartInfo->Usage = PIU_SWAP;
	    } else
		PartInfo->Usage = PIU_FILESYS;
	    if (MntTab.mnt_mntopts && !EQ(MntTab.mnt_mntopts, "-"))
		if (StrToArgv(MntTab.mnt_mntopts, ",", &Argv, NULL, 0) > 0)
    ...
    if (getvfsspec(vfstabFilePtr, &vfstab, PartInfo->DevPath) == 0) {
	if (EQ(vfstab.vfs_fstype, "swap")) {
	    PartInfo->Usage = PIU_SWAP;
	} else {
	    PartInfo->Usage = PIU_FILESYS;
	    if (vfstab.vfs_mountp && !EQ(vfstab.vfs_mountp, "-"))
		PartInfo->MntName = strdup(vfstab.vfs_mountp);
	    if (vfstab.vfs_fstype && !EQ(vfstab.vfs_fstype, "-"))
		PartInfo->Type = strdup(vfstab.vfs_fstype);
	    if (vfstab.vfs_mntopts && !EQ(vfstab.vfs_mntopts, "-"))
		if (StrToArgv(vfstab.vfs_mntopts, ",", &Argv, NULL, 0) > 0)
		    PartInfo->MntOpts = Argv;
	}
	return 0;
    }
    ...
    /*
     * This must be an unused partition.
     */
    PartInfo->Usage = PIU_UNUSED;
...
static int GetPartAmtUsed(Part)
     PartInfo_t		       *Part;
{
    static struct statvfs	Statvfs;

    if (!Part || Part->Usage != PIU_FILESYS)
	return -1;

    if (statvfs(Part->MntName, &Statvfs) != 0) {
	SImsg(SIM_DBG, "%s: statvfs failed: %s", Part->MntName, SYSERR);
	return -1;
    }

    if (!Part->Size)
	Part->Size = (Large_t) (Statvfs.f_blocks * Statvfs.f_frsize);

    Part->AmtUsed = (Large_t) ((Statvfs.f_blocks - Statvfs.f_bfree) * 
			       Statvfs.f_frsize);
    ...
    DKvtoc 		       *dk_vtoc;
{
	...
	(void) snprintf(Path, sizeof(Path),  "s%d", i);
	PartInfo->Name = strdup(Path);

	(void) snprintf(Path, sizeof(Path),  "%s/%s%s", _PATH_DEV_DSK,
			DevName, PartInfo->Name);
	PartInfo->DevPath = strdup(Path);

	(void) snprintf(Path, sizeof(Path),  "%s/%s%s", _PATH_DEV_RDSK,
			DevName, PartInfo->Name);
	PartInfo->DevPathRaw = strdup(Path);

	PartInfo->Num = i;
	PartInfo->StartSect = (Large_t) dk_vtoc->v_part[i].p_start;
	PartInfo->NumSect = (Large_t) dk_vtoc->v_part[i].p_size;
	/* Get the usage info */
     ...
     DKtype 		       *dk_type;
{
	...
	GotScsi = TRUE;
#if	defined(HAVE_ATA)
    AtaQuery(DevInfo, ProbeData->DevFile, DevFD, TRUE);
#endif	/* HAVE_ATA */
	...
	/*
	 * XXX It would be nice if we only read partition info when we
	 * are going to use it, but with the new mcSysInfo() API, it's not
	 * (Yet!) possible.  Reading partition info takes noticable amounts
	 * of time, especially on systems with many disks.
	 */
    ...
    Query.CtlDev = ProbeData->DevFile;
     ...
     int			CDspeed;
{
    ...
    (void) ScsiQuery(DevInfo, ProbeData->DevFile, ProbeData->FileDesc, 
		     TRUE);
     ...
     ProbeData_t	       *ProbeData;
{
    ...
    VolPathLen = strlen(VolRoot) + strlen(ProbeData->DevFile) + 2;
    VolPath = (char *) xmalloc(VolPathLen);

    (void) snprintf(VolPath, VolPathLen,  "%s%s", 
    ...
    if (!volmgt_acquire(VolPath, ProgramName, 0, NULL, NULL)) {
     ...
     int			UsingVolMgt;
{
     ...
     */
    if (DevDefine->Type == DT_DISKDRIVE || DevDefine->Type == DT_CD) {
...
extern char *GetMemorySunOSsysconf()
{
    ...
    Val = GetMemoryStr((Large_t) ((Amount+MBYTES-1) / MBYTES), MBYTES);
#endif	/* _SC_* */
    return(Val);
}
...
static void DetectDisks()
{
	 ...
	 */
	(void) snprintf(PathName, sizeof(PathName), "%s/%ss2", 
			_PATH_DEV_RDSK, DevName);
	if ((FileD = open(PathName, O_RDONLY|O_NDELAY)) > 0)
	    (void) close(FileD);
...
static void DetectTapes()
{
	 ...
	 */
	(void) snprintf(PathName, sizeof(PathName), "%s/%sn", 
			_PATH_DEV_RMT, DevName);
	if ((FileD = open(PathName, O_RDONLY|O_NDELAY)) > 0)
	    (void) close(FileD);
	NameListAdd(&UsedList, DevName);
    }
...
static int DetectFloppy()
{
    ...
    if ((FileD = open(DevName, O_RDONLY|O_NDELAY)) > 0)
	(void) close(FileD);
}
...
/*
 * Check the partition out and decide whether we're interested in it.
 */
static int CheckPart(Name, Type)
     char		       *Name;
     char		       *Type;
{
    if (!Type)
    ...
    /*
     * We only care about very specific filesystem types.  This check
     * is used to avoid listing "procfs", "tmpfs", etc.
     *
     * XXX - Filesystem types are hardcoded here.
     */
    if (EQ(Type, "ufs") || EQ(Type, "vxfs") || EQ(Type, "swap"))
	return 1;

    SImsg(SIM_DBG, "CheckPart: Special=<%s> Ignoring", Name);

    return 0;
}

/*
 * Build parititon information using local mounts info
 */
extern int BuildPartInfoMounts(PartInfoTree, SearchExp)
     ...
     char		      **SearchExp;
{
    ...
    SImsg(SIM_DBG, "BUILDING Partition Tree from mounts info...");

    /*
     * Try the static mount table, which may not reflect current reality.
     */
    if ((vfstabFilePtr = fopen(VFSTAB, "r")) == NULL) {
      SImsg(SIM_GERR, "%s: Cannot open for reading: %s.", 
	    VFSTAB, SYSERR);
    ...
    while (getvfsent(vfstabFilePtr, &vfstab) == 0) {
	SImsg(SIM_DBG, "GetVfsEnt: Special=<%s> Raw=<%s> Mnt=<%s> Type=<%s>",
	      PRTS(vfstab.vfs_special), PRTS(vfstab.vfs_fsckdev),
	      PRTS(vfstab.vfs_mountp), PRTS(vfstab.vfs_fstype));

	if (!vfstab.vfs_special)
	    continue;
	if (CheckPart(vfstab.vfs_special, vfstab.vfs_fstype) <= 0)
	    continue;

	if (cp = strrchr(vfstab.vfs_special, '/'))
	    ++cp;
	else
	    cp = vfstab.vfs_special;
	PartInfo = PartInfoCreate(NULL);
	PartInfo->DevName = strdup(cp);
	PartInfo->DevPath = strdup(vfstab.vfs_special);
	if (vfstab.vfs_fsckdev)
	    PartInfo->DevPathRaw = strdup(vfstab.vfs_fsckdev);
	...
	if (PartInfoAdd(PartInfoTree, PartInfo) != 0)
	    PartInfoDestroy(PartInfo);
    }

    /*
     * Try the current mount table
     */
    if ((mntFilePtr = fopen(MNTTAB, "r")) == NULL) {
	SImsg(SIM_GERR, "%s: Cannot open for reading: %s.", 
	      MNTTAB, SYSERR);
    ...
    while (getmntent(mntFilePtr, &MntTab) == 0) {
	SImsg(SIM_DBG, "GetMntEnt: Special=<%s> Mnt=<%s> Type=<%s>",
	      PRTS(MntTab.mnt_special), PRTS(MntTab.mnt_mountp), 
	      PRTS(MntTab.mnt_fstype));
	if (!MntTab.mnt_special)
	    continue;
	if (CheckPart(MntTab.mnt_special, MntTab.mnt_fstype) <= 0)
	    continue;
	if (cp = strrchr(MntTab.mnt_special, '/'))
	    ++cp;
	else
	...
	PartInfo = PartInfoCreate(NULL);
	PartInfo->DevPath = strdup(MntTab.mnt_special);
	PartInfo->DevName = strdup(cp);
	if (MntTab.mnt_mountp)
	    PartInfo->MntName = strdup(MntTab.mnt_mountp);
