...
char *GetModelSun()
{
    ...
    return (Name) ? (Name = strdup(Name)) : (char *)NULL;
}
...
#endif
#if	OSMVER == 4
	case DT_CD:		DevTypes[i].Probe = ProbeCDROMDrive;	break;
#endif	/* OSMVER==4 */
#if	defined(HAVE_OPENPROM)
    ...
    char		       *String;
{
	...
	if (*cp == '-' || *cp == '_') {
		...
		*cp = toupper(*cp);
	    ...
	    /* Lower case all but the first char */
	    *cp = tolower(*cp);
    }
...
/*
 * Issue ATA command and return the results.
 */
     ...
     AtaCmd_t		       *AtaCmd;
{
    ...
    (void) memset(Buff, 0, sizeof(Buff));
    (void) memcpy(Buff, &AtaCmd->Cdb, ATA_CDB_OFFSET);

    (void) memset(&Cmd, 0, sizeof(Cmd));
    Cmd.cmd = DADKIO_RWCMD_READ;
/*   Cmd.flags = DADKIO_FLAG_SILENT;	*/	/* No console messages */
    Cmd.blkaddr = 0;				/* ??? what is this? */
    Cmd.bufaddr = (caddr_t) Buff;
    Cmd.buflen = ATA_CDB_OFFSET + AtaCmd->DataLen;

    /*
     * Send cmd to device
     */
    if (ioctl(AtaCmd->DevFD, DIOCTL_RWCMD, &Cmd) < 0) {
	SImsg(SIM_GERR, "%s: ioctl DIOCTL_RWCMD for %s command failed: %s", 
	...
	if (Cmd.status.status > 0 && Cmd.status.status < NumDadErrs)
	    SImsg(SIM_DBG, "%s: %s query failed: %s",
		  AtaCmd->DevFile, AtaCmd->CmdName, 
		  DadErrs[Cmd.status.status]);
	else
	    SImsg(SIM_DBG, "%s: %s query failed with status %d.",
		  AtaCmd->DevFile, AtaCmd->CmdName, Cmd.status.status);
	return(-1);
    }

    AtaCmd->Data = (void *) (Buff + ATA_CDB_OFFSET);
     ...
     char		      **SearchExp;
{
    ...
    Status += BuildPartInfoDevices(PartInfoTree, SearchExp);
#if   OSMVER == 5
    Status += BuildPartInfoMounts(PartInfoTree, SearchExp);
#endif
