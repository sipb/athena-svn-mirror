    ...
    char		      **SearchNames;
{
	...
	if (cp = strchr(ObpModel, ',')) {
	    ObpModelBase = cp + 1;
	    Vendor = (char *) xmalloc(cp - ObpModel + 1);
    ...
    char		      **SearchNames;
{
    ...
    SImsg(SIM_DBG, 
	  ...
	  Node->NodeID, (Node->Name && Node->Name[0]) ? Node->Name : "", 
    ...
    for (Cond = SubSysDef->Conditions; Cond; Cond = Cond->Next) {
 	if (EQ("HasDev", Cond->Key) && Cond->StrVal) {
		...
		if (SubSysVar.IntVal >= (Cond->IntVal - CPUspeedDelta) &&
		    ...
		    Cond->Matches = TRUE;
	    } else if (EQ("BannerName", Cond->Key)) {
	        if (strstr(SubSysVar.StrVal,Cond->StrVal))
		    Cond->Matches = TRUE;
	    }
	} else {
