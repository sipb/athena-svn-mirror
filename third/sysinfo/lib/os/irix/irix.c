...
extern char *GetKernVerSGI()
{
    ...
    snprintf(Buff, sizeof(Buff),
	     ...
	     GetOSVerSGI(), GetKernArchSysinfo(),
 ...
 * that IRIX uses.
 */
static int IsSpecialUse(MntDevice, DevPath)
    char		       *MntDevice;
    char		       *DevPath;
{
	...
	SImsg(SIM_GERR, "stat failed: %s: %s", DevPath, SYSERR);
	return(0);
    }
 ...
 */
static int GetMountInfo(PartInfo, FilePtr)
    PartInfo_t		       *PartInfo;
    FILE		       *FilePtr;
{
	...
	if (EQ(MntEnt->mnt_fsname, PartInfo->DevPath) || 
	    IsSpecialUse(MntEnt->mnt_fsname, PartInfo->DevPath)) {
 ...
 */
static int PartUseLookup(PartInfo)
     PartInfo_t		       *PartInfo;
{
    ...
    if (GetMountInfo(PartInfo, mntFilePtr) == 0)
	return 0;
    ...
    if (GetMountInfo(PartInfo, fstabFilePtr) == 0)
	return 0;
...
/*
 * Get amount/space used for this partition
 */
 ...
 */
static void GetPartUse(PartInfo)
     PartInfo_t		       *PartInfo;
{
    char			Type[128];
    char			Usage[128];

    PartUseLookup(PartInfo);
    if (PartInfo->Usage && PartInfo->Type)
	return;

    Type[0] = Usage[0] = CNULL;

    switch (PartInfo->TypeNum) {
    ...
    if (Type[0] && !PartInfo->Type)
	PartInfo->Type = strdup(Type);

    if (Usage[0] && !PartInfo->Usage)
	PartInfo->UsageStatus = strdup(Usage); 
}
    ...
    struct volume_header       *VolHdr;
{
	...
	PartInfo->StartSect = (Large_t) VolHdr->vh_pt[i].pt_firstlbn;
	PartInfo->NumSect = (Large_t) VolHdr->vh_pt[i].pt_nblks;
	/* Store pt_type for possible later use in finding drive capacity */
	PartInfo->TypeNum = VolHdr->vh_pt[i].pt_type;
	/* What's it used for? */
	GetPartUse(PartInfo);
	/* How much space is used? */
	GetPartAmtUsed(PartInfo);
 ...
 */
static float IRIXGetDiskSize(DiskDrive)
    DiskDrive_t		       *DiskDrive;
{
	...
	if (dp->PartInfo && dp->PartInfo->TypeNum == PTYPE_VOLUME) {
	    SImsg(SIM_DBG, "DISK PTYPE_VOLUME numsects=%.0f size=%.0f",
		  (float) dp->PartInfo->NumSect, 
		  (float) DiskDrive->SecSize);
	    return (float) ((float)dp->PartInfo->NumSect * 
			    (float)DiskDrive->SecSize);
	}
    }
#else
     ...
     char		       *DevFile;
{
	...
	if (HWdisk && !HWdisk->Size && DiskCap && OSdisk && OSdisk->SecSize) {
    ...
    inventory_t		       *Inv;
{
	...
	if (Def->ValStr1)
	    DevInfo->Model = Def->ValStr1;
	if (Def->ValStr2) {
	    (void) snprintf(DevName, sizeof(DevName),  "%s%d-%d", 
			    Def->ValStr2, 
			    Inv->inv_controller,
    ...
    inventory_t		       *Inv;
{
    ...
    if (!DevInfo->Name) {
	(void) snprintf(DevName, sizeof(DevName), "prom%d-%d", 
			Inv->inv_controller, DevInfo->Unit);
	DevInfo->Name = strdup(DevName);
    }
    if (!DevInfo->Model) {
    ...
    inventory_t		       *Inv;
{
    ...
    snprintf(Name, sizeof(Name), "%s%d-%d", 
	     isprint(GFXinfoPtr->name) ? GFXinfoPtr->name : "graphics",
	     Inv->inv_controller, Inv->inv_unit);
    ...
    inventory_t		       *Inv;
{
	...
	Class = CT_CPUBOARD;
	/* 
	 * CPU's rarely have a valid Unit number, but we need to assign
	 * something so the device is properly handled.
	 */
	Unit = Inv->inv_unit;

	(void) snprintf(DevName, sizeof(DevName), "cpuboard%d", Unit);
	...
	if (cp = GetCpuTypeSGI())
	    (void) snprintf(Desc, sizeof(Desc), "%s %d MHz %s CPUs",
			    GetNumCpuSGI(), Inv->inv_controller, strupper(cp));
	else 
	    (void) snprintf(Desc, sizeof(Desc), "%s %d MHz",
			    GetNumCpuSGI(), Inv->inv_controller);
	break;
    case INV_CPUCHIP:
	Class = CT_CPU;
	...
	Class = CT_CPU;
	Unit = Inv->inv_unit;
	(void) snprintf(DevName, sizeof(DevName), "processor%d", Unit);
    ...
    if (DevName[0]) {
	DevInfo = (DevInfo_t *) xcalloc(1, sizeof(DevInfo_t));
	DevInfo->Addr = DevInfo->Prio = DevInfo->Vec = -1;
	DevInfo->Unit = Unit;
	DevInfo->Name = strdup(DevName);
	if (Model[0])
	    DevInfo->Model = strdup(Model);
    ...
    char		       **Names;
{
	...
	default:
	    DevInfo = (DevInfo_t *) xcalloc(1, sizeof(DevInfo_t));
	    DevInfo->Unit = Inv->inv_unit;
	    if (Def = DefGet("InvItem", NULL, Inv->inv_class, 0)) {
		(void) snprintf(Buff, sizeof(Buff), "%s%d", 
				(Def->ValStr2) ? Def->ValStr2 : "generic", 
				DevInfo->Unit);
		DevInfo->Name = strdup(Buff);
		if (Def->ValStr1)
		    DevInfo->Model = strdup(Def->ValStr1);
	    } else {
