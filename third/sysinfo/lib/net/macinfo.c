...
#if	GETMAC_TYPE == GETMAC_SIOCGIFHWADDR
...
/*
 * Find and set the MAC info using SIOCGIFHWADDR
 */
extern void SetMacInfo(DevInfo, NetIf)
     DevInfo_t		       *DevInfo;
     NetIF_t 		       *NetIf;
{
    struct ifreq	        ifreq;
    static struct ether_addr	EtherAddr;
    struct ether_addr	       *ether_aton();
    static char			Buff[256];
    char		       *cp;
    static int			Sock = -1;
	...
	SImsg(SIM_GERR, "%s: ioctl SIOCGIFHWADDR failed: %s.", 
	      ifreq.ifr_name, SYSERR);
	close(Sock);
	return;
    }
    ...
    (void) memcpy(&EtherAddr, (void *) ifreq.ifr_ifru.ifru_hwaddr.sa_data, 
		  sizeof(EtherAddr));

    if (cp = ether_ntoa(&EtherAddr))
        NetIf->MACaddr = strdup(cp);
    if (ether_ntohost(Buff, &EtherAddr) == 0)
	NetIf->MACname = strdup(Buff);
}
