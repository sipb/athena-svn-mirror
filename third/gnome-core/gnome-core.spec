...
%changelog
* Tue Mar 25 2001 Jens Finke <jens@gnome.org>
- Make use of predefined rpm macros. 
- Include gnome-terminal datadir stuff.
...
%build
%configure --quiet --disable-gtkhtml-help
CFLAGS="$RPM_OPT_FLAGS" make

%install
[ -n "$RPM_BUILD_ROOT" -a "$RPM_BUILD_ROOT" != / ] && rm -rf $RPM_BUILD_ROOT

%makeinstall > install.log 2>&1

#if [ -d $RPM_BUILD_ROOT%{_bindir} ] ; then
# strip `file $RPM_BUILD_ROOT%{_bindir}/* | grep ELF | cut -d':' -f 1`
#fi
if [ -d $RPM_BUILD_ROOT%{_mandir} ]; then
  find $RPM_BUILD_ROOT%{_mandir} -type f -exec gzip -9f {} \;
fi
...
function BuildFiles() {
  ...
  for i in `find $RPM_BUILD_ROOT%{_datadir} -maxdepth 1 -type d -print | \
    ...
    echo $i
    case $i in
     /applets|/control-center|/gnome|/gnome-about|/mc|/panelrc|/pixmaps|/gnome-terminal)
  ...
  ProcessDir $RPM_BUILD_ROOT%{_datadir}/idl %{name}-devel.files "0444, bin, bin, 0555"
  ProcessLib $RPM_BUILD_ROOT%{_libdir} devel %{name}-devel.files
  ProcessDir $RPM_BUILD_ROOT%{_includedir} %{name}-devel.files "0444, bin, bin, 0555"
}
