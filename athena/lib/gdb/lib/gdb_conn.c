...
char *id;					/* null terminated string */
{
	...
	GDB_INIT_CHECK
       ...
       /* 
        * Try to allocate a connection descriptor and fill it in with
        * null values.
        */
	...
	con = g_make_con();
 	...
 	if(!g_try_connecting(con,id)) {
       ...
       /*
        * The g_ver... routines have set the status to CON_UP iff
        * we have matching protocols with the other side.
        */
	if (con->status == CON_UP) {
		...
		if (con->in.fd +1 > gdb_mfd) 
			gdb_mfd = con->in.fd + 1;
                ...
                * This buffer will be used so that we can do relatively long
                * reads from the sockets, which tends to avoid the so-called
                * "silly" window problem (i.e. lots of line turnarounds caused
                * by short reads when the writer is running ahead of the 
                * reader)
                */
...
CONNECTION con;
{
        ...
        */
	con->id = GDB_CON_ID;			/* eye catcher for debugging */
...
gdb_allocate_connection()
{
	register int i;				/* index of next one */
						/* to check */
	register int mcons = gdb_mcons;		/* optimization */
...
char *id;
{
	...
	if (peer < 0) {
		gdb_perror("gdb: unexpected error creating connecting socket");
		g_stop_with_errno(con);
	...
	/*----------------------------------------------------------*/
	/*	
	/*	Copy the address/port to connect to, then 
	/*	make the connection.
	/*	
	/*----------------------------------------------------------*/
...
/*			and it may be disabled in production versions
/*			of the gdb system.)
/*	
...
u_short *port;
{
	...
	*host = gethostbyname(buffer);
	...
	/*----------------------------------------------------------*/
	/*	
	/*	Set up the port address - note the 3 forms
	/*	listed above.  They are :servicename, :#port-num
	/*	or null.
	/*	
	/*----------------------------------------------------------*/
...
char *id;
{
	...
	(void) listen (slisten, 5);		/* does not block, just */
						...
						/* cons. */
	fromlen = sizeof(from);
	peer = accept(slisten, &from, &fromlen); /* note: from parameter */
						 /* is required but */
						 /* ignored below*/
	if (peer < 0) {
		g_stop_with_errno(con);
...
/*	
/*	The GDB_NOPROTOCHK flag is provided for those clients who
/*	can't afford the time taken to do this at connection startup
/*	time.
/*	
/************************************************************************/
...
CONNECTION con;
{
	...
	if (!(gdb_Debug & GDB_NOPROTOCHK)) {	/* usual case: we verify */
						/* protocol levels */
		/*
		 * Because the connection may have been (server/client case)
		 * accepted on a non-blocking listening socket, the connection
		 * itself may be non-blocking.  We can't tolerate that here.
		...
		/*
		 * Write our protocol version number to other side.  It's encoded
		 * as a single byte (no byte order problems that way.)
		 */
		while (len=write(con->out.fd, &ver, 1) < 0) {
			if (len==0)
			  	continue;
			gdb_perror("write error on protocol version number");
			g_stop_with_errno(con);
			return;
		}
		
		/*
		 * Read back corresponding version number from other side
		 */
		do {
			len = read(con->in.fd, &theirs, 1);
			if (len == (-1)) {
				gdb_perror("read error on protocol version number");
				g_stop_with_errno(con);
				return;
			}
		} while (len !=1);
		...
		/*
		 * See whether we really are at the same protocol level
		 */
		if (theirs == ver)
		  	con->status = CON_UP;
		else
		  	con->status = CON_STOPPED;
	} else {				/* don't check protocol */
		con->status = CON_UP;
	}
}
...
/*	the version of the protocol being observed.  Actually, there is no 
/*	such thing as an inbound or outbound connection, except insofar 
/*	as we arbitrarily decide who sends the first protocol check
/*	byte and who reads first.
/*	
/************************************************************************/
...
/*	
/*	This routine is used by applications.  GDB code calls 
/*	g_stop_connection or g_stop_with_errno.  The important 
/*	distinction is that the g_stop routines leave the connection
/*	descriptor allocated, but in the CON_STOPPING state.  This
/*	routine de-allocates the descriptor.  In general, only
/*	applications can authorize de-allocation of a descriptor.
/*	
/************************************************************************/

CONNECTION
sever_connection(conxn)
CONNECTION conxn;
{
	register CONNECTION con = conxn;
...
CONNECTION con;
{
	register int status = connection_status(con);
       /*
        * Don't cleanup a connection which is already stopped.
        */
	if (status != CON_UP && 
	    status != CON_STARTING)
		return;
...
CONNECTION con;
{
	...
	con->status = CON_STOPPED;
       ...
       /*
        * Reset gdb_mcons to be one more than the index of the last
        * connection descriptor currently in use.  Note that, due to
        * "holes" gdb_mcons may drop by much more than one here.
        */
...
/*	types of operation are queued on a connection of this sort. 
/*	
...
char *id;
{
	...
	struct sockaddr_in self;		/* our own address */
	int retries = GDB_BIND_RETRY_COUNT;
	int onoff = 1;				/* used as argument to */
						/* setsockopt */
	struct servent *serv;			/* entry from /etc/services */

	GDB_INIT_CHECK				/* make sure GDB is started */

       /* 
        * Try to allocate a connection descriptor Aand fill it in with
        * null values.
        */
	con = g_make_con();
	...
	/* that didn't work, try 4.3 */
        ...
        */
	if (ioctl(slisten, FIONBIO, (char *)&onoff) < 0) {
		g_stop_with_errno(con);
	...
	con->in.flags |= HCON_LISTEN;		/* mark as special listening */
						/* connection*/
	con->out.flags |= HCON_UNUSED;		/* outbound half connection */
						/* remains unused*/
	con->status = CON_UP;			/* listening connection is */
						/* read for use by caller*/

	if (con->in.fd +1 > gdb_mfd)		/* bump high water mark */
...
CONNECTION con;
{
	register HALF_CONNECTION inbound = &(con->in);
