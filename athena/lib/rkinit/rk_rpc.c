...
static const char rcsid[] = "$Id: rk_rpc.c,v 1.2 1999-12-09 22:23:55 danw Exp $";
...
int rki_send_packet(int sock, char type, u_long length, char *data)
{
    ...
    packet = calloc(pkt_len, sizeof(u_char));
    if (packet == NULL) {
	sprintf(errbuf, "rki_send_packet: failure allocating %lu bytes",
    ...
    len = write(sock, packet, pkt_len);
    if (len != pkt_len) {
	if (len == -1)
	    sprintf(errbuf, "write: %s", strerror(errno));
	else
	    sprintf(errbuf, "write: %lu bytes written; "
		    "%d bytes actually sent", pkt_len, len);
	rkinit_errmsg(errbuf);
	return(RKINIT_WRITE);
    }
...
int rki_get_packet(int sock, char type, u_long *length, char *data)
{
	...
	sprintf(errbuf, "read: expected to receive %d bytes, "
		"but only had room for %lu", data_length, *length);
	rkinit_errmsg(errbuf);
...
int rki_setup_rpc(char *host)
{
    ...
    sock = socket(hp->h_addrtype, SOCK_STREAM, IPPROTO_IP);
    if (sock < 0) {
				  ...
				  int *s_lversion, int *s_hversion)
{
    ...
    status = rki_send_packet(sock, MT_CVERSION, length, (char *)version_info);
    if (status != RKINIT_SUCCESS)
	return(status);

    status = rki_get_packet(sock, MT_SVERSION, &length, (char *)version_info);
    if (status != RKINIT_SUCCESS)
	return(status);
...
int rki_rpc_get_status(void)
{
    ...
    status = rki_get_packet(sock, MT_STATUS, &length, msg);
    if (status)
	return(status);
...
int rki_rpc_get_ktext(int sock, KTEXT auth, u_char type)
{
    ...
    status = rki_get_packet(sock, type, &length, (char *)auth->dat);
    if (status)
	return(status);
