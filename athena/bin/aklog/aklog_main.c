...
static int auth_to_cell(char *cell, char *realm)
{
  ...
  if (!noauth)
      ...
      if (status != KSUCCESS)
	{
	  if (dflag)
	    {
	      sprintf(msgbuf, "Kerberos error code returned by get_cred: %d\n",
		      status);
	      params.pstdout(msgbuf);
	    }
...
static char *next_path(char *origpath)
{
  ...
  do
	...
	strncat(pathtocheck, last_comp++, 1);
      ...
      if (link = (params.readlink(pathtocheck, linkbuf, sizeof(linkbuf)) > 0))
	{
  ...
  for (i = 1; i < argc; i++)
      ...
      if (cmode)
	{
	  ...
	  if (cur_node = ll_add_node(&cells, ll_tail))
		...
		{
		  sprintf(msgbuf, "%s: failure copying cellinfo.\n", progname);
		  params.pstderr(msgbuf);
		  params.exitprog(AKLOG_MISC);
		}
	    }
