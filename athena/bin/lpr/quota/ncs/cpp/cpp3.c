...
#define	MAXINCLUDE	(NINCLUDE - 3 - IS_INCLUDE)
	...
	if ((incl_list = getenv("INCLUDE")) != NULL) {
		strncpy(buf, incl_list, sizeof(buf));
		for (start = buf; *start != '\0'; start = end+1) {
			ninc++;
...
#define	MAXINCLUDE	(NINCLUDE  - IS_INCLUDE - ninc)
#endif
}

int
