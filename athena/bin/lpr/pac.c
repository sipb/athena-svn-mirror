...
/* Currently "pac" runs only off information in /etc/printcap.
 * If you want it to query Hesiod, #define HESIOD-PAC here.
 */
...
fprintf(stderr,
	...
	if (printer == NULL) {
	  if ((printer = getenv("PRINTER")) == NULL) printer = DEFLP;
	}
	if (!chkprinter()) {
