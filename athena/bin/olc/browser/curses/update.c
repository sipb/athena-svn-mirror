 ...
 *	$Author: treese $
 *	$Locker:  $
 *	$Header: /afs/dev.mit.edu/source/repository/athena/bin/olc/browser/curses/update.c,v 1.1 1986-01-18 18:09:37 treese Exp $
 */

#ifndef lint
static char *rcsid_update_c = "$Header: /afs/dev.mit.edu/source/repository/athena/bin/olc/browser/curses/update.c,v 1.1 1986-01-18 18:09:37 treese Exp $";
#endif	lint
...
#ifndef lint
static char rcsid[] = "$Header: /afs/dev.mit.edu/source/repository/athena/bin/olc/browser/curses/update.c,v 1.1 1986-01-18 18:09:37 treese Exp $";
#endif
...
static char Current_Dir[FILENAME_SIZE];		/* Current CREF directory. */
static int Current_Entry;			/* Current CREF entry. */
static ENTRY Entry_Table[MAX_ENTRIES];		/* Table of CREF entries. */
static int Entry_Count;				/* Number of entries. */
static int Index_Start;				/* Current top of index. */
static int Is_Consultant;			/* Is user a consultant? */

/* Function:	get_current_dir() gets the name of the current directory.
 * Arguments:	buf:	Buffer to copy name into.
 * Returns:	Nothing.
 * Notes:
 */
...
/* Function:	set_current_index() sets the current entry variable.
 * Arguments:	Index:	New index value.
 * Returns:	Nothing.
 * Notes:
 */
...
/* Function:	get_current_entry() returns the current index in the entry
 *			table.
 * Arguments:	None.
...
parse_contents()
{
	...
	if ( (infile = fopen(contents_name, "r")) == NULL)
		...
		if ( open(contents_name, O_RDONLY, 0) < 0)
			...
			if (errno == EACCES)
				...
				sprintf(error,
				  "cref: parse_contents: Can't open file %s\n",
				  contents_name);
				message(1, error);
		...
		else message(1, "cref: parse_contents: strangeness here.\n");
		}
	i = 0;
	while ( fgets(inbuf, LINE_LENGTH, infile) != NULL)
		...
		*delim_ptr = (char) NULL;
		...
		else Entry_Table[i].type = CREF_DIR;
...
/* Function:	entry_count() returns the current number of entries in
 *			the entry table.
 * Arguments:	None.
 * Returns:	Current number of entries.
 * Notes:
 *	This function uses the local state variable Entry_Count.
 */
...
/* Function:	set_consultant() identifies a user as a consultant.
 * Arguments:	consultant:	Whether or not the user is a consultant.
 * Returns:	Nothing.
 * Notes:
 *	This function uses the local state variable Is_Consultant.
 */
...
int consultant;
{
	if (consultant != FALSE)
		Is_Consultant = TRUE;
	else Is_Consultant = FALSE;
}

/* Function:	is_consultant() tells if a user is a consultant.
 * Arguments:	None.
...
/* Function:	set_index_start() sets the start of the index page.
 * Arguments:	index:	Index to start with.
 * Returns:	Nothing.
 * Notes:
 */

set_index_start(index)
int index;
{
	if (index < 1)
		Index_Start = 1;
	else if (index > Entry_Count)
		Index_Start = Entry_Count;
	else Index_Start = index;
}

/* Function:	get_index_start() gets the start of the index page.
 * Arguments:	None.
 * Returns:	The starting index.
 * Notes:
 *	This function uses the local state variable Index_Start.
 */
...
int index;
{
	if (index > Entry_Count)
		return(NULL);
	else return( &(Entry_Table[index - 1]) );
}
