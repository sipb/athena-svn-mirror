     ...
     void *first_list, *second_list;
{
  sort_keys *k;
  int order = 0;
  LIST *first = (LIST *) first_list;
  LIST *second = (LIST *) second_list;
     ...
     sort_keys *skeys;
{
  ...
  for (n_entries = 0; list[n_entries].ustatus != END_OF_LIST; n_entries++)
    ;
  qsort(list, n_entries, sizeof (*list), compare_lists);
  return 0;
}
