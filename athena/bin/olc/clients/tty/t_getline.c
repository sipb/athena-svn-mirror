...
static char     rcsid[] =
  "$Id: t_getline.c,v 1.1.1.1 1992-03-05 19:44:21 lwvanels Exp $";
...
gl_char_init()			/* turn off input echo */
{
  ...
  ioctl(0, TCSETA, &new_termios);
#else /* !POSIX */
/* We want to be able to preserve whatever the user has typed ahead. */
/* However, under BSD tty's, changing to raw mode throws out whatever */
/* pending input there might be.  Sooo, change to `cbreak' mode first to */
/* see whether there is any pending input, and if so read it, before */
...
int     c;
{
  ...
  write(1, &ch, 1);
#ifndef _POSIX_SOURCE /* BSD in RAW mode, map NL to NL,CR */
  if (ch == '\n') {
    ch = '\r';
    write(1, &ch, 1);
  }
