     ...
     char *topic;
{
  ...
  if (statbuf.st_size == 0)
    {
      printf("You have no question to ask.\n");
      (void) unlink(file);
      return(SUCCESS);
    }
  ...
  switch(status)
    {
    case NOT_CONNECTED:
      printf("\nThere is no specialist currently");
      printf(" available for topic '%s'. Your\n", topic);
      printf("request will be forwarded to the first available");
      printf(" consultant.\n");
      status = SUCCESS;
      break;
    case CONNECTED:
