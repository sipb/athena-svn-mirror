 ...
 * 	$Author: bert $  $Revision: 1.3 $
 */

#ifndef lint
static char *rcsid = "$Header: /afs/dev.mit.edu/source/repository/athena/etc/newsyslog/newsyslog.c,v 1.3 1995-11-15 21:25:39 bert Exp $";
#endif  /* lint */
...
#define SLEEP_DELAY 1		/* Delay used after restarting each daemon, */
				/* to give it time to clean up (in seconds) */
...
/* This wastes a little space, but makes my life easier. */
struct flag_entry {
	char	option;		/* Option character used for this program */
	enum { EXEC, PID } type;  /* Type of flag ('run' or 'signal') */
...
struct flag_entry *flags;	/* several functions need to read or */
				/* change this as a side effect */
...
/* Tack a character to the end of a string. (Yukky.) */
   ...
   if (ext && (*ext)) {
	 ...
	 if ((cfl->type == EXEC) && (cfl->extension)) {
	    ...
	    if (!strncmp(ext, cfl->extension, len)) {
	       if (ext[len] == '\0')       /* end of string */
		  return 1;
	       else if (ext[len] == '.') { /* check the remainder of string */
		  if (valid_extension(ext + len + 1))
		     return 1;
		  /* if valid_ext returns false, try other possibilities;
		     that way, we can support aggregate extensions (tar.gz) */
	       }
	    }
	 }
	 cfl = cfl->next;
...
char *matching_file(char* name)
{
   ...
   if (have_path = (NULL != (namefrag = strrchr(dirname, '/')))) {
      *(namefrag++) = '\0';
   } else {
      namefrag = dirname;
      dirname = ".";
...
void do_compress(struct log_entry *ent)
{
   ...
   if (!noaction && !(ent->flags & CE_BINARY)) {
      old = (char*) malloc((unsigned) strlen(ent->log) + 3);
      if (! old) abort();
      strcpy (old, ent->log);
      strcat (old, ".0");
	...
	char **argv;
{
	...
	while ((c=getopt(argc,argv,"nrvf:")) != EOF)
		switch (c) {
		case 'n':
...
void parse_pid_line(char *line, struct flag_entry **first)
{
   ...
   if (!working) {
      (*first) = working = (struct flag_entry *) malloc(sizeof(struct flag_entry));
      if (!working) abort();
   } else {
      while (working->next)
	 working = working->next;
   ...
   (*parse) = oops;   /* falling off the edge of the world is bad. */
                      /* (parse may have been pointing at the end of line) */
...
void add_default_flags(struct flag_entry **first)
{
   ...
   if (! have_Z) {
      ...
      working->args = (char**) malloc(3*sizeof(char*));
      if (! working->args) abort();
...
#ifdef DEBUG
	{	
	   struct flag_entry *dbg;
	   int i;

	   for (dbg=(*flags); dbg; dbg=dbg->next) {
	      fprintf(stderr, "flag '%c':\n", dbg->option);
	      if (dbg->type == EXEC) {
		 if (dbg->extension)
		    fprintf(stderr, "    ext='%s'\n", dbg->extension);
		 for (i=0; (dbg->args) && (i<(dbg->nargs)); i++)
		    fprintf(stderr, "    arg[%d]='%s'\n", i, dbg->args[i]);
	      } else if (dbg->type == PID) {
		 fprintf(stderr, "    pidf='%s', sig=", dbg->pidfile);
		 fprint_signal_name(stderr, dbg->signal);
		 fprintf(stderr, "\n    rc=%d\n", dbg->ref_count);
	      }
	   }
	}
