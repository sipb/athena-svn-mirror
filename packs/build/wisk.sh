...
switch ( $machine )
  ...
  case sun4
    set machthird="third/unsupported/ditroff third/supported/talk third/unsupported/x3270 third/unsupported/transcript-v4.1 third/unsupported/ansi third/unsupported/jove third/unsupported/learn third/supported/mae-2.0"
    breaksw
  ...
  case sgi
    set machthird="third/unsupported/transcript-v4.1 athena/ucb/look third/unsupported/x3270 athena/lib/AL"
endsw
...
switch ( $machine )
  ...
  case sgi
    set packages =(setup $libs1 $tools $third $machthird $libs2 $etcs $bins \
	athena/bin/AL athena/bin/telnet $end)
endsw
...
if ($machine == "sun4" || $machine == "sgi") then
	rm $BUILD/transarc
	ln -s /afs/athena/system/afsuser/dist/3.4a.patches.1/@sys \
		$BUILD/transarc
endif
