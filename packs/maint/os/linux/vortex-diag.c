...
main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "#:aA:DeEfF:gG:mp:P:qrRst:vVx:wWH:BL:S:",
		...
		case 'p':
			...
			break;
		case 'P': set_ee_rom++; set_ee_rom_val = atoi(optarg); break;
...
static int get_media_index(const char *name)
{
	...
	for (i = 0; mediamap[i].name; i++)
		if (strcasecmp(name, mediamap[i].name) == 0)
			return i;
	if (name) {
		val = strtol(name, &errstr, 16);
		if (errstr == NULL)
			for (i = 0; mediamap[i].name; i++)
				if (mediamap[i].value == val)
					return i;
	}
...
int vortex_diag(int vendor_id, int dev_id, long ioaddr, int part_idx)
{
	...
	if (set_ee_rom) {
		...
		memcpy(new_ee_contents, eeprom_contents, eesize << 1);
		if (set_ee_rom_val == 1) {
			new_ee_contents[9] |= 0x0001;
			new_ee_contents[0x13] &= 0xfdff;
		}
		else {
			new_ee_contents[9] &= 0xfffe;
			new_ee_contents[0x13] |= 0x0200;
		}
		/* Recalculate the checksum: Cyclone only! */
		for (i = 0; i < 0x1B; i++)
	...
	if (set_ee_media) {
		...
		if (set_ee_media_auto == 1) {
			...
			if (set_ee_media_speed == 100) {
				...
				new_ee_contents[0x15] &= 0xfff2;
				new_ee_contents[0x15] |= 0x0002;
			}
		}
		/* Recalculate the checksum: Cyclone only! */
		for (i = 0; i < 0x1B; i++)
