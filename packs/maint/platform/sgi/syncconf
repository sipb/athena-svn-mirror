...
handle()
{
  ...
  SENDMAIL|sendmail)
    if [ "${SENDMAIL}" = "false" ]; then
      quiet_move /etc/rc2.d/S50mail /etc/rc2.d/s50mail
      quiet_move /etc/rc0.d/K20mail /etc/rc0.d/k20mail
    else
      move /etc/rc2.d/s50mail /etc/rc2.d/S50mail
      move /etc/rc0.d/k20mail /etc/rc0.d/K20mail
    fi
    ;;
  ...
  SAVECORE|savecore)
    if [ "${SAVECORE}" = "false" ]; then
      quiet_move /etc/rc2.d/S48savecore /etc/rc2.d/s48savecore
    else
      case "${SAVECORE}" {
